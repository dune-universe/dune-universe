module CI = Cstubs_internals

external _1_EverCrypt_HKDF_expand_sha1
  : Bytes.t CI.ocaml -> Bytes.t CI.ocaml -> Unsigned.uint32 ->
    Bytes.t CI.ocaml -> Unsigned.uint32 -> Unsigned.uint32 -> unit
  = "_1_EverCrypt_HKDF_expand_sha1_byte6" "_1_EverCrypt_HKDF_expand_sha1" 

external _2_EverCrypt_HKDF_extract_sha1
  : Bytes.t CI.ocaml -> Bytes.t CI.ocaml -> Unsigned.uint32 ->
    Bytes.t CI.ocaml -> Unsigned.uint32 -> unit
  = "_2_EverCrypt_HKDF_extract_sha1" 

external _3_EverCrypt_HKDF_expand_sha2_256
  : Bytes.t CI.ocaml -> Bytes.t CI.ocaml -> Unsigned.uint32 ->
    Bytes.t CI.ocaml -> Unsigned.uint32 -> Unsigned.uint32 -> unit
  =
  "_3_EverCrypt_HKDF_expand_sha2_256_byte6" "_3_EverCrypt_HKDF_expand_sha2_256"
  

external _4_EverCrypt_HKDF_extract_sha2_256
  : Bytes.t CI.ocaml -> Bytes.t CI.ocaml -> Unsigned.uint32 ->
    Bytes.t CI.ocaml -> Unsigned.uint32 -> unit
  = "_4_EverCrypt_HKDF_extract_sha2_256" 

external _5_EverCrypt_HKDF_expand_sha2_384
  : Bytes.t CI.ocaml -> Bytes.t CI.ocaml -> Unsigned.uint32 ->
    Bytes.t CI.ocaml -> Unsigned.uint32 -> Unsigned.uint32 -> unit
  =
  "_5_EverCrypt_HKDF_expand_sha2_384_byte6" "_5_EverCrypt_HKDF_expand_sha2_384"
  

external _6_EverCrypt_HKDF_extract_sha2_384
  : Bytes.t CI.ocaml -> Bytes.t CI.ocaml -> Unsigned.uint32 ->
    Bytes.t CI.ocaml -> Unsigned.uint32 -> unit
  = "_6_EverCrypt_HKDF_extract_sha2_384" 

external _7_EverCrypt_HKDF_expand_sha2_512
  : Bytes.t CI.ocaml -> Bytes.t CI.ocaml -> Unsigned.uint32 ->
    Bytes.t CI.ocaml -> Unsigned.uint32 -> Unsigned.uint32 -> unit
  =
  "_7_EverCrypt_HKDF_expand_sha2_512_byte6" "_7_EverCrypt_HKDF_expand_sha2_512"
  

external _8_EverCrypt_HKDF_extract_sha2_512
  : Bytes.t CI.ocaml -> Bytes.t CI.ocaml -> Unsigned.uint32 ->
    Bytes.t CI.ocaml -> Unsigned.uint32 -> unit
  = "_8_EverCrypt_HKDF_extract_sha2_512" 

external _9_EverCrypt_HKDF_expand
  : Unsigned.uint8 -> Bytes.t CI.ocaml -> Bytes.t CI.ocaml ->
    Unsigned.uint32 -> Bytes.t CI.ocaml -> Unsigned.uint32 ->
    Unsigned.uint32 -> unit
  = "_9_EverCrypt_HKDF_expand_byte7" "_9_EverCrypt_HKDF_expand" 

external _10_EverCrypt_HKDF_extract
  : Unsigned.uint8 -> Bytes.t CI.ocaml -> Bytes.t CI.ocaml ->
    Unsigned.uint32 -> Bytes.t CI.ocaml -> Unsigned.uint32 -> unit
  = "_10_EverCrypt_HKDF_extract_byte6" "_10_EverCrypt_HKDF_extract" 

external _11_EverCrypt_HKDF_hkdf_expand
  : Unsigned.uint8 -> Bytes.t CI.ocaml -> Bytes.t CI.ocaml ->
    Unsigned.uint32 -> Bytes.t CI.ocaml -> Unsigned.uint32 ->
    Unsigned.uint32 -> unit
  = "_11_EverCrypt_HKDF_hkdf_expand_byte7" "_11_EverCrypt_HKDF_hkdf_expand" 

external _12_EverCrypt_HKDF_hkdf_extract
  : Unsigned.uint8 -> Bytes.t CI.ocaml -> Bytes.t CI.ocaml ->
    Unsigned.uint32 -> Bytes.t CI.ocaml -> Unsigned.uint32 -> unit
  = "_12_EverCrypt_HKDF_hkdf_extract_byte6" "_12_EverCrypt_HKDF_hkdf_extract" 

type 'a result = 'a
type 'a return = 'a
type 'a fn =
 | Returns  : 'a CI.typ   -> 'a return fn
 | Function : 'a CI.typ * 'b fn  -> ('a -> 'b) fn
let map_result f x = f x
let returning t = Returns t
let (@->) f p = Function (f, p)
let foreign : type a b. string -> (a -> b) fn -> (a -> b) =
  fun name t -> match t, name with
| Function
    (CI.View {CI.ty = CI.Primitive CI.Uint8_t; write = x2; _},
     Function
       (CI.OCaml CI.Bytes,
        Function
          (CI.OCaml CI.Bytes,
           Function
             (CI.Primitive CI.Uint32_t,
              Function
                (CI.OCaml CI.Bytes,
                 Function (CI.Primitive CI.Uint32_t, Returns CI.Void)))))),
  "EverCrypt_HKDF_hkdf_extract" ->
  (fun x1 x4 x5 x6 x7 x8 ->
    let x3 = x2 x1 in _12_EverCrypt_HKDF_hkdf_extract x3 x4 x5 x6 x7 x8)
| Function
    (CI.View {CI.ty = CI.Primitive CI.Uint8_t; write = x10; _},
     Function
       (CI.OCaml CI.Bytes,
        Function
          (CI.OCaml CI.Bytes,
           Function
             (CI.Primitive CI.Uint32_t,
              Function
                (CI.OCaml CI.Bytes,
                 Function
                   (CI.Primitive CI.Uint32_t,
                    Function (CI.Primitive CI.Uint32_t, Returns CI.Void))))))),
  "EverCrypt_HKDF_hkdf_expand" ->
  (fun x9 x12 x13 x14 x15 x16 x17 ->
    let x11 = x10 x9 in
    _11_EverCrypt_HKDF_hkdf_expand x11 x12 x13 x14 x15 x16 x17)
| Function
    (CI.View {CI.ty = CI.Primitive CI.Uint8_t; write = x19; _},
     Function
       (CI.OCaml CI.Bytes,
        Function
          (CI.OCaml CI.Bytes,
           Function
             (CI.Primitive CI.Uint32_t,
              Function
                (CI.OCaml CI.Bytes,
                 Function (CI.Primitive CI.Uint32_t, Returns CI.Void)))))),
  "EverCrypt_HKDF_extract" ->
  (fun x18 x21 x22 x23 x24 x25 ->
    let x20 = x19 x18 in _10_EverCrypt_HKDF_extract x20 x21 x22 x23 x24 x25)
| Function
    (CI.View {CI.ty = CI.Primitive CI.Uint8_t; write = x27; _},
     Function
       (CI.OCaml CI.Bytes,
        Function
          (CI.OCaml CI.Bytes,
           Function
             (CI.Primitive CI.Uint32_t,
              Function
                (CI.OCaml CI.Bytes,
                 Function
                   (CI.Primitive CI.Uint32_t,
                    Function (CI.Primitive CI.Uint32_t, Returns CI.Void))))))),
  "EverCrypt_HKDF_expand" ->
  (fun x26 x29 x30 x31 x32 x33 x34 ->
    let x28 = x27 x26 in _9_EverCrypt_HKDF_expand x28 x29 x30 x31 x32 x33 x34)
| Function
    (CI.OCaml CI.Bytes,
     Function
       (CI.OCaml CI.Bytes,
        Function
          (CI.Primitive CI.Uint32_t,
           Function
             (CI.OCaml CI.Bytes,
              Function (CI.Primitive CI.Uint32_t, Returns CI.Void))))),
  "EverCrypt_HKDF_extract_sha2_512" -> _8_EverCrypt_HKDF_extract_sha2_512
| Function
    (CI.OCaml CI.Bytes,
     Function
       (CI.OCaml CI.Bytes,
        Function
          (CI.Primitive CI.Uint32_t,
           Function
             (CI.OCaml CI.Bytes,
              Function
                (CI.Primitive CI.Uint32_t,
                 Function (CI.Primitive CI.Uint32_t, Returns CI.Void)))))),
  "EverCrypt_HKDF_expand_sha2_512" -> _7_EverCrypt_HKDF_expand_sha2_512
| Function
    (CI.OCaml CI.Bytes,
     Function
       (CI.OCaml CI.Bytes,
        Function
          (CI.Primitive CI.Uint32_t,
           Function
             (CI.OCaml CI.Bytes,
              Function (CI.Primitive CI.Uint32_t, Returns CI.Void))))),
  "EverCrypt_HKDF_extract_sha2_384" -> _6_EverCrypt_HKDF_extract_sha2_384
| Function
    (CI.OCaml CI.Bytes,
     Function
       (CI.OCaml CI.Bytes,
        Function
          (CI.Primitive CI.Uint32_t,
           Function
             (CI.OCaml CI.Bytes,
              Function
                (CI.Primitive CI.Uint32_t,
                 Function (CI.Primitive CI.Uint32_t, Returns CI.Void)))))),
  "EverCrypt_HKDF_expand_sha2_384" -> _5_EverCrypt_HKDF_expand_sha2_384
| Function
    (CI.OCaml CI.Bytes,
     Function
       (CI.OCaml CI.Bytes,
        Function
          (CI.Primitive CI.Uint32_t,
           Function
             (CI.OCaml CI.Bytes,
              Function (CI.Primitive CI.Uint32_t, Returns CI.Void))))),
  "EverCrypt_HKDF_extract_sha2_256" -> _4_EverCrypt_HKDF_extract_sha2_256
| Function
    (CI.OCaml CI.Bytes,
     Function
       (CI.OCaml CI.Bytes,
        Function
          (CI.Primitive CI.Uint32_t,
           Function
             (CI.OCaml CI.Bytes,
              Function
                (CI.Primitive CI.Uint32_t,
                 Function (CI.Primitive CI.Uint32_t, Returns CI.Void)))))),
  "EverCrypt_HKDF_expand_sha2_256" -> _3_EverCrypt_HKDF_expand_sha2_256
| Function
    (CI.OCaml CI.Bytes,
     Function
       (CI.OCaml CI.Bytes,
        Function
          (CI.Primitive CI.Uint32_t,
           Function
             (CI.OCaml CI.Bytes,
              Function (CI.Primitive CI.Uint32_t, Returns CI.Void))))),
  "EverCrypt_HKDF_extract_sha1" -> _2_EverCrypt_HKDF_extract_sha1
| Function
    (CI.OCaml CI.Bytes,
     Function
       (CI.OCaml CI.Bytes,
        Function
          (CI.Primitive CI.Uint32_t,
           Function
             (CI.OCaml CI.Bytes,
              Function
                (CI.Primitive CI.Uint32_t,
                 Function (CI.Primitive CI.Uint32_t, Returns CI.Void)))))),
  "EverCrypt_HKDF_expand_sha1" -> _1_EverCrypt_HKDF_expand_sha1
| _, s ->  Printf.ksprintf failwith "No match for %s" s


let foreign_value : type a. string -> a Ctypes.typ -> a Ctypes.ptr =
  fun name t -> match t, name with
| _, s ->  Printf.ksprintf failwith "No match for %s" s
