...
let parse_data ?(overwrite=false) ?(output=false) filename includes env =
  try
    ...
    let e = (I.loop_handle succeed (fail lexbuf) (supplier lexbuf) (Data_parser.Incremental.main lexbuf.Lexing.lex_curr_p)) ~overwrite env in
...
let parse_heterogenous_term  ?(output=false) ~color t lex = 
  ...
  try 
    let obj_term,abs_type =
      I.loop_handle (fun x -> x) (fail lexbuf)  (supplier lexbuf)  (Data_parser.Incremental.heterogenous_term_and_type lexbuf.Lexing.lex_curr_p) abs obj in
...
let parse_sig_entry ~color t sg e = 
  let lexbuf = Lexing.from_string t in
  try
    Some (I.loop (supplier lexbuf) (Data_parser.Incremental.sig_entry_eoi lexbuf.Lexing.lex_curr_p) sg e)
  with
  | Error.Error er -> 
     let () = pp_error ~color er t in
     None
...
let parse_lex_entry ~color t lex e = 
  let lexbuf = Lexing.from_string t in
  try 
    Some (I.loop (supplier lexbuf) (Data_parser.Incremental.lex_entry_eoi lexbuf.Lexing.lex_curr_p) lex e )
  with
  | Error.Error er -> 
     let () = pp_error ~color er t in
     None
