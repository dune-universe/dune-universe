...
module Log = (val Logs.src_log (Logs.Src.create "ACGtkLib.datalog" ~doc:"logs ACGtkLib datalog events") : Logs.LOG) 
...
module Make (S:UnionFind.Store) =
  struct
    ...
    let to_abstract {p_id=id;arity=arity} (start,content) (vars,vargen) pred_table =
      Log.debug (fun m -> m "Starting the extraction of predicate %s/%d" (ASPred.to_string {ASPred.p_id=id;ASPred.arity=arity;ASPred.arguments=[]} pred_table ConstGen.Table.empty) arity);
      Log.debug (fun m -> m "From the following content:");
      UF.log_content Logs.Debug content;
	...
	| Not_found -> 
	   ...
	   Log.debug (fun m -> m "Generated the variable: %s" (VarGen.id_to_string new_var));
  ...
  (** [add_pred_arguments_to_content arguments (content,idx,mapped_vars)]
    ...
    let log_facts level facts pred_table cst_table =
      Log.msg
        level
        (fun m ->
          let () = PredMap.iter
                     (fun _ v -> FactSet.iter (fun elt -> Log.msg level (fun m -> m "%s" (ASPred.to_string elt pred_table cst_table))) v)
                     facts in
          m "Done.")
  ...
  (** [make_rule r] returns an internal rule, that is one whose
	...
	List.fold_left
      ...
      Log.debug (fun m -> m "It is represented by:");
      UF.log_content Logs.Debug internal_content;
      Log.debug (fun m -> m "Done.");
      {id=id;
  ...
  (* the [dag] parameter [h] is meant to be the components of some
    ...
    let to_abstract {id=id;lhs=lhs;e_rhs=e_rhs;i_rhs=i_rhs;i_rhs_num;_} content pred_table =
      Log.debug (fun m -> m "Going to work with the following content:");
      UF.log_content Logs.Debug content;
      Log.debug (fun m -> m "Done.");
...
(*		    e_pred_to_rules: Rule.Rules.t AbstractSyntax.Predicate.PredIdMap.t; *)
    ...
    let make_program {ASProg.rules=r;ASProg.pred_table=pred_table;ASProg.const_table=cst_table;ASProg.i_preds=i_preds;ASProg.rule_id_gen;ASProg.e_pred_to_rules=_} =
      let rules,e_facts,_rule_to_rule_map = 
	ASRule.Rules.fold
	  (fun ({ASRule.lhs=lhs;_} as r) (acc,e_facts,r_to_r) ->
            (*            Log.debug (fun m -> m "Dealing with rule:\t%s" (ASRule.to_string r pred_table cst_table));  *)
    ...
    let to_abstract {rules=r;idb=idb;pred_table=pred_table;const_table=cst_table;rule_id_gen;edb_facts;_(*e_pred_to_rules*)} =
...
(*      let rules = 
	...
	Predicate.PredMap.fold
	  (fun _pred fact_set (acc,gen) -> 
		...
		Log.debug (fun m -> m "Adding fact: %s" (ASRule.to_string r pred_table cst_table));
...
(*	  AbstractSyntax.Predicate.PredIdMap.map
  ...
  (* TODO: if a set of facts for a predicate of the rhs is empty, we
    ...
    let temp_facts r e_facts previous_step_facts facts delta_facts agg_function start pred_table cst_table=
      Log.debug (fun m -> m "Scanning the rule: %s" (ASRule.to_string (Rule.to_abstract r r.Rule.content pred_table) pred_table cst_table));
	...
	Log.debug (fun m -> m "Looking for facts for predicate %s in the following facts:" (ASPred.to_string {ASPred.p_id=delta_position.Predicate.p_id;ASPred.arity=0;ASPred.arguments=[]} pred_table cst_table));
        Predicate.log_facts Logs.Debug delta_facts pred_table cst_table;
  ...
  (** [p_semantics_for_predicate s prog e_facts previous_step_facts
	...
	let new_delta_facts,new_derivations_for_all_i_pred = 
	  List.fold_left
		...
		Log.debug (fun m -> m "Trying to derive facts for: %s" (ASPred.to_string {ASPred.p_id=pred;ASPred.arity=0;ASPred.arguments=[]} prog.pred_table prog.const_table));
	    ...
	    prog.idb  in 
	Log.debug (fun m -> m "%d new facts:" (Predicate.PredMap.fold (fun _ v acc -> acc+(Predicate.FactSet.cardinal v)) new_delta_facts 0));
        Predicate.log_facts Logs.Debug  new_delta_facts prog.pred_table prog.const_table;
        Log.debug (fun m -> m "That's it.");
	...
	ASRule.Rules.fold 
      ...
      let updated_rules =
	Rule.Rules.fold
	  (fun ({Rule.lhs=lhs;_} as rule) acc -> 
	    try
	      Predicate.PredMap.add
		lhs.Predicate.p_id
...
*)
	...
	(fun (l_acc,child_num,l_visit) fact ->
	  Log.debug (fun m -> m "Analysing fact: %s" (ASPred.to_string fact prog.pred_table prog.const_table));
	  if List.mem fact.ASPred.p_id prog.edb then
	    ...
	    (Log.debug (fun m -> m "Keeping it");
	     ...
	     Log.debug (fun m -> m "It will have address [%s]" (SharedForest.address_to_string (List.rev cur_add)));
	     try
		 ...
		 Log.debug (fun m -> m "Will point to: %s with patch %s" (SharedForest.address_to_string (List.rev existing_add)) (SharedForest.path_to_string patch));
	...
	facts
    and
	build_forest_aux _fact premises derivations add visited_facts_addresses prog =	
