...
| WAIT HELP l = SEMICOLONN {
                    fun (ctx,e) ->
                    let () = echo ctx l in
                    let () = F.help ctx (F.Help (Some F.Wait)) in
                    ctx,e}
...
| DONT WAIT HELP l = SEMICOLONN {
                    fun (ctx,e) ->
                    let () = echo ctx l in
                    let () = F.help ctx (F.Help (Some F.Dont_wait)) in
                    ctx,e}
                           
| params = LOAD_DATA  {
      fun (ctx,e) ->
      let s,_loc,l = params in
      let () = echo ctx l in
...
| LIST HELP l = SEMICOLONN {
                    fun (ctx,e) ->
                    let () = echo ctx l in
                    let () = F.help ctx (F.Help (Some F.List)) in
                    ctx,e}
...
| SELECT HELP l = SEMICOLONN {
                      fun (ctx,e) ->
                      let () = echo ctx l in
                      let () = F.help ctx (F.Help (Some F.Select)) in
                      ctx,e}
...
| UNSELECT HELP l = SEMICOLONN {
                      fun (ctx,e) ->
                      let () = echo ctx l in
                      let () = F.help ctx (F.Help (Some F.Unselect)) in
                      ctx,e}
...
| TRACE HELP l = SEMICOLONN {
                     fun (ctx,e) ->
                     let () = echo ctx l in
                     let () = F.help ctx (F.Help (Some F.Trace)) in
                     ctx,e}
...
| DONT TRACE HELP l = SEMICOLONN {
                     fun (ctx,e) ->
                     let () = echo ctx l in
                     let () = F.help ctx (F.Help (Some F.Dont_trace)) in
                     ctx,e}
...
| IDENTT? PRINT HELP l = SEMICOLONN {
                            fun (ctx,e) ->
                            let () = echo ctx l in
                            let () = F.help ctx (F.Help (Some F.Print)) in
                            ctx,e}
...
| names = IDENTT+ params = ANALYSE   {
                               ...
                               let t,l,line = params in
                               let () = echo ctx line in
                               let () = F.analyse  ~names ctx e t l in
                               ctx, e}
| params = CHECK  {
               fun (ctx,e) ->
               let t,l,line = params in
               let () = echo ctx line in 
               let () = F.check ctx e t l in
               ctx, e}
...
| names = IDENTT+ params = CHECK  {
                               ...
                               let t,l,line = params in
                               let () = echo ctx line in 
                               let () = F.check  ~names ctx e t l in
                               ctx, e}
| params = REALIZE   {
               fun (ctx,e) ->
               let t,l,line = params in
               let () = echo ctx line in  
               let () = F.realize ?svg_output:(F.svg ctx) ctx e t l in
               ctx, e}
...
| names = IDENTT+ params = REALIZE   {
                               ...
                               let t,l,line = params in
                               let () = echo ctx line in  
                               let () = F.realize  ~names ?svg_output:(F.svg ctx) ctx e t l in
                               ctx, e}
| params = REALIZE_SHOW   { 
               fun (ctx,e) ->
               let t,l,line = params in
               let () = echo ctx line in  
               let () = F.realize_show ctx e t l in
               ctx,e}
...
| names = IDENTT+ params = REALIZE_SHOW   { 
                               ...
                               let t,l,line = params in
                               let () = echo ctx line in  
                               let () = F.realize_show  ~names ctx e t l in
                               ctx,e}
| params = PARSE   {
               fun (ctx,e) ->
               let t,l,line = params in
               let () = echo ctx line in
               let () =  F.parse  ctx e t l in
               ctx,e}
| name = IDENTT params = PARSE   {
                             fun (ctx,e) ->
                             let t,_l,line = params in
                             let () = echo ctx line in
                             let n,lex_loc = name in
                             let () = F.parse ~name:n ctx e t lex_loc in
                             ctx,e}
| params = QUERY  {
               fun (ctx,e) ->
               let t,l,line = params in
               let () = echo ctx line in
               let () = F.query ctx e t l in
               ctx,e}
| name = IDENTT params = QUERY  {
                             ...
                             let () = echo ctx line in
                             let n,lex_loc = name in
                             let () = F.query ~name:n ctx e t lex_loc in
                             ctx,e}
...
| IDENTT? IDB HELP l = SEMICOLONN {
                            fun (ctx,e) ->
                            let () = echo ctx l in
                            let () = F.help ctx (F.Help (Some F.Idb)) in
                            ctx,e}
...
| COMPOSE HELP l = SEMICOLONN {
                       fun (ctx,e) ->
                       let () = echo ctx l in
                       let () = F.help ctx (F.Help (Some F.Compose)) in
                       ctx,e}
                              
| HELP l = SEMICOLONN {
               fun (ctx,e) ->
               let () = echo ctx l in
               let () = F.help ctx (F.Help None) in
               ctx,e}
                      
| HELP HELP l = SEMICOLONN {
                    fun (ctx,e) ->
                    let () = echo ctx l in
                    let () = F.help ctx (F.Help (Some (F.Help None))) in
                    ctx,e}
                                         
| LOAD_HELP l = SEMICOLONN {
                    fun (ctx,e) ->
                    let () = echo ctx l in
                    let () = F.help ctx (F.Help (Some F.Load)) in
                    ctx,e}
...
| ANALYSE_HELP l = SEMICOLONN {
                       fun (ctx,e) ->
                       let () = echo ctx l in
                       let () = F.help ctx (F.Help (Some F.Analyse)) in
                       ctx,e}
                              
| REALIZE_HELP l = SEMICOLONN {
                       fun (ctx,e) ->
                       let () = echo ctx l in
                       let () = F.help ctx (F.Help (Some F.Realize)) in ctx,e}
                              
| REALIZE_SHOW_HELP l = SEMICOLONN {
                            fun (ctx,e) ->
                            let () = echo ctx l in
                            let () = F.help ctx (F.Help (Some F.RealizeShow)) in ctx,e}
                                   
| CHECK_HELP l = SEMICOLONN {
                     fun (ctx,e) ->
                     let () = echo ctx l in
                     let () = F.help ctx (F.Help (Some F.Check)) in ctx,e}
                            
| PARSE_HELP l = SEMICOLONN {
                     fun (ctx,e) ->
                     let () = echo ctx l in
                     let () = F.help ctx (F.Help (Some F.Parse)) in ctx,e}
                            
| QUERY_HELP l = SEMICOLONN {
                     fun (ctx,e) ->
                     let () = echo ctx l in
                     let () = F.help ctx (F.Help (Some F.Query)) in ctx,e}
                            
| SAVE_HELP l = SEMICOLONN {
                    fun (ctx,e) ->
                    let () = echo ctx l in
                    let () = F.help ctx (F.Help (Some F.Save)) in ctx,e}
                           
| ADD_HELP l = SEMICOLONN {
                   fun (ctx,e) ->
                   let () = echo ctx l in
                   let () = F.help ctx (F.Help (Some F.Add)) in ctx,e}
                          
| CREATE_HELP l = SEMICOLONN {
                      fun (ctx,e) ->
                      let () = echo ctx l in
                      let () = F.help ctx (F.Help (Some F.Create)) in ctx,e}
