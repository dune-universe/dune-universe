...
(* This module was copied from OCaml 4.05 and adapted for the 4.03 AST. The function
   varify_constructors was copied from the 4.05 version of Ast_helper as it was needed
   here. *)
...
let tyvar_loc f str = pp f "'%s" str
...
and binding ctxt f {pvb_pat=p; pvb_expr=x; _} =
  ...
  in
  let tyvars_str tyvars = List.map (fun v -> v) tyvars in
  ...
  then pp f "%a@;=@;%a" (pattern ctxt) p (expression ctxt) x else
    ...
    | None -> begin
        ...
        | (Pexp_constraint (x, ty), _) -> (* XXX: this case is janestreet only *)
