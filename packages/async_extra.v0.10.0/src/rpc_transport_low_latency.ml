...
module Reader_internal = struct
  ...
  let shift_unconsumed t =
    if t.pos > 0 then begin
      ...
      if len > 0 then
        Bigstring.blit ~src:t.buf ~dst:t.buf ~src_pos:t.pos ~dst_pos:0 ~len;
      t.pos <- 0;
      t.max <- len;
    end;
  ;;
  ...
  let refill t =
    shift_unconsumed t;
    let result =
      Bigstring.read_assume_fd_is_nonblocking (Fd.file_descr_exn t.fd)
        t.buf ~pos:t.max ~len:(Bigstring.length t.buf - t.max)
    in
  ...
  module Message_len : sig
    ...
    val none : t
    val is_some : t -> bool
    val create_exn : int -> t (* fails on negative ints *)
    val value_exn : t -> int
  end = struct
    type t = int
    ...
    let create_exn n =
      if n < 0
      then failwithf "Message_len.create_exn of negative int: %d" n ()
      else n

    let value_exn t =
      if t < 0
      then failwith "Message_len.value_exn of None"
      else t
  end
  ...
  module Dispatcher = struct
    ...
    let rec process_received_messages t =
      if can_process_message t then begin
        let len = get_payload_length_of_next_available_message t.reader in
        if Message_len.is_some len then begin
    ...
    let rec run reader ~on_message ~on_end_of_batch =
      ...
      Scheduler.within' ~monitor (fun () ->
        ...
        in
        Fd.interruptible_every_ready_to ~interrupt
          t.reader.fd `Read process_incoming t)
      >>= function
      | `Bad_fd | `Unsupported ->
        failwith
          "Rpc_transport_low_latency.Reader.read_forever: \
      ...
      | `Closed | `Interrupted ->
        ...
        | Stopped (Waiting_for_handler d) ->
          ...
          if reader.closed then
            ...
            run reader ~on_message ~on_end_of_batch
    ;;
  end

  let read_forever t ~on_message ~on_end_of_batch =
    if t.closed then
      failwith "Rpc_transport_low_latency.Reader: reader closed";
    if t.reading then
...
module Writer_internal = struct
  ...
  let single_write t : Single_write_result.t =
    match
      Bigstring.write_assume_fd_is_nonblocking (Fd.file_descr_exn t.fd) t.buf
        ~pos:0 ~len:t.pos
    with
    | n -> discard t n; Continue
  ...
  let flush t =
    if not t.writing && t.pos > 0 then begin
      t.writing <- true;
  ...
  let unsafe_send_bytes t ~buf ~pos ~len =
    let result =
      writev2 (Fd.file_descr_exn t.fd)
        ~buf1:t.buf
        ~pos1:0
        ~len1:t.pos
        ~buf2:buf
    ...
    if Unix.Syscall_result.Int.is_ok result then begin
      ...
      if n <= t.pos then begin
        ...
        discard t t.pos;
        if remaining_in_other_buf > 0 then
          copy_bytes t ~buf
  ...
  let should_send_now t =
    ...
    if current_job = t.last_send_job then
