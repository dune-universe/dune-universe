...
include Util.Logging (struct
  let label = "dose_applications.ceve"
end)
...
module Options = struct
  ...
  let otypes =
    ["cnf"; "dimacs"; "cudf"; "deb"; "debsrc"; "dot"; "gml"; "grml"; "table"]

  let out_option ?default
      ?(metavar = Printf.sprintf "<%s>" (String.concat "|" otypes)) () =
  ...
  let grp_option ?default
      ?(metavar = Printf.sprintf "<%s>" (String.concat "|" gtypes)) () =
    ...
    (List.map
       (fun ((name, aop), sel) ->
         ...
         | Some (op, v) ->
             [ ( encname,
                 Some
                   (Dose_pef.Pefcudf.pefcudf_op op, snd (to_cudf (encname, v)))
               ) ])
       l)

let parse_request to_cudf l =
  let pkgs_of s =
    (* convert request into list of packages *)
    let rs = String.strip (snd (String.split s " ")) in
    let field = ("ceve pkg req", (Format822.dummy_loc, rs)) in
    let v =
      Dose_pef.Packages.lexbuf_wrapper
        Dose_pef.Packages_parser.vpkglist_top
        field
    in
    loadl to_cudf v
  in
  ...
  Cudf.fold_packages
    (fun acc p ->
      let cfl =
        List.filter
          (fun x -> not (x =% p))
          (List.flatten (List.rev_map (CudfAdd.who_provides univ) p.conflicts))
      in
      debug
        "%s: %d conflicts"
        (Cudf_types_pp.string_of_pkgname p.package)
        (List.length cfl) ;
      List.iter
        (fun c -> debug "- %s" (Cudf_types_pp.string_of_pkgname c.package))
        cfl ;
      acc + List.length cfl)
    0
    univ
...
let main () =
  ...
  let (input_type, implicit) =
    ...
    else (Input.guess_format [posargs], false)
  in
    ...
    List.unique
      (List.fold_left
         (fun acc (p, c) ->
           ...
           else Depsolver.reverse_dependency_closure universe l @ acc)
         []
         (OptParse.Opt.get Options.reverse_cone))
  in
  let plist =
    if OptParse.Opt.is_set Options.cone then pkg_cone ()
    else if OptParse.Opt.is_set Options.reverse_cone then pkg_reverse_cone ()
    ...
    (fun l ->
      ...
      | "dimacs" ->
          Printf.fprintf
            oc
            "%s"
            (Depsolver.output_clauses ~enc:Depsolver.Dimacs u)
      ...
      | ("deb" | "debsrc") as t ->
          ...
          | Some dll ->
              List.iter2
                ...
                dll
          | None ->
              fatal "If -T is deb or debsrc, then the raw list must not be None") ;
          let cudf2deb cudfpkg =
            let id = (cudfpkg.Cudf.package, cudfpkg.Cudf.version) in
            try Hashtbl.find cudftodeb_table id
            with Not_found ->
              fatal
                "cannot find cudf package the mapping - is the input deb:// or \
                 debsrc://?"
          in
          if t = "deb" then
            List.iter
      ...
      | ("dot" | "gml" | "grml") as t -> (
          ...
          match OptParse.Opt.get Options.grp_type with
          | "syn" -> (
              let g =
                Defaultgraphs.SyntacticDependencyGraph.dependency_graph u
              in
              match t with
              | "dot" ->
                  Defaultgraphs.SyntacticDependencyGraph.DotPrinter.print fmt g
              | "gml" ->
                  Defaultgraphs.SyntacticDependencyGraph.GmlPrinter.print fmt g
              | "grml" ->
                  Defaultgraphs.SyntacticDependencyGraph.GraphmlPrinter.print
                    fmt
                    g
              | _ -> assert false)
          | ("pkg" | "strdeps" | "conj" | "dom") as gt -> (
              let g =
                ...
                | "strdeps" ->
                     ...
                     * packages *)
                    if
                      (not (OptParse.Opt.get Options.deb_ignore_essential))
                      && (input_type = `Deb || input_type = `DebSrc)
                    then (
                      ...
                      Defaultgraphs.PackageGraph.G.iter_vertex
                        (fun v ->
                          List.iter
                            (Defaultgraphs.PackageGraph.add_edge
                               ~transitive:true
                               g
                               v)
                            essential)
                ...
                | "dom" ->
                    let g =
                      Strongdeps.strongdeps_univ ~transitive:true universe
                    in
