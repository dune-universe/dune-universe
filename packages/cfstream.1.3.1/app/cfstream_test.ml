...
let test_exists () =
  ...
  assert_bool "Find an even number" (exists (of_list [ 5; 9; 11; 8 ]) ~f);
  assert_bool "No even number" (not (exists (of_list [ -45; 11 ]) ~f))
;;

let test_group () =
  let f x = x mod 2 in
  let groups =
    [ 1; 3; 4; 5; 4; 2; 9 ] |> of_list |> group ~f |> map ~f:to_list |> to_list
  in
  assert_equal
    ~msg:"Find groups with same parity"
    groups
    [ [ 1; 3 ]; [ 4 ]; [ 5 ]; [ 4; 2 ]; [ 9 ] ];
...
let test_concat () =
  ...
  let m = of_list l |> group ~f |> concat |> to_list in
  assert_equal ~msg:"Concat grouped enum is idempotent" l m
;;
...
let test_uncombine () =
  ...
  assert_equal
    ...
    ~msg:"Check first list"
    (List.rev l1)
    [ 1; 3; 5; 7 ];
  assert_equal
    ~printer:int_list_printer
    ~msg:"Check second list"
    (List.rev l2)
    [ 2; 4; 6; 8 ]
;;
...
let test_iter () =
  ...
  assert_equal ~printer:int_list_printer ~msg:"Check list built with iter" l (List.rev !c)
;;
...
let test_take () =
  assert_equal
    ...
    ~msg:"Check take"
    [ 1; 2; 3 ]
    (to_list (take (of_list [ 1; 2; 3; 4; 5 ]) ~n:3));
...
let test_range () =
  ...
  assert_equal
    ...
    10
    (range 1 |> take ~n:10 |> to_list |> List.length)
;;

let test_scan () =
  let factorials = scan (1 -- 10) ~f:( * ) |> to_list in
  assert_equal
    ~printer:int_list_printer
...
let () = ignore (OUnit.run_test_tt_main ("All" >::: [ tests ]))
