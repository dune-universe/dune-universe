...
include Meta_conv.Coder.Make(struct
  ...
  module Constr = struct
    ...
    let variant _tyname tag ts = match ts with
      | [] -> Variant (tag, None)
      | [x] -> Variant (tag, Some x)
      | _ -> Variant (tag, Some (Tuple ts))
    let poly_variant _tyname tag ts = match ts with
      | [] -> Poly_variant (tag, None)
      | [t] -> Poly_variant (tag, Some t)
      | _ -> Poly_variant (tag, Some (Tuple ts))
  ...
  module Deconstr = struct
    ...
    let variant _tyname = function 
      | Variant (tag, None) -> tag, []
      | Variant (tag, Some (Tuple ts)) -> tag, ts
      | Variant (tag, Some t) -> tag, [t]
      | Poly_variant (tag, None) -> tag, []
      | Poly_variant (tag, Some (Tuple ts)) -> tag, ts
      | Poly_variant (tag, Some t) -> tag, [t]
...
module Default = struct
  ...
  let list_of_ocaml f = 
    ...
    let rec get_list revs = function
      ...
      | Variant ("[]", None) -> Some (List.rev revs)
      | Variant ("::", Some (Tuple [x; xs])) -> get_list (x::revs) xs
      | _ -> None
    in
  ...
  let option_of_ocaml f = Helper.option_of (function
    | Variant ("None", None) -> Some None 
    | Variant ("Some", Some (Tuple vs)) -> Some (Some (Tuple vs))
    | Variant ("Some", Some v) -> Some (Some v)
    | _ -> None) f
  ...
  let ocaml_of_result a b = 
    let emb x = Variant ("Ok", Some x) in
    let emb' x = Variant ("Error", Some x) in
    Helper.of_result emb emb' a b

  let result_of_ocaml a b = 
    let ext ?(trace=[]) = function
      | Variant ("Ok", Some x) -> Ok (Ok x)
      | Variant ("Error", Some x) -> Ok (Error x)
      | v -> Error (`Primitive_decoding_failure "result_of_ocaml: result expected", v, trace)
    in
    Helper.result_of ext a b 
end
