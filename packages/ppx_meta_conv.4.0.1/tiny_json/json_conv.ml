...
module Default = struct
  ...
  let json_of_hashtbl x = Helper.of_hashtbl json_of_list x
  let hashtbl_of_json x = Helper.hashtbl_of list_of_json x
  let json_of_result a b =
    let emb x = Object ["Ok", x] in
    let emb' x = Object ["Error", x] in
    Helper.of_result emb emb' a b
  let result_of_json a b = 
    let ext ?(trace=[]) = function
      | Object ["Ok", x] -> Ok (Ok x)
      | Object ["Error", x] -> Ok (Error x)
      | v -> Error (`Primitive_decoding_failure "result_of_json: result expected", v, trace)
    in
