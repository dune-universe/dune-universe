...
module Default = struct
  ...
  let sexp_of_result a b =
    let emb x = List [Atom "Ok"; x] in
    let emb' x = List [Atom "Error"; x] in
    Helper.of_result emb emb' a b
  let result_of_sexp a b = 
    let ext ?(trace=[]) = function
      | List [Atom "Ok"; x] -> Ok (Ok x)
      | List [Atom "Error"; x] -> Ok (Error x)
      | v -> Error (`Primitive_decoding_failure "result_of_sexp: result expected", v, trace)
    in
