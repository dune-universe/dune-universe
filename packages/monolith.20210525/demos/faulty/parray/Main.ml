...
(* Define [element_list], etc. as concrete types. *)

let element_list =
  deconstructible Print.(list int)

let index_list =
  deconstructible Print.(list int)

let index_element_list =
  deconstructible Print.(list (pair int int))
...
let () =
  ...
  let spec = wrap_init (lt 16 ^> index_list *** array) in
  ...
  let spec = array ^> element_list in
  declare "to_list" spec R.to_list C.to_list;

  let spec = wrap_iter (array ^> element_list) in
  declare "iter" spec R.iter C.iter;

  let spec = wrap_iteri (array ^> index_element_list) in
  declare "iteri" spec R.iteri C.iteri;

  let spec = wrap_fold_left (array ^> element_list) in
  declare "fold_left" spec R.fold_left C.fold_left;

  let spec = wrap_fold_right (array ^> element_list) in
