...
CAMLprim value ordma_rselect_select(value readfds,
                                    value writefds,
                                    value exceptfds,
                                    value timeout)
{
  ORDMA_LOG("ordma_rselect_select: begin");
  ...
  Begin_roots3 (readfds, writefds, exceptfds);
    ...
    if (retcode != 0) {
      unix_error(EINVAL, "select", Nothing);
    }
    ...
    retcode = rselect(maxfd + 1, &read, &write, &except, tvp);//librdmacm
  ...
  End_roots();
  ORDMA_LOG("ordma_rselect_select: end");
  return res;
}
