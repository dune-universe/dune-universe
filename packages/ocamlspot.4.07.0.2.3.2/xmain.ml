...
let main argv =
  try
    (* Register the appropriate argv for ocamlspot --recheck *)
    Spot.File.set_argv argv; 
    Arg.parse_argv ~current:(ref 0) argv Options.list anonymous usage;
    Clflags.annotations := true;
(*
    Clflags.annotations_only := true;
*)    Clflags.make_archive := false;
    Clflags.output_c_object := false;
    if not !Clflags.make_package then Clflags.compile_only := true;
