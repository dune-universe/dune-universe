...
let is_list_antiquotation l =
  match l with
  | loc::qtyp::l' when l == l' ->
      let loc = (Obj.magic loc : Location.t)  in
      if is_antiquotation loc
      then Some (loc, (Obj.magic qtyp : quotation))
      else None
  | _ -> None 
let quote_bool =
  (fun _  -> Pa_ast.exp_bool : expression -> Location.t -> bool -> expression)
  
let quote_int =
  (fun _  -> Pa_ast.exp_int : expression -> Location.t -> int -> expression) 
let quote_int32 =
  (fun _  -> Pa_ast.exp_int32 : expression ->
                                  Location.t -> int32 -> expression)
  
let quote_int64 =
  (fun _  -> Pa_ast.exp_int64 : expression ->
                                  Location.t -> int64 -> expression)
  
let quote_nativeint =
  (fun _  -> Pa_ast.exp_nativeint : expression ->
                                      Location.t -> nativeint -> expression)
  
let quote_char =
  (fun _  -> Pa_ast.exp_char : expression -> Location.t -> char -> expression)
  ...
  (fun _  ->
     fun loc  ->
       fun s  ->
         ...
         with | Not_found  -> Pa_ast.exp_string loc s : expression ->
                                                          Location.t ->
                                                            string ->
                                                              expression)
  ...
  fun qe  ->
    fun e_loc  ->
      fun _loc  ->
        fun eo  ->
          let e =
            match eo with | None  -> None | Some e -> Some (qe e_loc _loc e)
             in
...
let quote_tuple =
  (fun _  -> Pa_ast.exp_tuple : expression ->
                                  Location.t -> expression list -> expression)
     ...
     fun _loc  ->
       fun l  ->
         ...
         | Lapply (l,l') ->
             let l = quote_longident e_loc _loc l  in
             let l' = quote_longident e_loc _loc l'  in
             quote_const e_loc _loc (longident "Lapply") [l; l'] : expression
                                                                    ->
                                                                    Location.t
                                                                    ->
                                                                    Longident.t
...
let quote_record =
  (fun _  -> Pa_ast.exp_record : expression ->
                                   Location.t ->
                                     (Longident.t * expression) list ->
                                       expression)
     ...
     fun _loc  ->
       fun
         ...
         quote_record e_loc _loc
           ...
           ((lexing "pos_lnum"), ln);
           ((lexing "pos_bol"), bl);
           ((lexing "pos_cnum"), pcn)] : expression ->
                                           Location.t ->
                                             Lexing.position -> expression)
  
let quote_parser_position = ref false 
let quote_location_t =
  (fun e_loc  ->
     fun _loc  ->
       fun
         ...
         then
           ...
           quote_record e_loc _loc
             ...
             ((location "loc_end"), le);
             ((location "loc_ghost"), g)]
         else e_loc : expression -> Location.t -> Location.t -> expression)
...
and quote_row_field e_loc _loc x = match x with
  | Rtag(x1,x2,x3,x4) -> quote_const e_loc _loc (Ldot(Lident "Parsetree", "Rtag")) [ quote_label e_loc _loc x1; quote_attributes e_loc _loc x2; quote_bool e_loc _loc x3; (quote_list quote_core_type) e_loc _loc x4;]
...
and quote_with_constraint e_loc _loc x = match x with
  ...
  | Pwith_modsubst(x1,x2) -> quote_const e_loc _loc (Ldot(Lident "Parsetree", "Pwith_modsubst")) [ (quote_loc quote_string) e_loc _loc x1; (quote_loc quote_longident) e_loc _loc x2;]
