...
let rec eq_longident i1 i2 =
  ...
  | (Lapply (e1, f1), Lapply (e2, f2)) ->
      (eq_longident e1 e2) && (eq_longident f1 f2)
  | _ -> false
(* asttypes.mli *)
...
let eq_arg_label c1 c2 =
  match c1, c2 with
  | Nolabel, Nolabel -> true
  | Labelled(x), Labelled(y) -> (=) x y
...
let eq_constant c1 c2 =
  match c1, c2 with
  | Pconst_integer(x1,x2), Pconst_integer(y1,y2) -> true && ((=) x1 y1) && ((eq_option (=)) x2 y2)  | Pconst_char(x), Pconst_char(y) -> (=) x y
  | Pconst_string(x1,x2), Pconst_string(y1,y2) -> true && ((=) x1 y1) && ((eq_option (=)) x2 y2)  | Pconst_float(x1,x2), Pconst_float(y1,y2) -> true && ((=) x1 y1) && ((eq_option (=)) x2 y2)  | _, _ -> false
...
and eq_payload c1 c2 =
  ...
  | PSig(x), PSig(y) -> eq_signature x y
...
and eq_core_type_desc c1 c2 =
  ...
  | Ptyp_arrow(x1,x2,x3), Ptyp_arrow(y1,y2,y3) -> true && (eq_arg_label x1 y1) && (eq_core_type x2 y2) && (eq_core_type x3 y3)  | Ptyp_tuple(x), Ptyp_tuple(y) -> (eq_list eq_core_type) x y
  | Ptyp_constr(x1,x2), Ptyp_constr(y1,y2) -> true && ((eq_loc eq_longident) x1 y1) && ((eq_list eq_core_type) x2 y2)  | Ptyp_object(x1,x2), Ptyp_object(y1,y2) -> true && ((eq_list eq_object_field) x1 y1) && (eq_closed_flag x2 y2)  | Ptyp_class(x1,x2), Ptyp_class(y1,y2) -> true && ((eq_loc eq_longident) x1 y1) && ((eq_list eq_core_type) x2 y2)  | Ptyp_alias(x1,x2), Ptyp_alias(y1,y2) -> true && (eq_core_type x1 y1) && ((=) x2 y2)  | Ptyp_variant(x1,x2,x3), Ptyp_variant(y1,y2,y3) -> true && ((eq_list eq_row_field) x1 y1) && (eq_closed_flag x2 y2) && ((eq_option (eq_list eq_label)) x3 y3)  | Ptyp_poly(x1,x2), Ptyp_poly(y1,y2) -> true && ((eq_list (eq_loc (=))) x1 y1) && (eq_core_type x2 y2)  | Ptyp_package(x), Ptyp_package(y) -> eq_package_type x y
...
and eq_row_field c1 c2 =
  match c1, c2 with
  | Rtag(x1,x2,x3,x4), Rtag(y1,y2,y3,y4) -> true && ((eq_loc eq_label) x1 y1) && (eq_attributes x2 y2) && ((=) x3 y3) && ((eq_list eq_core_type) x4 y4)  | Rinherit(x), Rinherit(y) -> eq_core_type x y
  | _, _ -> false
and eq_object_field c1 c2 =
  match c1, c2 with
  | Otag(x1,x2,x3), Otag(y1,y2,y3) -> true && ((eq_loc eq_label) x1 y1) && (eq_attributes x2 y2) && (eq_core_type x3 y3)  | Oinherit(x), Oinherit(y) -> eq_core_type x y
  | _, _ -> false
...
and eq_pattern_desc c1 c2 =
  ...
  | Ppat_open(x1,x2), Ppat_open(y1,y2) -> true && ((eq_loc eq_longident) x1 y1) && (eq_pattern x2 y2)  | _, _ -> false
...
and eq_expression_desc c1 c2 =
  ...
  | Pexp_fun(x1,x2,x3,x4), Pexp_fun(y1,y2,y3,y4) -> true && (eq_arg_label x1 y1) && ((eq_option eq_expression) x2 y2) && (eq_pattern x3 y3) && (eq_expression x4 y4)  | Pexp_apply(x1,x2), Pexp_apply(y1,y2) -> true && (eq_expression x1 y1) && ((eq_list (fun (x1,x2) (y1,y2) -> true && (eq_arg_label x1 y1) && (eq_expression x2 y2))) x2 y2)  | Pexp_match(x1,x2), Pexp_match(y1,y2) -> true && (eq_expression x1 y1) && ((eq_list eq_case) x2 y2)  | Pexp_try(x1,x2), Pexp_try(y1,y2) -> true && (eq_expression x1 y1) && ((eq_list eq_case) x2 y2)  | Pexp_tuple(x), Pexp_tuple(y) -> (eq_list eq_expression) x y
  ...
  | Pexp_ifthenelse(x1,x2,x3), Pexp_ifthenelse(y1,y2,y3) -> true && (eq_expression x1 y1) && (eq_expression x2 y2) && ((eq_option eq_expression) x3 y3)  | Pexp_sequence(x1,x2), Pexp_sequence(y1,y2) -> true && (eq_expression x1 y1) && (eq_expression x2 y2)  | Pexp_while(x1,x2), Pexp_while(y1,y2) -> true && (eq_expression x1 y1) && (eq_expression x2 y2)  | Pexp_for(x1,x2,x3,x4,x5), Pexp_for(y1,y2,y3,y4,y5) -> true && (eq_pattern x1 y1) && (eq_expression x2 y2) && (eq_expression x3 y3) && (eq_direction_flag x4 y4) && (eq_expression x5 y5)  | Pexp_constraint(x1,x2), Pexp_constraint(y1,y2) -> true && (eq_expression x1 y1) && (eq_core_type x2 y2)  | Pexp_coerce(x1,x2,x3), Pexp_coerce(y1,y2,y3) -> true && (eq_expression x1 y1) && ((eq_option eq_core_type) x2 y2) && (eq_core_type x3 y3)  | Pexp_send(x1,x2), Pexp_send(y1,y2) -> true && (eq_expression x1 y1) && ((eq_loc eq_label) x2 y2)  | Pexp_new(x), Pexp_new(y) -> (eq_loc eq_longident) x y
  | Pexp_setinstvar(x1,x2), Pexp_setinstvar(y1,y2) -> true && ((eq_loc eq_label) x1 y1) && (eq_expression x2 y2)  | Pexp_override(x), Pexp_override(y) -> (eq_list (fun (x1,x2) (y1,y2) -> true && ((eq_loc eq_label) x1 y1) && (eq_expression x2 y2))) x y
  | Pexp_letmodule(x1,x2,x3), Pexp_letmodule(y1,y2,y3) -> true && ((eq_loc (=)) x1 y1) && (eq_module_expr x2 y2) && (eq_expression x3 y3)  | Pexp_letexception(x1,x2), Pexp_letexception(y1,y2) -> true && (eq_extension_constructor x1 y1) && (eq_expression x2 y2)  | Pexp_assert(x), Pexp_assert(y) -> eq_expression x y
  ...
  | Pexp_newtype(x1,x2), Pexp_newtype(y1,y2) -> true && ((eq_loc (=)) x1 y1) && (eq_expression x2 y2)  | Pexp_pack(x), Pexp_pack(y) -> eq_module_expr x y
  ...
  | Pexp_unreachable, Pexp_unreachable -> true
  | _, _ -> false
...
and eq_constructor_declaration = fun r1 r2 -> true && (eq_loc (=)) r1.pcd_name r2.pcd_name && eq_constructor_arguments r1.pcd_args r2.pcd_args && (eq_option eq_core_type) r1.pcd_res r2.pcd_res && (fun _ _ -> true) r1.pcd_loc r2.pcd_loc && eq_attributes r1.pcd_attributes r2.pcd_attributes
and eq_constructor_arguments c1 c2 =
  match c1, c2 with
  | Pcstr_tuple(x), Pcstr_tuple(y) -> (eq_list eq_core_type) x y
  | Pcstr_record(x), Pcstr_record(y) -> (eq_list eq_label_declaration) x y
  | _, _ -> false
...
and eq_extension_constructor_kind c1 c2 =
  match c1, c2 with
  | Pext_decl(x1,x2), Pext_decl(y1,y2) -> true && (eq_constructor_arguments x1 y1) && ((eq_option eq_core_type) x2 y2)  | Pext_rebind(x), Pext_rebind(y) -> (eq_loc eq_longident) x y
  | _, _ -> false
...
and eq_class_type_desc c1 c2 =
  ...
  | Pcty_arrow(x1,x2,x3), Pcty_arrow(y1,y2,y3) -> true && (eq_arg_label x1 y1) && (eq_core_type x2 y2) && (eq_class_type x3 y3)  | Pcty_extension(x), Pcty_extension(y) -> eq_extension x y
  | Pcty_open(x1,x2,x3), Pcty_open(y1,y2,y3) -> true && (eq_override_flag x1 y1) && ((eq_loc eq_longident) x2 y2) && (eq_class_type x3 y3)  | _, _ -> false
...
and eq_class_expr_desc c1 c2 =
  ...
  | Pcl_fun(x1,x2,x3,x4), Pcl_fun(y1,y2,y3,y4) -> true && (eq_arg_label x1 y1) && ((eq_option eq_expression) x2 y2) && (eq_pattern x3 y3) && (eq_class_expr x4 y4)  | Pcl_apply(x1,x2), Pcl_apply(y1,y2) -> true && (eq_class_expr x1 y1) && ((eq_list (fun (x1,x2) (y1,y2) -> true && (eq_arg_label x1 y1) && (eq_expression x2 y2))) x2 y2)  | Pcl_let(x1,x2,x3), Pcl_let(y1,y2,y3) -> true && (eq_rec_flag x1 y1) && ((eq_list eq_value_binding) x2 y2) && (eq_class_expr x3 y3)  | Pcl_constraint(x1,x2), Pcl_constraint(y1,y2) -> true && (eq_class_expr x1 y1) && (eq_class_type x2 y2)  | Pcl_extension(x), Pcl_extension(y) -> eq_extension x y
  | Pcl_open(x1,x2,x3), Pcl_open(y1,y2,y3) -> true && (eq_override_flag x1 y1) && ((eq_loc eq_longident) x2 y2) && (eq_class_expr x3 y3)  | _, _ -> false
...
and eq_class_field_desc c1 c2 =
  match c1, c2 with
  | Pcf_inherit(x1,x2,x3), Pcf_inherit(y1,y2,y3) -> true && (eq_override_flag x1 y1) && (eq_class_expr x2 y2) && ((eq_option (eq_loc (=))) x3 y3)  | Pcf_val(x1,x2,x3), Pcf_val(y1,y2,y3) -> true && ((eq_loc eq_label) x1 y1) && (eq_mutable_flag x2 y2) && (eq_class_field_kind x3 y3)  | Pcf_method(x1,x2,x3), Pcf_method(y1,y2,y3) -> true && ((eq_loc eq_label) x1 y1) && (eq_private_flag x2 y2) && (eq_class_field_kind x3 y3)  | Pcf_constraint(x1,x2), Pcf_constraint(y1,y2) -> true && (eq_core_type x1 y1) && (eq_core_type x2 y2)  | Pcf_initializer(x), Pcf_initializer(y) -> eq_expression x y
...
and eq_signature_item_desc c1 c2 =
  ...
  | Psig_type(x1,x2), Psig_type(y1,y2) -> true && (eq_rec_flag x1 y1) && ((eq_list eq_type_declaration) x2 y2)  | Psig_typext(x), Psig_typext(y) -> eq_type_extension x y
...
and eq_with_constraint c1 c2 =
  match c1, c2 with
  | Pwith_type(x1,x2), Pwith_type(y1,y2) -> true && ((eq_loc eq_longident) x1 y1) && (eq_type_declaration x2 y2)  | Pwith_module(x1,x2), Pwith_module(y1,y2) -> true && ((eq_loc eq_longident) x1 y1) && ((eq_loc eq_longident) x2 y2)  | Pwith_typesubst(x1,x2), Pwith_typesubst(y1,y2) -> true && ((eq_loc eq_longident) x1 y1) && (eq_type_declaration x2 y2)  | Pwith_modsubst(x1,x2), Pwith_modsubst(y1,y2) -> true && ((eq_loc eq_longident) x1 y1) && ((eq_loc eq_longident) x2 y2)  | _, _ -> false
...
and eq_structure_item_desc c1 c2 =
  ...
  | Pstr_type(x1,x2), Pstr_type(y1,y2) -> true && (eq_rec_flag x1 y1) && ((eq_list eq_type_declaration) x2 y2)  | Pstr_typext(x), Pstr_typext(y) -> eq_type_extension x y
...
and eq_directive_argument c1 c2 =
  ...
  | Pdir_int(x1,x2), Pdir_int(y1,y2) -> true && ((=) x1 y1) && ((eq_option (=)) x2 y2)  | Pdir_ident(x), Pdir_ident(y) -> eq_longident x y
