...
module Jbuilder = struct
  ...
  let executable ?(ppx = []) ?(modules = []) ?(single_module = false)
      ~libraries name =
  ...
  let with_stdout_to path l =
    sprintf "(with-stdout-to %S (progn %s))" path
      (String.concat ~sep:"\n  " l)
...
module Main = struct
  let make ?(command = "ocaml " ^ Sys.argv.(0)) ~files ?(argv = Sys.argv)
      ?version ?describe ?(more_commands = []) () =
    let usage () =
      eprintf
        "usage: %s {clean,configure,version <pkg?>,describe <pkg?>%s}\n%!"
        command
    ...
    | "version" -> (
      match version with
      | None -> failwith "version function not defined"
      | Some f -> ( try f (Some Sys.argv.(2)) with _ -> f None ) )
    | other -> (
