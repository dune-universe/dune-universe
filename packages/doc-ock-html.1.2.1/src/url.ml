...
let rec from_identifier : type a. get_package:('x -> string) -> stop_before:bool ->
  ...
  fun ~get_package ~stop_before ->
    ...
    | Module (parent, mod_name) ->
      from_identifier_no_anchor ~get_package parent ("module " ^ mod_name)
      >>| fun parent ->
      let kind = "module" in
      if stop_before then
    ...
    | Argument (functor_id, arg_num, arg_name) ->
      from_identifier_no_anchor ~get_package functor_id ("arg " ^ arg_name)
      >>| fun parent ->
    ...
    | ModuleType (parent, modt_name) ->
      from_identifier_no_anchor ~get_package parent ("module type " ^ modt_name)
      >>| fun parent ->
    ...
    | Type (parent, type_name) ->
      from_identifier_no_anchor ~get_package parent ("type " ^ type_name)
      >>| fun page ->
      let kind = "type" in
...
(*       | { anchor = ""; _ } as t -> Error (Missing_anchor (t, name)) *)
    ...
    | Extension (parent, name) ->
      from_identifier_no_anchor ~get_package parent ("extension " ^ name)
      >>| fun parent ->
      let kind = "extension" in
      { page = parent; anchor = Printf.sprintf "%s-%s" kind name; kind }
    | Exception (parent, name) ->
      from_identifier_no_anchor ~get_package parent ("exception " ^ name)
      >>| fun parent ->
    ...
    | Value (parent, name) ->
      from_identifier_no_anchor ~get_package parent ("val " ^ name)
      >>| fun parent ->
      let kind = "val" in
      { page = parent; anchor = Printf.sprintf "%s-%s" kind name; kind }
    | Class (parent, name) ->
      from_identifier_no_anchor ~get_package parent ("class " ^ name)
      >>| fun parent ->
      let kind = "class" in
    ...
    | ClassType (parent, name) ->
      from_identifier_no_anchor ~get_package parent ("class type " ^ name)
      >>| fun parent ->
      ...
      if stop_before then
        ...
        { page = suffix :: parent; anchor = ""; kind }
    | Method (parent, name) ->
      from_identifier_no_anchor ~get_package parent ("method " ^ name)
      >>| fun page ->
      let kind = "method" in
...
and from_identifier_no_anchor : type a. get_package:('x -> string) ->
  ('x, a) Identifier.t -> string -> (string list, Error.t) result =
  fun ~get_package id child ->
