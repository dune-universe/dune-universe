...
(* euro.ml : drawing of the graphps library logo.

   compile as a standalone using the graphps version of the graphics
   library. For instance

   ocamlc -I .. graphics.cmo graphps_logo.ml

   produces a.out. Then run ./a.out that writes the PostScript file
   "graphps_logo.eps", than can be visualized with a PostScript interpreter
   (gs, ghostview, gv, ...).
 *)

open Graphics;;

(* If debug is true, some auxilliary building drawings are shown. *)
...
open_eps "graphps_logo.eps";;
(*#use "graphps_logo.new.ml";;
*)
...
open_graph (Printf.sprintf " %ix%i" (szx + margin_x) (szy + margin_y));;
...
let line (xa, ya as a) (vx, vy as v) =
...
let solve_second_degre_eqn ({ a = a; b = b; c = c } as coefs) =
...
inter_segment_circle orig {x = 1.0; y = 0.0}  c_trigo;;
 *)
 ...
 | [ ({x = x1; y = y1} as p1); ({x = x2; y = y2} as p2) ] as inter ->
     let inter1 =
      let l1 =
...
let euro c x0 y0 sz =
 ...
 let show_seg (a, b) =
   if !debug_flag then 
    let (xc, yc as c) = current_point () in
    set_color red;
...
(* let poly2 = [| ta; (xp, yp); (xeast, yp); circle_east |] in*)
...
let cx = (szx / 2 + 2 * (round sz))
and cy = szy / 2;;

euro blue cx cy sz;;
euro magenta (cx + step) cy (sz -. (float step));;
euro green (cx + 2 * step) cy  ((sz -. 1.8 *. (float step)));;
euro red (cx + 3 * step) cy  ((sz -. 2.4 *. (float step)));;
euro cyan (cx + 4 * step) cy  ((sz -. 2.85 *. (float step)));;
euro black (cx + 5 * step) cy  ((sz -. 3.2 *. (float step)));;
(*
euro blue (cx + 6 * step) cy  ((sz -. 3.45 *. (float step)));;
euro magenta (cx + 7 * step) cy  ((sz -. 3.6 *. (float step)));;
euro green (cx + 8 * step) cy  ((sz -. 3.75 *. (float step)));;
euro red (cx + 9 * step) cy  ((sz -. 3.85 *. (float step)));;
*)
(*
