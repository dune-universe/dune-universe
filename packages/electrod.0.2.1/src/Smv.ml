...
module Make_SMV_file_format (Ltl : Solver.LTL)
  ...
  let make_script_file bmc dir script = 
    ...
    let first_line = match bmc with 
      | None -> ""
      | Some length -> "set bmc_length " ^ string_of_int length ^ "; "
    in
    begin
      match script with
        | Solver.File filename ->  (* script given on the command line *)
        ...
        | Solver.Default default ->
            IO.with_out tgt (fun out -> 
                  IO.write_line out (first_line ^ default))
    end;
    tgt
  ...
  let analyze ~conversion_time ~cmd ~script ~keep_files
    ...
    if no_analysis then begin
      ...
      let validity_check line = match bmc with
        | None -> String.suffix ~suf:"is true" line
        | Some length -> 
            let valid_bmc_string = 
              "-- no counterexample found with bound " ^ string_of_int length in 
