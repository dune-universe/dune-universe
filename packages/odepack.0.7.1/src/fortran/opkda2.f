*DECK DGEFA
      SUBROUTINE odepack_DGEFA (A, LDA, N, IPVT, INFO)
...
C
      DOUBLE PRECISION T
      INTEGER odepack_IDAMAX,J,K,KP1,L,NM1
C
...
C
         L = odepack_IDAMAX(N-K+1,A(K,K),1) + K - 1
         IPVT(K) = L
C
...
C
            T = -1.0D0/A(K,K)
            CALL odepack_DSCAL(N-K,T,A(K+1,K),1)
C
...
C
   ...
   70 CONTINUE
      ...
      END
*DECK DGESL
      SUBROUTINE odepack_DGESL (A, LDA, N, IPVT, B, JOB)
...
C
      DOUBLE PRECISION odepack_DDOT,T
      INTEGER K,KB,L,NM1
...
C
         ...
         DO 20 K = 1, NM1
            ...
            IF (L .EQ. K) GO TO 10
               ...
               B(K) = T
   10       CONTINUE
            CALL odepack_DAXPY(N-K,T,A(K+1,K),1,B(K+1),1)
   20    CONTINUE
   30    CONTINUE
C
C        NOW SOLVE  U*X = Y
...
C
         DO 60 K = 1, N
            T = odepack_DDOT(K-1,A(1,K),1,B(1),1)
            B(K) = (B(K) - T)/A(K,K)
   60    CONTINUE
C
...
C
         ...
         DO 80 KB = 1, NM1
            K = N - KB
            B(K) = B(K) + odepack_DDOT(N-K,A(K+1,K),1,B(K+1),1)
            L = IPVT(K)
            IF (L .EQ. K) GO TO 70
               T = B(L)
               B(L) = B(K)
  ...
  100 CONTINUE
      ...
      END
*DECK DGBFA
      SUBROUTINE odepack_DGBFA (ABD, LDA, N, ML, MU, IPVT, INFO)
...
C
      DOUBLE PRECISION T
      INTEGER I,odepack_IDAMAX,I0,J,JU,JZ,J0,J1,K,KP1,L,LM,M,MM,NM1
C
...
C
         LM = MIN(ML,N-K)
         L = odepack_IDAMAX(LM+1,ABD(M,K),1) + M - 1
         IPVT(K) = L + K - M
C
C        ZERO PIVOT IMPLIES THIS COLUMN ALREADY TRIANGULARIZED
C
...
C
            T = -1.0D0/ABD(M,K)
            CALL odepack_DSCAL(LM,T,ABD(M+1,K),1)
C
C           ROW ELIMINATION WITH COLUMN INDEXING
C
            ...
            DO 80 J = KP1, JU
               ...
               IF (L .EQ. MM) GO TO 70
                  ...
                  ABD(MM,J) = T
   70          CONTINUE
               CALL odepack_DAXPY(LM,T,ABD(M+1,K),1,ABD(MM+1,J),1)
   80       CONTINUE
   90       CONTINUE
         GO TO 110
  100    CONTINUE
  ...
  130 CONTINUE
      ...
      END
*DECK DGBSL
      SUBROUTINE odepack_DGBSL (ABD, LDA, N, ML, MU, IPVT, B, JOB)
         ...
         IF (NM1 .LT. 1) GO TO 30
            DO 20 K = 1, NM1
               ...
               IF (L .EQ. K) GO TO 10
                  ...
                  B(K) = T
   10          CONTINUE
               CALL odepack_DAXPY(LM,T,ABD(M+1,K),1,B(K+1),1)
   20       CONTINUE
   30    CONTINUE
C
C        NOW SOLVE  U*X = Y
C
         DO 40 KB = 1, N
            ...
            LB = K - LM
            T = -B(K)
            CALL odepack_DAXPY(LM,T,ABD(LA,K),1,B(LB),1)
   40    CONTINUE
      GO TO 100
   50 CONTINUE
C
...
C
         DO 60 K = 1, N
            ...
            LA = M - LM
            LB = K - LM
            T = odepack_DDOT(LM,ABD(LA,K),1,B(LB),1)
            B(K) = (B(K) - T)/ABD(M,K)
   60    CONTINUE
C
C        NOW SOLVE TRANS(L)*X = Y
C
         ...
         IF (NM1 .LT. 1) GO TO 90
            DO 80 KB = 1, NM1
               K = N - KB
               LM = MIN(ML,N-K)
               B(K) = B(K) + odepack_DDOT(LM,ABD(M+1,K),1,B(K+1),1)
               L = IPVT(K)
               IF (L .EQ. K) GO TO 70
                  T = B(L)
                  B(L) = B(K)
  ...
  100 CONTINUE
      ...
      END
*DECK DAXPY
      SUBROUTINE odepack_DAXPY (N, DA, DX, INCX, DY, INCY)
...
C
   ...
   70 CONTINUE
      ...
      END
*DECK DCOPY
      SUBROUTINE odepack_DCOPY (N, DX, INCX, DY, INCY)
...
C
   ...
   70 CONTINUE
      ...
      END
*DECK DDOT
      DOUBLE PRECISION FUNCTION odepack_DDOT (N, DX, INCX, DY, INCY)
...
C
    5 IX = 1
      ...
      IF (INCY .LT. 0) IY = (-N+1)*INCY + 1
      DO 10 I = 1,N
        odepack_DDOT = odepack_DDOT + DX(IX)*DY(IY)
        IX = IX + INCX
        IY = IY + INCY
   10 CONTINUE
      RETURN
...
C
   20 M = MOD(N,5)
      IF (M .EQ. 0) GO TO 40
      DO 30 I = 1,M
         odepack_DDOT = odepack_DDOT + DX(I)*DY(I)
   30 CONTINUE
      IF (N .LT. 5) RETURN
   40 MP1 = M + 1
      DO 50 I = MP1,N,5
      odepack_DDOT = odepack_DDOT + DX(I)*DY(I) + DX(I+1)*DY(I+1) +
     &  DX(I+2)*DY(I+2) +
...
C
   ...
   70 CONTINUE
      ...
      END
*DECK DNRM2
      DOUBLE PRECISION FUNCTION odepack_DNRM2 (N, DX, INCX)
...
C
      odepack_DNRM2 = XMAX * SQRT(SUM)
  300 CONTINUE
      RETURN
      END
*DECK DSCAL
      SUBROUTINE odepack_DSCAL (N, DA, DX, INCX)
...
C
   ...
   50 CONTINUE
      ...
      END
*DECK IDAMAX
      INTEGER FUNCTION odepack_IDAMAX (N, DX, INCX)
...
C
   ...
   30 CONTINUE
      ...
      END
*DECK XERRWD
      SUBROUTINE odepack_XERRWD (MSG, NMES, NERR, LEVEL, NI, I1, I2,
     &  NR, R1, R2)
...
C
      INTEGER LUNIT, odepack_IXSAV, MESFLG
C
C  Get logical unit number and message print flag.
C
C***FIRST EXECUTABLE STATEMENT  XERRWD
      LUNIT = odepack_IXSAV (1, 0, .FALSE.)
      MESFLG = odepack_IXSAV (2, 0, .FALSE.)
...
C----------------------- End of Subroutine XERRWD ----------------------
      END
*DECK XSETF
      SUBROUTINE odepack_XSETF (MFLAG)
...
C-----------------------------------------------------------------------
C**End
      INTEGER MFLAG, JUNK, odepack_IXSAV
C
C***FIRST EXECUTABLE STATEMENT  XSETF
      IF (MFLAG .EQ. 0 .OR. MFLAG .EQ. 1) JUNK = odepack_IXSAV
     &  (2,MFLAG,.TRUE.)
      RETURN
C----------------------- End of Subroutine XSETF -----------------------
      END
*DECK XSETUN
      SUBROUTINE odepack_XSETUN (LUN)
...
C-----------------------------------------------------------------------
C**End
      INTEGER LUN, JUNK, odepack_IXSAV
C
C***FIRST EXECUTABLE STATEMENT  XSETUN
      IF (LUN .GT. 0) JUNK = odepack_IXSAV (1,LUN,.TRUE.)
      RETURN
C----------------------- End of Subroutine XSETUN ----------------------
      END
*DECK IXSAV
      INTEGER FUNCTION odepack_IXSAV (IPAR, IVALUE, ISET)
...
C**End
      ...
      INTEGER IPAR, IVALUE
C-----------------------------------------------------------------------
      INTEGER odepack_IUMACH, LUNIT, MESFLG
C-----------------------------------------------------------------------
...
C***FIRST EXECUTABLE STATEMENT  IXSAV
      IF (IPAR .EQ. 1) THEN
        IF (LUNIT .EQ. -1) LUNIT = odepack_IUMACH()
        odepack_IXSAV = LUNIT
        IF (ISET) LUNIT = IVALUE
        ENDIF
C
      IF (IPAR .EQ. 2) THEN
        odepack_IXSAV = MESFLG
...
C----------------------- End of Function IXSAV -------------------------
      END
*DECK IUMACH
      INTEGER FUNCTION odepack_IUMACH()
