...
let format_failure formatter failure =
  Format.fprintf formatter "@[<v>@[%a@]@ "
    Pprintast.pattern failure.common;
...
let rec elim_type_constraints (expression : Parsetree.expression) =
  ...
  | { pexp_desc = Pexp_constraint (sub, _); _ } ->
      elim_type_constraints sub
  | _ -> expression

let pp_failure =
  Refl.pp [%refl: failure] []

let pp_pattern_result fmt_arg =
  Refl.pp [%refl: _ pattern_result] [Some fmt_arg]
