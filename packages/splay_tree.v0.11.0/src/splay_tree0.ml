...
= struct
  ...
  *)
  ...
  module Tree = struct
      ...
      fun t x ->
        let rec loop ctx this =
          ...
          | Node {left; key; data; right; _} ->
            ...
            if cmp < 0 then
              loop (Fst (ctx, key, data, right)) left
            else if cmp > 0 then
              loop (Snd (left, key, data, ctx)) right
            else
              (ctx, this)
        in
        loop Top t
    ...
    let nth_ctx t i =
      let rec loop ctx this i =
        ...
        | Node {left; key; data; right; _} ->
          ...
          if i < lsize then
            loop (Fst (ctx, key, data, right)) left i
    ...
    *)
    ...
    let splay_to_triple found =
      ...
      | (ctx, Empty) ->
        inject None               (splay empty empty ctx)

    (* Splay the phantom node to the root, splitting off the greater keys as the right side *)
    ...
    (* Splay the tree back together, but keep aside a reference to the node contents, if any *)
    let splay_to_result = function
      | (ctx, Node {left; key; data; right; _}) ->
        (splay_node left key data right ctx, Some (key, data))
      | (ctx, Empty) ->
        (splay_empty ctx,                    None)
    ...
    let remove_before t k =
      ...
      | None ->
        Second (concat_triple_unchecked before at after)
    ...
    let search t ~f =
      search_ctx t ~f |> splay_to_result
    ...
    let sexp_of_t t =
      Sexp.List (fold_right t ~init:[] ~f:(fun ~key ~data acc ->
    ...
    let rec merge left right ~f =
      ...
      | Node {left=l1; key; data=v1; right=r1; _}, Node _ ->
        ...
        let v =
          ...
          | None -> `Left v1
          | Some (k2, v2) ->
            assert (Key.compare key k2 = 0); (* Sanity check *)
            `Both (v1, v2)
        in
    ...
    let join l r =
      ...
      | (Snd (_, k1, _, _), Some (k2, v2, r)) ->
        ...
        then error_s [%message "Trees were overlapping"
                                 ~left_max:(k1 : key)
                                 ~right_min:(k2 : key)]
  ...
  let subrange ?min_key ?max_key t =
    pack t (subrange ?min_key ?max_key (unpack t))
  ...
  let merge a b ~f =
    create (merge ~f (unpack a) (unpack b))

  let join a b =
    Or_error.map ~f:create (join (unpack a) (unpack b))

  let join_exn a b =
    create (join_exn (unpack a) (unpack b))
end
...
module Reduction_operations = struct
  let reduce2 (type k) (type d) (type a) (type b)
        (module R1 : Reduction_operation
          with type key = k and type data = d and type accum = a)
        (module R2 : Reduction_operation
          with type key = k and type data = d and type accum = b)
    =
    (module struct
      type key = k
      type data = d
      ...
      let combine (l1, l2) (r1, r2) =
        ( R1.combine l1 r1
        , R2.combine l2 r2
        )
    end : Reduction_operation with type key = k and type data = d and type accum = a * b)
end
