...
let map kind input output =
  let decoder, close_input = match input with
    | `Std ->
      Coin.decoder kind (`Channel stdin),
      (fun () -> ())
    | `File file ->
      let ic = open_in file in
      Coin.decoder kind (`Channel ic),
      (fun () -> close_in ic) in
  let encoder, close_output, flush_output = match output with
    ...
    | `File file ->
      ...
      (fun () -> close_out oc),
      (fun () -> flush oc) in
  let rec go () = match Coin.decode decoder with
...
let koi8 =
  let parser s =
    try Ok (Coin.encoding_of_string s)
    with Invalid_argument err -> Error (`Msg err) in
  let pp = Fmt.using Coin.encoding_to_string Fmt.string in
  Arg.conv (parser, pp)

let kind =
  let doc = "Kind of KOI8." in
  Arg.(required & opt (some koi8) None & info ["k"; "kind"] ~doc ~docv:"<kind>")
