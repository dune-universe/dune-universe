...
module Log = (val Logs.src_log (Logs.Src.create "ACGtkLib.varUnionFind" ~doc:"logs ACGtkLib varUnionFind events") : Logs.LOG) 
...
struct
  ...
  let rec content_to_string c =
    ...
    | Value _v -> Printf.sprintf "Some Value"
 ...
 *)
  ...
  let to_string {rank=r;parents=p;_} =
  ...
  let log_content level {rank=r;parents=p;_} =
    Log.msg level
            (fun m ->
              let log_content_aux i =
                let content = content_to_string (Store.get i p) in
                let rk = Store.get i r in
  ...
  let rec find_aux i f =
    ...
    | Link_to next ->
	 ...
	 indexed by [i] directly to the representative index *)
      ...
      Log.debug (fun m ->
          m
            "the \"UnionFinf.find\" function indeed returns a Link_to itself: %B"
            (let () =
  ...
  let cyclic i h = 
    Log.debug (fun m -> m "Checking cyclicity from %d of:" i);
