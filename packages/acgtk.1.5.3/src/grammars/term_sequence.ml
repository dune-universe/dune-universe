...
(*  | (Term _)::(Term _)::tl,Some ((Op _) as tok) ->
  ...
  | (Op (_,f1,_))::(Op (_,f2,_))::_,Some tok when (is_prefix f2) && (is_prefix f1)->
     (* shift *)
     let token',stream' = next stream in
     parse_sequence_aux (tok::stack) token' stream' sg
...
(*  | (Op (_,f1,_))::(Op (o2,f2,l_o2))::_,_ when (is_prefix f2) && (is_infix f1) ->
  ...
  | (Term (t2,l2))::(Op (o1,f1,l_o1))::(Term (t1,l1))::tl,Some (Op (_o2,f2,_l_o2)) when
  ...
  | (Term (_t2,_l2))::(Op (_o1,f1,_l_o1))::(Term (_t1,_l1))::_, Some ((Term _) as t) when
         (is_infix f1) ->
