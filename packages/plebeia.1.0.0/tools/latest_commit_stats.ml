...
let () =
  ...
  let last_root = 
    ...
    Seq.fold_left (fun x root ->
        let y = match x with None -> Index.of_int 0 | Some r -> r.Roots.index in
        if y < root.Roots.index then Some root
        else x) None @@ Roots.to_seq roots 
  in
  
  Format.eprintf "last root index= %d@." @@ Index.to_int last_root.Roots.index;

  let segs = Hashtbl.create 0 in

  let c = from_Some @@ Vc.checkout vc last_root.Roots.hash in
