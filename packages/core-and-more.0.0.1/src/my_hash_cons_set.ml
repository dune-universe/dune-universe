...
module HCSetOf(D : UIDData) = struct
  ...
  let rec filter ~f:pr t =
    ...
    | Branch (p,m,t0,t1) -> branch (p, m, filter ~f:pr t0, filter ~f:pr t1)
  ...
  let rec min_exn t =
    ...
    | Empty -> failwith "not found"
    | Leaf k -> k
    | Branch (_,_,s,t) -> Stdlib.min (min_exn s) (min_exn t)

  let rec max_exn t =
    match t with
    | Empty -> failwith "not found"
    | Leaf k -> k
    | Branch (_,_,s,t) -> Stdlib.max (max_exn s) (max_exn t)

  let show t =
    let aux x acc = (D.show x) ^ (if String.equal acc "" then acc else "," ^ acc) in
