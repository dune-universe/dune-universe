...
let drain buf chan =
  try while true do Buffer.add_channel buf chan 1 done with End_of_file -> ()
...
let conv7 ~f ~t s () =
  ...
  let ic, oc, ec = Unix.open_process_full (Fmt.strf "./conv7 %s %s" f t) [||] in
    output_string oc s
  ; close_out oc
  ; drain rs ic
  ; drain er ec
  ; if Buffer.length er > 0
    then Fmt.epr "Got on stderr: @[%a@].\n%!" pp_raw (Buffer.contents er)
  ; let () = match Unix.close_process_full (ic, oc, ec) with
    | Unix.WEXITED 0 -> ()
    | Unix.WEXITED r ->
      Fmt.epr "%a: conv7 terminated with exit code (%d).\n%!" Fmt.(styled `Red string) "[ERROR]" r
    | Unix.WSIGNALED n ->
...
let generate length =
  ...
  let rec go rest =
    ...
    then (let x = Buffer.contents rs in Buffer.clear rs; x)
      ...
      try let uchar = Uchar.of_int n in
        Uutf.Buffer.add_utf_8 rs uchar
      ; go (pred rest)
      with Invalid_argument _ -> go rest in
  go length

let utf8 =
  Alcotest.testable pp_raw String.equal
