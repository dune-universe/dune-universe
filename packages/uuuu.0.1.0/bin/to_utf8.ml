...
let map kind input output =
  let decoder, close_input = match input with
    | `Std ->
      Uuuu.decoder kind (`Channel stdin),
      (fun () -> ())
    | `File file ->
      let ic = open_in file in
      Uuuu.decoder kind (`Channel ic),
      (fun () -> close_in ic) in
...
let iso8859 =
  let parser s =
    try Ok (Uuuu.encoding_of_string s)
    with Invalid_argument err -> Error (`Msg err) in
  let pp = Fmt.using Uuuu.encoding_to_string Fmt.string in
  Arg.conv (parser, pp)

let kind =
  let doc = "Kind of ISO-8859." in
  Arg.(required & opt (some iso8859) None & info ["k"; "kind"] ~doc ~docv:"<kind>")
