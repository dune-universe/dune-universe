...
(** [set_char_expand_factor factor] sets the current character expansion factor (width to height ratio).
    ...
    The expansion factor alters the width of the generated characters, but not their height.
    The default text expansion factor is 1, or one times the normal width-to-height ratio of the text.
...
(** [spline ?linetype ?linewidth ?coloridx x y m method_t] generates a cubic spline-fit, starting from the first data point and ending at the last data point.
    ...
    - [x]: The X coordinates
    - [y]: The Y coordinates
    - [m]: The number of points in the polygon to be drawn
...
(** [contour ?major_h x y h z] sraw contours of a three-dimensional data set whose values are specified over a rectangular mesh.
    ...
    - [h]: height values
    - [pz]: Z coordinates
    - [major_h]: Directs GR to label contour lines.
      For example, a value of 3 would label every third line.
      A value of 1 will label every line.
      A value of 0 produces no labels.
...
Parameters
    ...
    - [x]: X coordinates
    - [y]: Y coordinates
    - [h]: height values. If NULL, use nh evenly distributed height values between minimum and maximum Z value.
    - [major_h]: Directs GR to label contour lines.
      For example, a value of 3 would label every third line.
      A value of 1 will label every line. A value of 0 produces no labels.
      To produce colored contour lines, add an offset of 1000 to [major_h]
...
(** [grid ?scale ?linetype ?linewidth ?origin:(0,0) ?major:(1,1) x_tick y_tick] draws a linear and/or logarithmic grid.
    ...
    Major grid lines are drawn using black lines and minor grid lines are drawn using gray lines.
