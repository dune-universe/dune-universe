...
let get_skip_hours opts xmls =
  try
    ...
    match List.find (find_ele "skiphours") xmls with
      E ((_,_),subs) ->
        Some (List.sort Stdlib.compare (List.fold_left f_hour [] subs))
    | D _ ->
        assert false
  with
  | Error msg -> add_error opts msg; None
...
let get_skip_days opts xmls =
  ...
  try
    match List.find (find_ele "skipdays") xmls with
      E ((_,_),subs) ->
        Some (List.sort Stdlib.compare (List.fold_left f_day [] subs))
    | D _ ->
        assert false
  with
  | Error msg -> add_error opts msg; None
...
module SMap = Map.Make (struct type t = string let compare = Stdlib.compare end);;
