...
let test_shards_to_option_shards_to_shards _ =
  for _ = 0 to (100) - 1 do
...
let test_option_shards_to_shards_missing_shards_case1 _ =
  let shards = make_random_shards_bytes 1_000 10 in
...
let test_option_shards_to_shards_missing_shards_case2 _ =
  let shards = make_random_shards_bytes 1_000 10 in
  let option_shards = RS_Shard_utils.shards_to_option_shards_bytes shards in
...
let test_option_shards_to_missing_shards _ =
  let shards = make_random_shards_bytes 1_000 10 in
  let option_shards = RS_Shard_utils.shards_to_option_shards_bytes shards in
...
let test_reconstruct_error_handling _ =
  let r = ReedSolomon.make 2 2 in
...
let test_verify_too_few_shards _ =
...
let test_verify_with_buffer_incorrect_buffer_sizes _ =
  let r = ReedSolomon.make 3 2 in
...
let test_verify_with_buffer_gives_correct_parity_shards _ =
  let r = ReedSolomon.make 10 3 in
...
let test_slices_or_shards_count_check _ =
  let r = ReedSolomon.make 3 2 in
...
let test_check_slices_or_shards_size _ =
  let r = ReedSolomon.make 2 2 in
...
let shardbyshard_encode_correctly _ =
  begin (* bytes *)
    let r = ReedSolomon.make 10 3 in
...
let shardbyshard_encode_sep_correctly _ =
  let r = ReedSolomon.make 10 3 in
  let sbs = ShardByShard.make r in
...
let shardbyshard_encode_correctly_more_rigorous _ =
  begin (* bytes *)
    let r = ReedSolomon.make 10 3 in
    let sbs = ShardByShard.make r in
...
let shardbyshard_encode_error_handling _ =
  begin (* bytes *)
    begin
      let r = ReedSolomon.make 10 3 in
      let sbs = ShardByShard.make r in
...
let shardbyshard_encode_sep_error_handling _ =
  begin (* bytes *)
    begin
      let r = ReedSolomon.make 10 3 in
      let sbs = ShardByShard.make r in
...
let test_encode_single_sep_error_handling _ =
  let r = ReedSolomon.make 10 3 in
...
let test_encode_sep_error_handling _ =
  let r = ReedSolomon.make 10 3 in
...
let test_encode_single_error_handling _ =
  let r = ReedSolomon.make 10 3 in
