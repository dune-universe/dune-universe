...
let log_table_same_as_backblaze _ =
  for i = 0 to (256) - 1 do
...
let test_slice_add _ =
  List.iter
    (fun len ->
       let input = Bigstring.of_bytes (Bytes.init len (fun _ -> char_of_int (Random.int 256))) in
       let output = Bigstring.init len ~f:(fun _ -> char_of_int (Random.int 256)) in
...
let test_pure_ocaml_same_as_maybe_ffi _ =
  let len = 10_003 in
  for _ = 0 to (5000) - 1 do
    let c = char_of_int (Random.int 256) in
    let input = Bigstring.init len ~f:(fun _ -> char_of_int (Random.int 256)) in
    begin
      let output      = Bigstring.init len ~f:(fun _ -> char_of_int (Random.int 256)) in
