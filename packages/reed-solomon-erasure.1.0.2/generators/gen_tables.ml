...
let write_1D_table_str (oc : out_channel) (name : string) (table : int array) : unit =
...
let write_1D_table_bigarr (oc : out_channel) (name : string) (table : int array) : unit =
...
let write_2D_table_str (oc : out_channel) (name : string) (table : int array array) : unit =
  let rows = Array.length table in
  let cols = Array.length table.(0) in

  fprintf oc "let %s : string array = [|\n" name;
...
let write_2D_table_bigarr (oc : out_channel) (name : string) (table : int array array) : unit =
  let rows = Array.length table in
  let cols = Array.length table.(0) in
...
let main () =
  ...
  write_1D_table_str    oc "log_table"      log_table;
  write_1D_table_str    oc "exp_table"      exp_table;
  write_2D_table_str    oc "mul_table"      mul_table;
  write_2D_table_bigarr oc "mul_table_low"  mul_table_low;
  write_2D_table_bigarr oc "mul_table_high" mul_table_high;
