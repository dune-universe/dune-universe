...
void ocaml_srt_log_handler(void* opaque, int level, const char* file, int line, const char* area, const char* message) {
...
let () =
  ...
  in
  if locked then
    fn format ~prefix:"ocaml_srt" (module Srt_stubs_locked.Def)
  else
    fn ~concurrency:Cstubs.unlocked format ~prefix:"ocaml_srt" (module Srt_stubs.Def);
