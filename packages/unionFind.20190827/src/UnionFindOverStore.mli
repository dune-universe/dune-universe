...
(* Instead of hard-coding the use of mutable state, we parameterize the module
   over an implementation [S] of stores, as described by the signature
   [STORE]. This allows the client to choose between many different
   representations of the store: e.g., based on primitive references, based on
   a (possibly extensible) primitive array, based on a persistent map, based
   on a persistent or semi-persistent array, based on transactional
   references, etc. *)
...
module Make (S : STORE) : sig
  ...
  (* [union f s x y] does nothing if the references [x] and [y] are equal, and
     merges them if they are distinct. In the latter case, the new content of
     the merged reference is computed by applying the user-supplied function
     [f] to the original contents of [x] and [y]. *)
