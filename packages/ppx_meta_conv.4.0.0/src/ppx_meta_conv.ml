...
let str_value_maybe_rec vbs =
  ...
  Str.value Nonrecursive
    [ Vb.mk (ptuple & List.map pvar vars)
        (Exp.let_ Recursive vbs ~attrs:[ocaml_attr "warning" (str "-27-39")]

         & tuple & List.map evar vars)
    ]
  :: List.map (fun v -> Str.value Nonrecursive [ Vb.mk (Pat.any ()) (Exp.var v) ] ) vars
...
module Make(M : sig
  ...
  module Encoder = struct
    ...
    let sig_of_type ~path:_ type_decl =
      ...
      [ Sig.value & Val.mk
          (mknoloc (mangle_type_decl type_decl_current))
  ...
  module Decoder = struct
    ...
    let rec expr_of_typ_exn typ = 
      ...
      | { ptyp_desc = Ptyp_constr ({ txt = lid }, args) } ->
          ...
          exn & app (Exp.ident (mknoloc (mangle_lid lid))) args_pp
    ...
    let sig_of_type ~path:_ type_decl =
      ...
      let type_decl_current = Convert_to_current.copy_type_declaration type_decl in
      [ Sig.value & Val.mk
          (mknoloc & mangle_type_decl_exn type_decl_current)
            ...
            let cases =
              constrs |> List.map begin fun { pcd_name = { txt = constr_name }; pcd_args; pcd_attributes = attrs } ->
                ...
                | Pcstr_tuple ctys ->
                    [ Exp.case
                        (* (<constr_name'>, [a0; ..; an]) -> *)
                        (* or if record then (<constr_name'>, [<pat for record>] -> *)
                        [%pat?
                            ( [%p pstr constr_name'],
                    ...
                    fun ?trace:__t __v ->
                      [%e econv "result"]
                        [%e 
                         ...
                         app (Exp.ident (mknoloc (mangle_lid_exn lid))) args_pp ]
                        ?trace:__t __v
                ]))
        ]
      else
  ...
  let structure ~options ~path type_decls =
    ...
    & concat & flip map targets (fun s ->
      ...
      let module M = Make(struct
        let deriver = deriver
        let name = name
        let type_ = String.capitalize name ^ ".t"
        let conv = String.capitalize name ^ "_conv"
      end) in
