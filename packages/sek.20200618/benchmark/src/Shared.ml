...
(** Tools for parsing command line arguments (and other stuff)
    by Arthur Charguraud. *)
...
let rec list_pairing l =
  match l with
  | [] -> []
  | [_] -> failwith "invalid argument for list_pairing"
...
let list_assoc_replace_pair (k,v) l =
  ...
  with Not_found -> List.iter (fun (a,_) -> printf "%s\n" a) l; failwith  ("failed to find ")
...
let cmp_fst (x1,_) (x2,_) =
   x1 - x2

let cmp_snd (_,y1) (_,y2) =
   y1 - y2

let cmp_fst_desc (x1,_) (x2,_) =
   x2 - x1

let cmp_snd_desc (_,y1) (_,y2) =
   y2 - y1
...
let iter_flags f =
   Hashtbl.iter (fun k _v -> f k) flags
...
let figure n =
  if n mod 1000000000 = 0 then
    Printf.sprintf "%dG" (n / 1000000000)
  else if n mod 1000000 = 0 then
    Printf.sprintf "%dM" (n / 1000000)
