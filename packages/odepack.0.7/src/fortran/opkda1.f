*DECK DUMACH
      DOUBLE PRECISION FUNCTION odepack_DUMACH ()
...
C***FIRST EXECUTABLE STATEMENT  DUMACH
      U = 1.0D0
 10   U = U*0.5D0
      CALL odepack_DUMSUM(1.0D0, U, COMP)
      IF (COMP .NE. 1.0D0) GO TO 10
      odepack_DUMACH = U*2.0D0
      RETURN
C----------------------- End of Function DUMACH ------------------------
      END
      SUBROUTINE odepack_DUMSUM(A,B,C)
C     Routine to force normal storing of A + B, for DUMACH.
      ...
      END
*DECK DCFODE
      SUBROUTINE odepack_DCFODE (METH, ELCO, TESCO)
...
C----------------------- END OF SUBROUTINE DCFODE ----------------------
      END
*DECK DINTDY
      SUBROUTINE odepack_DINTDY (T, K, YH, NYH, DKY, IFLAG)
...
C
 ...
 60     DKY(I) = R*DKY(I)
      RETURN
C
 80   MSG = 'odepack_DINTDY-  K (=I1) illegal      '
      CALL odepack_XERRWD (MSG, 30, 51, 0, 1, K, 0, 0, 0.0D0, 0.0D0)
      IFLAG = -1
      RETURN
 90   MSG = 'odepack_DINTDY-  T (=R1) illegal      '
      CALL odepack_XERRWD (MSG, 30, 52, 0, 0, 0, 0, 1, T, 0.0D0)
      MSG='      T not in interval TCUR - HU (= R1) to TCUR (=R2)      '
      CALL odepack_XERRWD (MSG, 60, 52, 0, 0, 0, 0, 2, TP, TN)
      IFLAG = -2
      RETURN
C----------------------- END OF SUBROUTINE DINTDY ----------------------
      END
*DECK DPREPJ
      SUBROUTINE odepack_odepack_DPREPJ (NEQ, Y, YH, NYH, EWT, FTEM,
     &  SAVF, WM, IWM,
     1   F, JAC)
...
C If MITER = 2, make N calls to F to approximate J. --------------------
 200  FAC = odepack_DVNORM (N, SAVF, EWT)
...
C Do LU decomposition on P. --------------------------------------------
      CALL odepack_DGEFA (WM(3), N, N, IWM(21), IER)
      IF (IER .NE. 0) IERPJ = 1
      RETURN
...
C Do LU decomposition of P. --------------------------------------------
      CALL odepack_DGBFA (WM(3), MEBAND, N, ML, MU, IWM(21), IER)
      IF (IER .NE. 0) IERPJ = 1
      RETURN
C----------------------- END OF SUBROUTINE DPREPJ ----------------------
      END
*DECK DSOLSY
      SUBROUTINE odepack_DSOLSY (WM, IWM, X, TEM)
...
C***FIRST EXECUTABLE STATEMENT  DSOLSY
 ...
 100  CALL odepack_DGESL (WM(3), N, N, IWM(21), X, 0)
      RETURN
C
 300  PHL0 = WM(2)
      HL0 = H*EL0
...
C
 400  ML = IWM(1)
      MU = IWM(2)
      MEBAND = 2*ML + MU + 1
      CALL odepack_DGBSL (WM(3), MEBAND, N, ML, MU, IWM(21), X, 0)
      RETURN
C----------------------- END OF SUBROUTINE DSOLSY ----------------------
      END
*DECK DSRCOM
      SUBROUTINE odepack_DSRCOM (RSAV, ISAV, JOB)
...
C----------------------- END OF SUBROUTINE DSRCOM ----------------------
      END
*DECK DSTODE
      SUBROUTINE odepack_DSTODE (NEQ, Y, YH, NYH, YH1, EWT, SAVF, ACOR,
...
C**End
     ...
     1   R, RH, RHDN, RHSM, RHUP, TOLD, odepack_DVNORM
...
C-----------------------------------------------------------------------
 100  IPUP = MITER
      ...
      CALL odepack_DCFODE (METH, ELCO, TESCO)
      MEO = METH
 ...
 125    EL(I) = ELCO(I,NQ)
      ...
      EL0 = EL(1)
      CONIT = 0.5D0/(NQ+2)
      DDN = odepack_DVNORM (N, SAVF, EWT)/TESCO(1,L)
      EXDN = 1.0D0/L
...
C-----------------------------------------------------------------------
 140  CALL odepack_DCFODE (METH, ELCO, TESCO)
 150  DO 155 I = 1,L
 155    EL(I) = ELCO(I,NQ)
      NQNYH = NQ*NYH
      RC = RC*EL(1)/EL0
...
C-----------------------------------------------------------------------
 450  JCUR = 0
      ...
      IF (M .GT. 0) DSM = odepack_DVNORM (N, ACOR, EWT)/TESCO(2,NQ)
...
C-----------------------------------------------------------------------
 ...
 530    SAVF(I) = ACOR(I) - YH(I,LMAX)
      DUP = odepack_DVNORM (N, SAVF, EWT)/TESCO(3,NQ)
      EXUP = 1.0D0/(L+1)
 ...
 540  EXSM = 1.0D0/L
      ...
      DDN = odepack_DVNORM (N, YH(1,L), EWT)/TESCO(1,NQ)
      EXDN = 1.0D0/NQ
      RHDN = 1.0D0/(1.3D0*DDN**EXDN + 0.0000013D0)
...
C----------------------- END OF SUBROUTINE DSTODE ----------------------
      END
*DECK DEWSET
      SUBROUTINE odepack_DEWSET (N, ITOL, RTOL, ATOL, YCUR, EWT)
...
C----------------------- END OF SUBROUTINE DEWSET ----------------------
      END
*DECK DVNORM
      DOUBLE PRECISION FUNCTION odepack_DVNORM (N, V, W)
...
C***FIRST EXECUTABLE STATEMENT  DVNORM
 ...
 10     SUM = SUM + (V(I)*W(I))**2
      odepack_DVNORM = SQRT(SUM/N)
      RETURN
C----------------------- END OF FUNCTION DVNORM ------------------------
      END
*DECK DIPREP
      SUBROUTINE odepack_DIPREP (NEQ, Y, RWORK, IA, JA, IPFLAG, F, JAC)
      EXTERNAL F, JAC
...
C Call DPREP to do matrix preprocessing operations. --------------------
      CALL odepack_DPREP (NEQ, Y, RWORK(LYH), RWORK(LSAVF), RWORK(LEWT),
...
C----------------------- End of Subroutine DIPREP ----------------------
      END
*DECK DPREP
      SUBROUTINE odepack_DPREP (NEQ, Y, YH, SAVF, EWT, FTEM, IA, JA,
...
C Compute grouping of column indices (MITER = 2). ----------------------
      ...
      CALL odepack_JGROUP (N, IWK(IPIAN), IWK(IPJAN), MAXG, NGP,
     &  IWK(IPIGP),
...
C Reorder JAN and do symbolic LU factorization of matrix. --------------
 ...
 180    WK(IBA+I) = 0.0D0
     ...
     1   WK(IPA),WK(IPA),WK(IPA),NSP,IWK(IPISP),WK(IPRSP),IESP,5,IYS)
      ...
      CALL odepack_ADJLR (N, IWK(IPISP), LDIF)
      LREQ = LREQ + LDIF
 190  CONTINUE
...
C
 230  IPPER = -3
      CALL odepack_CNTNZU (N, IWK(IPIAN), IWK(IPJAN), NZSUT)
...
C----------------------- End of Subroutine DPREP -----------------------
      END
*DECK JGROUP
      SUBROUTINE odepack_JGROUP
     &  (N,IA,JA,MAXG,NGRP,IGP,JGP,INCL,JDONE,IER)
...
C----------------------- End of Subroutine JGROUP ----------------------
      END
*DECK ADJLR
      SUBROUTINE odepack_ADJLR (N, ISP, LDIF)
      INTEGER N, ISP, LDIF
      DIMENSION ISP(*)
C-----------------------------------------------------------------------
...
C----------------------- End of Subroutine ADJLR -----------------------
      END
*DECK CNTNZU
      SUBROUTINE odepack_CNTNZU (N, IA, JA, NZSUT)
...
C----------------------- End of Subroutine CNTNZU ----------------------
      END
*DECK DPRJS
      SUBROUTINE odepack_DPRJS (NEQ,Y,YH,NYH,EWT,FTEM,SAVF,WK,IWK,F,JAC)
      EXTERNAL F,JAC
      INTEGER NEQ, NYH, IWK
...
C----------------------- End of Subroutine DPRJS -----------------------
      END
*DECK DSOLSS
      SUBROUTINE odepack_DSOLSS (WK, IWK, X, TEM)
      INTEGER IWK
...
C----------------------- End of Subroutine DSOLSS ----------------------
      END
*DECK DSRCMS
      SUBROUTINE odepack_DSRCMS (RSAV, ISAV, JOB)
C-----------------------------------------------------------------------
C This routine saves or restores (depending on JOB) the contents of
...
c ** error.. illegal path specification
 111  flag = 11*n + 1
      ...
      subroutine odepack_nroc (n, ic, ia, ja, a, jar, ar, p, flag)
c
c       ----------------------------------------------------------------
c
...
*DECK DSTODA
      SUBROUTINE odepack_DSTODA (NEQ, Y, YH, NYH, YH1, EWT, SAVF, ACOR,
     1   WM, IWM, F, JAC, PJAC, SLVS)
      EXTERNAL F, JAC, PJAC, SLVS
      INTEGER NEQ, NYH, IWM
      DOUBLE PRECISION Y, YH, YH1, EWT, SAVF, ACOR, WM
     ...
     2   IOWND2(3), ICOUNT, IRFLAG, JTYP, MUSED, MXORDN, MXORDS
      ...
      DOUBLE PRECISION DCON, DDN, DEL, DELP, DSM, DUP, EXDN, EXSM, EXUP,
     1   R, RH, RHDN, RHSM, RHUP, TOLD, odepack_DMNORM
...
C Initialize switching parameters.  METH = 1 is assumed initially. -----
      ...
      PDLAST = 0.0D0
      RATIO = 5.0D0
      CALL odepack_DCFODE (2, ELCO, TESCO)
      DO 10 I = 1,5
 10     CM2(I) = TESCO(2,I)*ELCO(I+1,I)
      CALL odepack_DCFODE (1, ELCO, TESCO)
      DO 20 I = 1,12
...
CDIR$ IVDEP
 ...
 210      YH1(I) = YH1(I) + YH1(I+NYH)
 215    CONTINUE
      PNORM = odepack_DMNORM (N, YH1, EWT)
C-----------------------------------------------------------------------
C Up to MAXCOR corrector iterations are taken.  A convergence test is
...
C-----------------------------------------------------------------------
 450  JCUR = 0
      IF (M .EQ. 0) DSM = DEL/TESCO(2,NQ)
      IF (M .GT. 0) DSM = odepack_DMNORM (N, ACOR, EWT)/TESCO(2,NQ)
      IF (DSM .GT. 1.0D0) GO TO 500
C-----------------------------------------------------------------------
C After a successful step, update the YH array.
...
C-----------------------------------------------------------------------
 ...
 470  CONTINUE
      ...
      IF (NQ .LE. MXORDS) GO TO 474
         ...
         EXM2 = 1.0D0/LM2
         LM2P1 = LM2 + 1
         DM2 = odepack_DMNORM (N, YH(1,LM2P1), EWT)/CM2(MXORDS)
...
C-----------------------------------------------------------------------
 480  CONTINUE
      ...
      IF (MXORDN .GE. NQ) GO TO 484
         ...
         EXM1 = 1.0D0/LM1
         LM1P1 = LM1 + 1
         DM1 = odepack_DMNORM (N, YH(1,LM1P1), EWT)/CM1(MXORDN)
...
C-----------------------------------------------------------------------
 520  RHUP = 0.0D0
      ...
      DO 530 I = 1,N
 530    SAVF(I) = ACOR(I) - YH(I,LMAX)
      DUP = odepack_DMNORM (N, SAVF, EWT)/TESCO(3,NQ)
      EXUP = 1.0D0/(L+1)
      RHUP = 1.0D0/(1.4D0*DUP**EXUP + 0.0000014D0)
 540  EXSM = 1.0D0/L
      RHSM = 1.0D0/(1.2D0*DSM**EXSM + 0.0000012D0)
      ...
      DDN = odepack_DMNORM (N, YH(1,L), EWT)/TESCO(1,NQ)
      EXDN = 1.0D0/NQ
      RHDN = 1.0D0/(1.3D0*DDN**EXDN + 0.0000013D0)
...
C----------------------- End of Subroutine DSTODA ----------------------
      END
*DECK DPRJA
      SUBROUTINE odepack_DPRJA (NEQ, Y, YH, NYH, EWT, FTEM, SAVF, WM,
     &  IWM,
     1   F, JAC)
      EXTERNAL F, JAC
      INTEGER NEQ, NYH, IWM
     ...
     1   odepack_DMNORM, odepack_DFNORM, odepack_DBNORM
C-----------------------------------------------------------------------
...
C If MITER = 1, call JAC and multiply by scalar. -----------------------
 ...
 120    WM(I+2) = WM(I+2)*CON
      GO TO 240
C If MITER = 2, make N calls to F to approximate J. --------------------
 200  FAC = odepack_DMNORM (N, SAVF, EWT)
      R0 = 1000.0D0*ABS(H)*UROUND*N*FAC
      IF (R0 .EQ. 0.0D0) R0 = 1.0D0
      SRUR = WM(1)
      J1 = 2
...
C Compute norm of Jacobian. --------------------------------------------
      PDNORM = odepack_DFNORM (N, WM(3), EWT)/ABS(HL0)
C Add identity matrix. -------------------------------------------------
      J = 3
      NP1 = N + 1
      DO 250 I = 1,N
...
C If MITER = 5, make MBAND calls to F to approximate J. ----------------
 ...
 570  CONTINUE
C Compute norm of Jacobian. --------------------------------------------
      PDNORM = odepack_DBNORM (N, WM(ML+3), MEBAND, ML, MU,
     &  EWT)/ABS(HL0)
C Add identity matrix. -------------------------------------------------
      II = MBAND + 2
      DO 580 I = 1,N
...
C----------------------- End of Subroutine DPRJA -----------------------
      END
*DECK DMNORM
      DOUBLE PRECISION FUNCTION odepack_DMNORM (N, V, W)
C-----------------------------------------------------------------------
...
C----------------------- End of Function DMNORM ------------------------
      END
*DECK DFNORM
      DOUBLE PRECISION FUNCTION odepack_DFNORM (N, A, W)
C-----------------------------------------------------------------------
...
C----------------------- End of Function DFNORM ------------------------
      END
*DECK DBNORM
      DOUBLE PRECISION FUNCTION odepack_DBNORM (N, A, NRA, ML, MU, W)
C-----------------------------------------------------------------------
...
C----------------------- End of Function DBNORM ------------------------
      END
*DECK DSRCMA
      SUBROUTINE odepack_DSRCMA (RSAV, ISAV, JOB)
C-----------------------------------------------------------------------
C This routine saves or restores (depending on JOB) the contents of
...
C----------------------- End of Subroutine DSRCMA ----------------------
      END
*DECK DRCHEK
      SUBROUTINE odepack_DRCHEK (JOB, G, NEQ, Y, YH,NYH, G0, G1, GX,
     &  JROOT, IRT)
      EXTERNAL G
...
C If a root was found on the previous step, evaluate G0 = g(T0). -------
      CALL odepack_DINTDY (T0, 0, YH, NYH, Y, IFLAG)
      CALL G (NEQ, T0, Y, NGC, G0)
      NGE = NGE + 1
      ZROOT = .FALSE.
      DO 210 I = 1,NGC
...
C g has a zero at T0.  Look at g at T + (small increment). -------------
 ...
 220    Y(I) = Y(I) + TEMP2*YH(I,2)
      GO TO 240
 230  CALL odepack_DINTDY (T0, 0, YH, NYH, Y, IFLAG)
...
C Set T1 to TN or TOUTC, whichever comes first, and get g at T1. -------
      ...
      CALL odepack_DINTDY (T1, 0, YH, NYH, Y, IFLAG)
      GO TO 330
 310  T1 = TN
      DO 320 I = 1,N
 320    Y(I) = YH(I,1)
...
C Call DROOTS to search for root in interval from T0 to T1. ------------
      JFLAG = 0
 350  CONTINUE
      CALL odepack_DROOTS (NGC, HMING, JFLAG, T0, T1, G0, G1, GX, X,
     &  JROOT)
      IF (JFLAG .GT. 1) GO TO 360
      CALL odepack_DINTDY (X, 0, YH, NYH, Y, IFLAG)
      CALL G (NEQ, X, Y, NGC, GX)
      NGE = NGE + 1
      GO TO 350
 360  T0 = X
      CALL odepack_DCOPY (NGC, GX, 1, G0, 1)
...
C----------------------- End of Subroutine DRCHEK ----------------------
      END
*DECK DROOTS
      SUBROUTINE odepack_DROOTS (NG, HMIN, JFLAG, X0, X1, G0, G1, GX,
     &  X, JROOT)
...
C Sign change between X0 and X2, so replace X1 with X2. ----------------
      X1 = X2
      CALL odepack_DCOPY (NG, GX, 1, G1, 1)
      LAST = 1
      XROOT = .FALSE.
      GO TO 270
...
C No sign change between X0 and X2.  Replace X0 with X2. ---------------
 260  CONTINUE
      CALL odepack_DCOPY (NG, GX, 1, G0, 1)
      X0 = X2
      LAST = 0
      XROOT = .FALSE.
...
C Return with X1 as the root.  Set JROOT.  Set X = X1 and GX = G1. -----
 300  JFLAG = 2
      X = X1
      CALL odepack_DCOPY (NG, G1, 1, GX, 1)
      DO 320 I = 1,NG
        JROOT(I) = 0
...
C No sign changes in this interval.  Set X = X1, return JFLAG = 4. -----
 420  CALL odepack_DCOPY (NG, G1, 1, GX, 1)
      X = X1
      JFLAG = 4
      RETURN
C----------------------- End of Subroutine DROOTS ----------------------
      END
*DECK DSRCAR
      SUBROUTINE odepack_DSRCAR (RSAV, ISAV, JOB)
C-----------------------------------------------------------------------
C This routine saves or restores (depending on JOB) the contents of
...
C----------------------- End of Subroutine DSRCAR ----------------------
      END
*DECK DSTODPK
      SUBROUTINE odepack_DSTODPK (NEQ, Y, YH, NYH, YH1, EWT, SAVF,
     &  SAVX, ACOR,
     1   WM, IWM, F, JAC, PSOL)
      EXTERNAL F, JAC, PSOL
      INTEGER NEQ, NYH, IWM
...
C-----------------------------------------------------------------------
      CALL odepack_DPKSET (NEQ, Y, YH1, EWT, ACOR, SAVF, WM, IWM, F,
     &  JAC)
      IPUP = 0
      RC = 1.0D0
      NSLP = NST
...
C-----------------------------------------------------------------------
 ...
 360    SAVX(I) = H*SAVF(I) - (YH(I,2) + ACOR(I))
      CALL odepack_DSOLPK (NEQ, Y, SAVF, SAVX, EWT, WM, IWM, F, PSOL)
      IF (IERSL .LT. 0) GO TO 430
      IF (IERSL .GT. 0) GO TO 410
      DEL = odepack_DVNORM (N, SAVX, EWT)
      DO 380 I = 1,N
...
C----------------------- End of Subroutine DSTODPK ---------------------
      END
*DECK DPKSET
      SUBROUTINE odepack_DPKSET (NEQ, Y, YSV, EWT, FTEM, SAVF, WM, IWM,
     &  F, JAC)
      EXTERNAL F, JAC
      INTEGER NEQ, IWM
...
C----------------------- End of Subroutine DPKSET ----------------------
      END
*DECK DSOLPK
      SUBROUTINE odepack_DSOLPK (NEQ, Y, SAVF, X, EWT, WM, IWM, F, PSOL)
      EXTERNAL F, PSOL
      INTEGER NEQ, IWM
...
C-----------------------------------------------------------------------
 100  CONTINUE
      ...
      LHES = LB + N
      LWK = LHES + MAXL*MAXL
      CALL odepack_DCOPY (N, X, 1, WM(LB), 1)
      CALL odepack_DSCAL (N, RSQRTN, EWT, 1)
      CALL odepack_DSPIOM (NEQ, TN, Y, SAVF, WM(LB), EWT, N, MAXL, KMP,
     &  DELTA,
     ...
     2   LIOM, WM(LOCWP), IWM(LOCIWP), WM(LWK), IFLAG)
      ...
      NLI = NLI + LIOM
      NPS = NPS + NPSL
      CALL odepack_DSCAL (N, SQRTN, EWT, 1)
...
C-----------------------------------------------------------------------
 200  CONTINUE
      ...
      CALL odepack_DCOPY (N, X, 1, WM(LB), 1)
      CALL odepack_DSCAL (N, RSQRTN, EWT, 1)
      CALL odepack_DSPIGMR (NEQ, TN, Y, SAVF, WM(LB), EWT, N, MAXL,
     &  MAXLP1, KMP,
...
C-----------------------------------------------------------------------
 300  CONTINUE
      ...
      LZ = LW + N
      LWK = LZ + N
      CALL odepack_DCOPY (N, X, 1, WM(LR), 1)
      CALL odepack_DPCG (NEQ, TN, Y, SAVF, WM(LR), EWT, N, MAXL, DELTA,
     &  HL0,
...
C-----------------------------------------------------------------------
 400  CONTINUE
      ...
      LWK = LZ + N
      CALL odepack_DCOPY (N, X, 1, WM(LR), 1)
      CALL odepack_odepack_DPCGS (NEQ, TN, Y, SAVF, WM(LR), EWT, N,
     &  MAXL, DELTA, HL0,
     1           JPRE, MNEWT, F, PSOL, NPSL, X, WM(LP), WM(LW), WM(LZ),
     2           LPCG, WM(LOCWP), IWM(LOCIWP), WM(LWK), IFLAG)
      NNI = NNI + 1
...
C-----------------------------------------------------------------------
 900  CONTINUE
      ...
      LWK = LB + N
      CALL odepack_DCOPY (N, X, 1, WM(LB), 1)
      CALL odepack_DUSOL (NEQ, TN, Y, SAVF, WM(LB), EWT, N, DELTA, HL0,
     &  MNEWT,
...
C----------------------- End of Subroutine DSOLPK ----------------------
      END
*DECK DSPIOM
      SUBROUTINE odepack_DSPIOM (NEQ, TN, Y, SAVF, B, WGHT, N, MAXL,
     &  KMP, DELTA,
...
C-----------------------------------------------------------------------
      ...
      DOUBLE PRECISION BNRM, BNRM0, PROD, RHO, SNORMW, odepack_DNRM2,
     &  TEM
C
      IFLAG = 0
      LIOM = 0
...
C-----------------------------------------------------------------------
      DO 10 I = 1,N
 10     V(I,1) = B(I)*WGHT(I)
      BNRM0 = odepack_DNRM2 (N, V, 1)
      BNRM = BNRM0
      IF (BNRM0 .GT. DELTA) GO TO 30
      IF (MNEWT .GT. 0) GO TO 20
      CALL odepack_DCOPY (N, B, 1, X, 1)
      RETURN
 20   DO 25 I = 1,N
 25     X(I) = 0.0D0
      RETURN
...
C Calculate norm of scaled vector V(*,1) and normalize it. -------------
      DO 50 I = 1,N
 50     V(I,1) = B(I)*WGHT(I)
      BNRM = odepack_DNRM2(N, V, 1)
      DELTA = DELTA*(BNRM/BNRM0)
 55   TEM = 1.0D0/BNRM
      CALL odepack_DSCAL (N, TEM, V(1,1), 1)
...
C-----------------------------------------------------------------------
        CALL odepack_DATV (NEQ, Y, SAVF, V(1,LL), WGHT, X, F, PSOL,
     &  V(1,LL+1),
     1        WK, WP, IWP, HL0, JPRE, IER, NPSL)
        IF (IER .NE. 0) GO TO 300
        CALL odepack_DORTHOG (V(1,LL+1), V, HES, N, LL, MAXL, KMP,
     &  SNORMW)
        CALL odepack_DHEFA (HES, MAXL, LL, IPVT, INFO, LL)
        LM1 = LL - 1
...
C If l .lt. MAXL, store HES(l+1,l) and normalize the vector v(*,l+1).
 80     CONTINUE
        HES(LL+1,LL) = SNORMW
        TEM = 1.0D0/SNORMW
        CALL odepack_DSCAL (N, TEM, V(1,LL+1), 1)
 90     CONTINUE
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
 ...
 210    B(K) = 0.0D0
      B(1) = BNRM
      CALL odepack_DHESL (HES, MAXL, LL, IPVT, B)
      DO 220 K = 1,N
 220    X(K) = 0.0D0
      DO 230 I = 1,LL
        CALL odepack_DAXPY (N, B(I), V(1,I), 1, X, 1)
 230    CONTINUE
      DO 240 I = 1,N
 240    X(I) = X(I)/WGHT(I)
      IF (JPRE .LE. 1) RETURN
...
C----------------------- End of Subroutine DSPIOM ----------------------
      END
*DECK DATV
      SUBROUTINE odepack_DATV (NEQ, Y, SAVF, V, WGHT, FTEM, F, PSOL, Z,
     &  VTEM,
...
C-----------------------------------------------------------------------
      INTEGER I
      DOUBLE PRECISION FAC, RNORM, odepack_DNRM2, TEMPN
C
C Set VTEM = D * V.
      DO 10 I = 1,N
...
C----------------------- End of Subroutine DATV ------------------------
      END
*DECK DORTHOG
      SUBROUTINE odepack_DORTHOG (VNEW, V, HES, N, LL, LDHES, KMP,
     &  SNORMW)
...
C-----------------------------------------------------------------------
      INTEGER I, I0
      DOUBLE PRECISION ARG, odepack_DDOT, odepack_DNRM2, SUMDSQ, TEM,
     &  VNRM
C
C Get norm of unaltered VNEW for later use. ----------------------------
      VNRM = odepack_DNRM2 (N, VNEW, 1)
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      I0 = MAX(1,LL-KMP+1)
      DO 10 I = I0,LL
        HES(I,LL) = odepack_DDOT (N, V(1,I), 1, VNEW, 1)
        TEM = -HES(I,LL)
        CALL odepack_DAXPY (N, TEM, V(1,I), 1, VNEW, 1)
 10     CONTINUE
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      SNORMW = odepack_DNRM2 (N, VNEW, 1)
      IF (VNRM + 0.001D0*SNORMW .NE. VNRM) RETURN
      SUMDSQ = 0.0D0
      DO 30 I = I0,LL
        TEM = -odepack_DDOT (N, V(1,I), 1, VNEW, 1)
...
C----------------------- End of Subroutine DORTHOG ---------------------
      END
*DECK DSPIGMR
      SUBROUTINE odepack_DSPIGMR (NEQ, TN, Y, SAVF, B, WGHT, N, MAXL,
     &  MAXLP1,
...
C-----------------------------------------------------------------------
      ...
      DOUBLE PRECISION
     &  BNRM,BNRM0,C,DLNRM,PROD,RHO,S,SNORMW,odepack_DNRM2,TEM
C
      IFLAG = 0
      LGMR = 0
      NPSL = 0
...
C-----------------------------------------------------------------------
     ...
     1        WK, WP, IWP, HL0, JPRE, IER, NPSL)
        IF (IER .NE. 0) GO TO 300
        CALL odepack_DORTHOG (V(1,LL+1), V, HES, N, LL, MAXLP1, KMP,
     &  SNORMW)
        HES(LL+1,LL) = SNORMW
        CALL odepack_DHEQR (HES, MAXLP1, LL, Q, INFO, LL)
...
C-----------------------------------------------------------------------
        ...
        IF ((LL.GT.KMP) .AND. (KMP.LT.MAXL)) THEN
          IF (LL .EQ. KMP+1) THEN
            CALL odepack_DCOPY (N, V(1,1), 1, DL, 1)
            DO 75 I = 1,KMP
              IP1 = I + 1
              I2 = I*2
              S = Q(I2)
...
C-----------------------------------------------------------------------
 ...
 210    B(K) = 0.0D0
      B(1) = BNRM
      CALL odepack_DHELS (HES, MAXLP1, LL, Q, B)
      DO 220 K = 1,N
 220    X(K) = 0.0D0
      DO 230 I = 1,LL
        CALL odepack_DAXPY (N, B(I), V(1,I), 1, X, 1)
...
C----------------------- End of Subroutine DSPIGMR ---------------------
      END
*DECK DPCG
      SUBROUTINE odepack_DPCG (NEQ, TN, Y, SAVF, R, WGHT, N, MAXL,
     &  DELTA, HL0,
...
C-----------------------------------------------------------------------
      INTEGER I, IER
      DOUBLE PRECISION
     &  ALPHA,BETA,BNRM,PTW,RNRM,odepack_DDOT,odepack_DVNORM,ZTR,ZTR0
C
      IFLAG = 0
      NPSL = 0
      LPCG = 0
...
C Loop point for PCG iterations. ---------------------------------------
 ...
 40   CONTINUE
      ZTR0 = ZTR
      ZTR = odepack_DDOT (N, Z, 1, R, 1)
      IF (LPCG .NE. 1) GO TO 50
      CALL odepack_DCOPY (N, Z, 1, P, 1)
      GO TO 70
 50   CONTINUE
...
C-----------------------------------------------------------------------
      CALL odepack_DATP (NEQ, Y, SAVF, P, WGHT, HL0, WK, F, W)
C
      PTW = odepack_DDOT (N, P, 1, W, 1)
      IF (PTW .EQ. 0.0D0) GO TO 200
      ALPHA = ZTR/PTW
      CALL odepack_DAXPY (N, ALPHA, P, 1, X, 1)
      ALPHA = -ALPHA
      CALL odepack_DAXPY (N, ALPHA, W, 1, R, 1)
      RNRM = odepack_DVNORM (N, R, WGHT)
...
C----------------------- End of Subroutine DPCG ------------------------
      END
*DECK DPCGS
      SUBROUTINE odepack_odepack_DPCGS (NEQ, TN, Y, SAVF, R, WGHT, N,
     &  MAXL, DELTA, HL0,
     1 JPRE, MNEWT, F, PSOL, NPSL, X, P, W, Z, LPCG, WP, IWP, WK, IFLAG)
      EXTERNAL F, PSOL
      INTEGER NEQ, N, MAXL, JPRE, MNEWT, NPSL, LPCG, IWP, IFLAG
...
C-----------------------------------------------------------------------
      INTEGER I, IER
      DOUBLE PRECISION ALPHA, BETA, BNRM, PTW, RNRM, odepack_DVNORM,
     &  ZTR, ZTR0
C
      IFLAG = 0
      NPSL = 0
...
C----------------------- End of Subroutine DPCGS -----------------------
      END
*DECK DATP
      SUBROUTINE odepack_DATP (NEQ, Y, SAVF, P, WGHT, HL0, WK, F, W)
      EXTERNAL F
      INTEGER NEQ
...
C-----------------------------------------------------------------------
      INTEGER I
      DOUBLE PRECISION FAC, PNRM, RPNRM, odepack_DVNORM
C
      PNRM = odepack_DVNORM (N, P, WGHT)
      RPNRM = 1.0D0/PNRM
      CALL odepack_DCOPY (N, Y, 1, W, 1)
      DO 20 I = 1,N
 20     Y(I) = W(I) + P(I)*RPNRM
      CALL F (NEQ, TN, Y, WK)
      NFE = NFE + 1
      CALL odepack_DCOPY (N, W, 1, Y, 1)
      FAC = HL0*PNRM
      DO 40 I = 1,N
...
C----------------------- End of Subroutine DATP ------------------------
      END
*DECK DUSOL
      SUBROUTINE odepack_DUSOL (NEQ, TN, Y, SAVF, B, WGHT, N, DELTA,
     &  HL0, MNEWT,
...
C-----------------------------------------------------------------------
      INTEGER I, IER
      DOUBLE PRECISION BNRM, odepack_DVNORM
C
      IFLAG = 0
      NPSL = 0
C-----------------------------------------------------------------------
...
C----------------------- End of Subroutine DUSOL -----------------------
      END
*DECK DSRCPK
      SUBROUTINE odepack_DSRCPK (RSAV, ISAV, JOB)
C-----------------------------------------------------------------------
C This routine saves or restores (depending on JOB) the contents of
...
C
      IF (JOB .EQ. 2) GO TO 100
      CALL odepack_DCOPY (LENRLS, RLS, 1, RSAV, 1)
      CALL odepack_DCOPY (LENRLP, RLSP, 1, RSAV(LENRLS+1), 1)
      DO 20 I = 1,LENILS
 20     ISAV(I) = ILS(I)
      DO 40 I = 1,LENILP
...
C
 100  CONTINUE
      CALL odepack_DCOPY (LENRLS, RSAV, 1, RLS, 1)
      CALL odepack_DCOPY (LENRLP, RSAV(LENRLS+1), 1, RLSP, 1)
      DO 120 I = 1,LENILS
 120     ILS(I) = ISAV(I)
      DO 140 I = 1,LENILP
...
C----------------------- End of Subroutine DSRCPK ----------------------
      END
*DECK DHEFA
      SUBROUTINE odepack_DHEFA (A, LDA, N, IPVT, INFO, JOB)
...
C-----------------------------------------------------------------------
      INTEGER odepack_IDAMAX, J, K, KM1, KP1, L, NM1
      DOUBLE PRECISION T
C
...
C
         L = odepack_IDAMAX (2, A(K,K), 1) + K - 1
         IPVT(K) = L
C
...
C
            DO 30 J = KP1, N
               ...
               IF (L .EQ. K) GO TO 20
                  ...
                  A(K,J) = T
   20          CONTINUE
               CALL odepack_DAXPY (N-K, T, A(K+1,K), 1, A(K+1,J), 1)
   30       CONTINUE
         GO TO 50
   40    CONTINUE
            INFO = K
...
C        Find L = pivot index
C
         L = odepack_IDAMAX (2, A(NM1,NM1), 1) + NM1 - 1
         IPVT(NM1) = L
C
C        Zero pivot implies this column already triangularized
C
...
C----------------------- End of Subroutine DHEFA -----------------------
      END
*DECK DHESL
      SUBROUTINE odepack_DHESL (A, LDA, N, IPVT, B)
      INTEGER LDA, N, IPVT(*)
...
C
         DO 40 KB = 1, N
            ...
            B(K) = B(K)/A(K,K)
            T = -B(K)
            CALL odepack_DAXPY (K-1, T, A(1,K), 1, B(1), 1)
   40    CONTINUE
      RETURN
C----------------------- End of Subroutine DHESL -----------------------
      END
*DECK DHEQR
      SUBROUTINE odepack_DHEQR (A, LDA, N, Q, INFO, IJOB)
...
C----------------------- End of Subroutine DHEQR -----------------------
      END
*DECK DHELS
      SUBROUTINE odepack_DHELS (A, LDA, N, Q, B)
      INTEGER LDA, N
...
C----------------------- End of Subroutine DHELS -----------------------
      END
*DECK DLHIN
      SUBROUTINE odepack_DLHIN (NEQ, N, T0, Y0, YDOT, F, TOUT, UROUND,
...
C
     ...
     1     HUB, HUN, PT1, T1, TDIST, TROUND, TWO, odepack_DVNORM, YDDNRM
      INTEGER I, ITER
C-----------------------------------------------------------------------
...
C Estimate the second derivative as a difference quotient in f. --------
 ...
 70     TEMP(I) = (TEMP(I) - YDOT(I))/HG
      YDDNRM = odepack_DVNORM (N, TEMP, EWT)
...
C----------------------- End of Subroutine DLHIN -----------------------
      END
*DECK DSTOKA
      SUBROUTINE odepack_DSTOKA (NEQ, Y, YH, NYH, YH1, EWT, SAVF, SAVX,
     &  ACOR,
     1   WM, IWM, F, JAC, PSOL)
      EXTERNAL F, JAC, PSOL
      INTEGER NEQ, NYH, IWM
...
C-----------------------------------------------------------------------
     ...
     1   EXUP, odepack_DFNORM, R, RH, RHDN, RHSM, RHUP, ROC, STIFF,
     &  TOLD, odepack_DVNORM
C
      KFLAG = 0
      TOLD = TN
...
C-----------------------------------------------------------------------
      ...
      IF (JOK .EQ. -1) THEN
        ...
        NJEV = NJEV + 1
        ENDIF
      CALL odepack_DSETPK (NEQ, Y, YH1, EWT, ACOR, SAVF, JOK, WM, IWM,
     &  F, JAC)
      IPUP = 0
      RC = 1.0D0
      DRC = 0.0D0
...
C-----------------------------------------------------------------------
 350  DO 360 I = 1,N
 360    SAVX(I) = H*SAVF(I) - (YH(I,2) + ACOR(I))
      odepack_DFNORM = odepack_DVNORM (N, SAVX, EWT)
      CALL odepack_DSOLPK (NEQ, Y, SAVF, SAVX, EWT, WM, IWM, F, PSOL)
      IF (IERSL .LT. 0) GO TO 430
      IF (IERSL .GT. 0) GO TO 410
      DEL = odepack_DVNORM (N, SAVX, EWT)
      IF (DEL .GT. 1.0D-8) STIFF = MAX(STIFF, odepack_DFNORM/DEL)
      DO 380 I = 1,N
        ACOR(I) = ACOR(I) + SAVX(I)
 380    Y(I) = YH(I,1) + EL(1)*ACOR(I)
C-----------------------------------------------------------------------
...
C----------------------- End of Subroutine DSTOKA ----------------------
      END
*DECK DSETPK
      SUBROUTINE odepack_DSETPK (NEQ, Y, YSV, EWT, FTEM, SAVF, JOK, WM,
     &  IWM,
     1                  F, JAC)
      EXTERNAL F, JAC
      INTEGER NEQ, JOK, IWM
...
C----------------------- End of Subroutine DSETPK ----------------------
      END
*DECK DSRCKR
      SUBROUTINE odepack_DSRCKR (RSAV, ISAV, JOB)
C-----------------------------------------------------------------------
C This routine saves or restores (depending on JOB) the contents of
...
C
      ...
      CALL odepack_DCOPY (LENRLS, RLS, 1, RSAV, 1)
      RSAV(LENRLS+1) = RLS2
      CALL odepack_DCOPY (LENRLR, RLSR, 1, RSAV(LENRLS+2), 1)
      CALL odepack_DCOPY (LENRLP, RLSP, 1, RSAV(LENRLS+LENRLR+2), 1)
      DO 20 I = 1,LENILS
 20     ISAV(I) = ILS(I)
...
C
 100  CONTINUE
      CALL odepack_DCOPY (LENRLS, RSAV, 1, RLS, 1)
      RLS2 = RSAV(LENRLS+1)
      CALL odepack_DCOPY (LENRLR, RSAV(LENRLS+2), 1, RLSR, 1)
      CALL odepack_DCOPY (LENRLP, RSAV(LENRLS+LENRLR+2), 1, RLSP, 1)
      DO 120 I = 1,LENILS
 120    ILS(I) = ISAV(I)
...
C----------------------- End of Subroutine DSRCKR ----------------------
      END
*DECK DAINVG
      SUBROUTINE odepack_DAINVG (RES, ADDA, NEQ, T, Y, YDOT, MITER,
     1                   ML, MU, PW, IPVT, IER )
      EXTERNAL RES, ADDA
...
C
      ...
      CALL odepack_DGEFA ( PW, NEQ, NEQ, IPVT, IER )
      IF (IER .EQ. 0) GO TO 20
         IER = -IER
         RETURN
   20 CALL odepack_DGESL ( PW, NEQ, NEQ, IPVT, YDOT, 0 )
      RETURN
C
C Band matrix case -----------------------------------------------------
C
...
C
      ...
      CALL odepack_DGBFA ( PW, NROWPW, NEQ, ML, MU, IPVT, IER )
      IF (IER .EQ. 0) GO TO 120
         IER = -IER
         RETURN
  120 CALL odepack_DGBSL ( PW, NROWPW, NEQ, ML, MU, IPVT, YDOT, 0 )
      RETURN
C----------------------- End of Subroutine DAINVG ----------------------
      END
*DECK DSTODI
      SUBROUTINE odepack_DSTODI (NEQ, Y, YH, NYH, YH1, EWT, SAVF, SAVR,
     ...
     2   R, RH, RHDN, RHSM, RHUP, TOLD, odepack_DVNORM
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
 270  CONTINUE
      ...
      IF (IERSL .GT. 0) GO TO 410
      EL1H = EL(1) * H
      DEL = odepack_DVNORM (N, SAVR, EWT) * ABS(H)
      DO 380 I = 1,N
...
C-----------------------------------------------------------------------
 460  JCUR = 0
      IF (M .EQ. 0) DSM = DEL/TESCO(2,NQ)
      IF (M .GT. 0) DSM = ABS(H) * odepack_DVNORM (N, ACOR,
     &  EWT)/TESCO(2,NQ)
      IF (DSM .GT. 1.0D0) GO TO 500
C-----------------------------------------------------------------------
C After a successful step, update the YH array.
...
C-----------------------------------------------------------------------
 520  RHUP = 0.0D0
      ...
      DO 530 I = 1,N
 530    SAVF(I) = ACOR(I) - YH(I,LMAX)
      DUP = ABS(H) * odepack_DVNORM (N, SAVF, EWT)/TESCO(3,NQ)
      EXUP = 1.0D0/(L+1)
      RHUP = 1.0D0/(1.4D0*DUP**EXUP + 0.0000014D0)
 540  EXSM = 1.0D0/L
      RHSM = 1.0D0/(1.2D0*DSM**EXSM + 0.0000012D0)
...
C----------------------- End of Subroutine DSTODI ----------------------
      END
*DECK DPREPJI
      SUBROUTINE odepack_odepack_odepack_DPREPJI (NEQ, Y, YH, NYH, EWT,
     &  RTEM, SAVR, S, WM, IWM,
     1   RES, JAC, ADDA)
...
C----------------------- End of Subroutine DPREPJI ---------------------
      END
*DECK DAIGBT
      SUBROUTINE odepack_DAIGBT (RES, ADDA, NEQ, T, Y, YDOT,
     1                   MB, NB, PW, IPVT, IER )
      EXTERNAL RES, ADDA
...
C-----------------------------------------------------------------------
 ...
 10     PW(I) = 0.0D0
      ...
      CALL odepack_DDECBT (MB, NB, PW, PW(LPB), PW(LPC), IPVT, IER)
      IF (IER .EQ. 0) GO TO 20
      IER = -IER
      RETURN
 20   CALL odepack_DSOLBT (MB, NB, PW, PW(LPB), PW(LPC), YDOT, IPVT)
      RETURN
C----------------------- End of Subroutine DAIGBT ----------------------
      END
*DECK DPJIBT
      SUBROUTINE odepack_DPJIBT (NEQ, Y, YH, NYH, EWT, RTEM, SAVR, S,
     &  WM, IWM,
     1   RES, JAC, ADDA)
      EXTERNAL RES, JAC, ADDA
      INTEGER NEQ, NYH, IWM
...
C Do LU decomposition on P. --------------------------------------------
      CALL odepack_DDECBT (MB, NB, WM(3), WM(LPB), WM(LPC), IWM(21),
     &  IER)
      IF (IER .NE. 0) IERPJ = 1
      RETURN
C Error return for IRES = 2 or IRES = 3 return from RES. ---------------
...
C----------------------- End of Subroutine DPJIBT ----------------------
      END
*DECK DSLSBT
      SUBROUTINE odepack_DSLSBT (WM, IWM, X, TEM)
      INTEGER IWM
...
C-----------------------------------------------------------------------
      ...
      LPB = 3 + LBLOX
      LPC = LPB + LBLOX
      CALL odepack_DSOLBT (MB, NB, WM(3), WM(LPB), WM(LPC), X, IWM(21))
      RETURN
C----------------------- End of Subroutine DSLSBT ----------------------
      END
*DECK DDECBT
      SUBROUTINE odepack_DDECBT (M, N, A, B, C, IP, IER)
...
C-----------------------------------------------------------------------
      ...
      DOUBLE PRECISION DP, odepack_DDOT
      IF (M .LT. 1 .OR. N .LT. 4) GO TO 210
      NM1 = N - 1
      NM2 = N - 2
C Process the first block-row. -----------------------------------------
      CALL odepack_DGEFA (A, M, M, IP, IER)
      K = 1
      IF (IER .NE. 0) GO TO 200
      DO 10 J = 1,M
        CALL odepack_DGESL (A, M, M, IP, B(1,J,1), 0)
        CALL odepack_DGESL (A, M, M, IP, C(1,J,1), 0)
 10     CONTINUE
C Adjust B(*,*,2). -----------------------------------------------------
      DO 40 J = 1,M
        DO 30 I = 1,M
          DP = odepack_DDOT (M, C(I,1,2), M, C(1,J,1), 1)
          B(I,J,2) = B(I,J,2) - DP
 30       CONTINUE
 40     CONTINUE
C Main loop.  Process block-rows 2 to N-1. -----------------------------
      DO 100 K = 2,NM1
        ...
        DO 70 J = 1,M
          DO 60 I = 1,M
            DP = odepack_DDOT (M, C(I,1,K), M, B(1,J,KM1), 1)
            A(I,J,K) = A(I,J,K) - DP
 60         CONTINUE
 70       CONTINUE
        CALL odepack_DGEFA (A(1,1,K), M, M, IP(1,K), IER)
        IF (IER .NE. 0) GO TO 200
        DO 80 J = 1,M
 80       CALL odepack_DGESL (A(1,1,K), M, M, IP(1,K), B(1,J,K), 0)
 100    CONTINUE
C Process last block-row and return. -----------------------------------
      DO 130 J = 1,M
        DO 120 I = 1,M
          DP = odepack_DDOT (M, B(I,1,N), M, B(1,J,NM2), 1)
          C(I,J,N) = C(I,J,N) - DP
 120      CONTINUE
 130    CONTINUE
      DO 160 J = 1,M
        DO 150 I = 1,M
          DP = odepack_DDOT (M, C(I,1,N), M, B(1,J,NM1), 1)
          A(I,J,N) = A(I,J,N) - DP
 150      CONTINUE
 160    CONTINUE
      CALL odepack_DGEFA (A(1,1,N), M, M, IP(1,N), IER)
      K = N
      IF (IER .NE. 0) GO TO 200
      RETURN
C Error returns. -------------------------------------------------------
...
C----------------------- End of Subroutine DDECBT ----------------------
      END
*DECK DSOLBT
      SUBROUTINE odepack_DSOLBT (M, N, A, B, C, Y, IP)
      INTEGER M, N, IP(M,N)
...
C Forward solution sweep. ----------------------------------------------
      CALL odepack_DGESL (A, M, M, IP, Y, 0)
      DO 30 K = 2,NM1
        KM1 = K - 1
        DO 20 I = 1,M
          DP = odepack_DDOT (M, C(I,1,K), M, Y(1,KM1), 1)
          Y(I,K) = Y(I,K) - DP
 20       CONTINUE
        CALL odepack_DGESL (A(1,1,K), M, M, IP(1,K), Y(1,K), 0)
 30     CONTINUE
      DO 50 I = 1,M
        DP = odepack_DDOT (M, C(I,1,N), M, Y(1,NM1), 1)
     1     + odepack_DDOT (M, B(I,1,N), M, Y(1,NM2), 1)
        Y(I,N) = Y(I,N) - DP
 50     CONTINUE
      CALL odepack_DGESL (A(1,1,N), M, M, IP(1,N), Y(1,N), 0)
C Backward solution sweep. ---------------------------------------------
      DO 80 KB = 1,NM1
        K = N - KB
        KP1 = K + 1
        DO 70 I = 1,M
          DP = odepack_DDOT (M, B(I,1,K), M, Y(1,KP1), 1)
          Y(I,K) = Y(I,K) - DP
 70       CONTINUE
 80     CONTINUE
      DO 100 I = 1,M
        DP = odepack_DDOT (M, C(I,1,1), M, Y(1,3), 1)
        Y(I,1) = Y(I,1) - DP
 100    CONTINUE
      RETURN
C----------------------- End of Subroutine DSOLBT ----------------------
      END
*DECK DIPREPI
      SUBROUTINE odepack_odepack_DIPREPI (NEQ, Y, S, RWORK, IA, JA, IC,
     &  JC, IPFLAG,
     1   RES, JAC, ADDA)
      EXTERNAL RES, JAC, ADDA
...
C Call DPREPI to do matrix preprocessing operations. -------------------
      CALL odepack_odepack_DPREPI (NEQ, Y, S, RWORK(LYH), RWORK(LSAVF),
     &  RWORK(LEWT),
...
C----------------------- End of Subroutine DIPREPI ---------------------
      END
*DECK DPREPI
      SUBROUTINE odepack_odepack_DPREPI (NEQ, Y, S, YH, SAVR, EWT,
     &  RTEM, IA, JA, IC, JC,
...
C----------------------- End of Subroutine DPREPI ----------------------
      END
*DECK DAINVGS
      SUBROUTINE odepack_odepack_DAINVGS (NEQ, T, Y, WK, IWK, TEM,
     &  YDOT, IER, RES, ADDA)
      EXTERNAL RES, ADDA
      INTEGER NEQ, IWK, IER
      INTEGER IPLOST, IESP, ISTATC, IYS, IBA, IBIAN, IBJAN, IBJGP,
...
C----------------------- End of Subroutine DAINVGS ---------------------
      END
*DECK DPRJIS
      SUBROUTINE odepack_DPRJIS (NEQ, Y, YH, NYH, EWT, RTEM, SAVR, S,
     &  WK, IWK,
     1   RES, JAC, ADDA)
      EXTERNAL RES, JAC, ADDA
      INTEGER NEQ, NYH, IWK
