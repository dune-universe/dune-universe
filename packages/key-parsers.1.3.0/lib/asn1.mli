...
module Ec : sig
  type point = Cstruct.t [@@deriving ord, eq, show]
  ...
  module Specified_domain : sig
    type field_element = Cstruct.t [@@deriving eq, ord, show]
...
module PKCS8 : sig
  ...
  val decode_dsa :
    Cstruct.t -> (Dsa.Params.t * Dsa.Private.t, string) Result.result

  val decode_ec :
    Cstruct.t -> (Ec.Params.t * Ec.Private.t, string) Result.result

  val decode_dh :
    Cstruct.t -> (Dh.Params.t * Dh.Private.t, string) Result.result
end

module Dsa_private_key : sig
  type t =
    { p : Derivable.Z.t
    ; q : Derivable.Z.t
    ; g : Derivable.Z.t
