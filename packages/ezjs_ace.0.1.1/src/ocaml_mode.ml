...
let token_type =
  ...
  (*| COMMENT_OPEN
    ...
    | COMMENT_CODE_CLOSE
  *)
  | COMMENT | OCAMLDOC_CODE | OCAMLDOC_VERB | COMMENTCONT ->
      "comment"
  | AND
  | AS
  | ASSERT
  ...
  | INFIXOP4 "asr" ->
      "keyword"
  | TRUE | FALSE | FLOAT _ | INT _ | INT32 _ | INT64 _ | NATIVEINT _ ->
      "constant"
  | INFIXOP0 _ | INFIXOP1 _ | INFIXOP2 _ | INFIXOP3 _ | INFIXOP4 _ | PREFIXOP _
    ->
      "function"
  | LABEL _ | OPTLABEL _ ->
      "type" (* Hack *)
...
type state = {block: IndentBlock.t; lex_ctxt: Nstream.snapshot}
...
let wrap_token state token =
  ...
  let type_ =
    if in_comment (fst state.lex_ctxt) then "comment"
    else token_type token.token
  in
  Ace.token ~type_ value

type config = {indent: IndentConfig.t; forced: bool}
...
let remove_phrases doc row =
  let rec remove row = function
    | [] ->
        []
    | pos' :: _ as phrases when Ace.greater_position row pos' ->
        phrases
    | _ :: phrases ->
        remove row phrases
  in
...
let get_line_tokens line st row doc =
  ...
  let rec iter ?(first = false) st offset stream tokens =
    ...
    | Some (tok, lex_ctxt, stream) -> (
        ...
        let tok, block, offset =
          ...
          then (tok, block, offset)
            ...
            Ace.replace doc
              ...
              spaces ;
            ({tok with between= spaces}, block, indent - old_indent)
        in
        ...
        if !debug_indent > 1 && tok.token <> EOL && tok.token <> ESCAPED_EOL
        then IndentBlock.dump block ;
...
let report_error editor ?(set_class = true) err warnings =
  ...
  | None, [] ->
      if set_class then Ace.add_class editor.ace "ocaml-check-success"
  | None, warnings ->
      if set_class then Ace.add_class editor.ace "ocaml-check-warn" ;
      List.iter (add_warning editor.ace) warnings
  | Some {locs; msg}, warnings -> (
      if set_class then Ace.add_class editor.ace "ocaml-check-error" ;
      List.iter (add_warning editor.ace) warnings ;
      match locs with
      | [] ->
...
let get_state editor row =
  ...
  if Js.to_string (Js.typeof s) = "string" then initial_state
  else (Obj.magic s : state)

let get_old_indent line =
  let rec loop line len i =
    if i < len && line.[i] = ' ' then loop line len (i + 1) else i
  in
...
let get_indent state line =
  ...
  | Some (token, stream) ->
      ...
      let block = IndentBlock.update !config.indent state.block stream token in
...
let do_indent ace_editor =
  ...
  if IndentBlock.is_in_comment state.block || not !config.forced then (
    if !debug_indent > 0 then
      debug "Tab-indent: line %d (%a)" row Approx_lexer.print_context
        (fst state.lex_ctxt) ;
    ...
    if old_indent <> indent && indent >= 0 then
      Ace.replace (Ace.document ace_editor)
        (Ace.create_range
           (Ace.create_position row 0)
           (Ace.create_position row old_indent))
        (String.make indent ' ') )
...
let rec all_trailing_spaces line i =
  if i <= 0 || line.[i] <> ' ' then i else all_trailing_spaces line (i - 1)

let remove_trailing_spaces line =
  let last = String.length line - 1 in
  let last_non_space = all_trailing_spaces line last in
  if last != last_non_space then String.sub line 0 (last_non_space + 1)
  else line

let may_reset_indent ace_editor =
  let _, (row, col) = Ace.read_range (Ace.get_selection_range ace_editor) in
  let line = Ace.get_line ace_editor row in
  if all_spaces line 0 (String.length line) then
    let state = get_state ace_editor (row - 1) in
    if not (IndentBlock.is_in_comment state.block) then
      let indent = IndentBlock.guess_indent state.block in
      let old_indent = get_old_indent line in
      Ace.replace (Ace.document ace_editor)
        (Ace.create_range
...
let do_delete ace_editor =
  if !config.forced then (
    ...
    if IndentBlock.is_in_comment state.block then Ace.remove ace_editor "left"
    else if
      not
        ( all_spaces line 0 (min col (String.length line))
        && all_spaces selected 0 (String.length selected) )
    then (
      ...
      if String.length prev_line = 0 then
        Ace.delete (Ace.document ace_editor)
          (Ace.create_range
        ...
        Ace.replace (Ace.document ace_editor)
          (Ace.create_range
             ...
             (Ace.create_position row2 old_indent))
          " " )
  else Ace.remove ace_editor "left"
...
let create_ocaml_editor div =
  ...
  Ace.record_event_handler editor.ace "change" (fun () ->
      Lwt.async (fun () -> reset_error editor)) ;
