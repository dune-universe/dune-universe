...
module Log = struct
  ...
  let info msg =
    let message = User_message.make msg in
    User_message.prerr message
end
  ...
  module Session = struct
    ...
    let close t =
      ...
      | Open { in_channel; out_channel; reader; writer; socket } ->
        Worker.stop reader;
        Worker.stop writer;
        (* with a socket, there's only one fd. We make sure to close it only
           once. with dune rpc init, we have two separate fd's (stdin/stdout) so
           we must close both. *)
    ...
    let write t sexps =
      ...
      | Open { writer; out_channel; socket; _ } -> (
        ...
        | None ->
          (if socket then
            try
              (* TODO this hack is temporary until we get rid of dune rpc
                 init *)
              Unix.shutdown
  ...
  module Server = struct
    module Transport = struct
      ...
      let create sockaddr ~backlog =
        ...
        let r_interrupt_accept, w_interrupt_accept =
