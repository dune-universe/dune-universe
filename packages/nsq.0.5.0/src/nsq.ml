...
module ServerMessage = struct
  ...
  let to_string = function
    ...
    | Heartbeat ->
      "Heartbeat"
    | ErrorInvalid s ->
      Printf.sprintf "ErrorInvalid: %s" s
    | ErrorBadTopic s ->
      Printf.sprintf "ErrorBadTopic: %s" s
    | ErrorBadChannel s ->
      Printf.sprintf "ErrorBadChannel: %s" s
    | ErrorFINFailed s ->
      Printf.sprintf "ErrorFINFailed: %s" s
    | ErrorREQFailed s ->
      Printf.sprintf "ErrorREQFailed: %s" s
    | ErrorTOUCHFailed s ->
      Printf.sprintf "ErrTouchFailed: %s" s
    | Message m ->
  ...
  let parse_response_body body =
    ...
    | "_heartbeat_" ->
      Ok Heartbeat
    | _ ->
      Error (Printf.sprintf "Unknown response: %s" body)
  ...
  let parse_error_body body =
    ...
    | [code; detail] -> (
        match code with
        | "E_INVALID" ->
          return (ErrorInvalid detail)
        | "E_BAD_TOPIC" ->
          return (ErrorBadTopic detail)
        | "E_BAD_CHANNEL" ->
          return (ErrorBadChannel detail)
        | "E_FIN_FAILED" ->
          return (ErrorFINFailed detail)
        | "E_REQ_FAILED" ->
          return (ErrorREQFailed detail)
        | "E_TOUCH_FAILED" ->
          return (ErrorTOUCHFailed detail)
        | _ ->
          fail (Printf.sprintf "Unknown error code: %s. %s" code detail) )
    | _ ->
  ...
  let of_raw_frame raw =
    ...
    | FrameResponse ->
      parse_response_body raw.data
    | FrameMessage ->
      parse_message_body raw.data
    | FrameError ->
      parse_error_body raw.data
    | FrameUnknown i ->
...
let query_nsqlookupd ~topic a =
  let host, port =
    match a with
    | Address.Host h ->
      (h, default_lookupd_port)
    | Address.HostPort (h, p) ->
      (h, p)
  in
...
let send_expect_ok ~read_timeout ~write_timeout ~conn cmd =
  send ~timeout:write_timeout ~conn cmd
  >>= fun () ->
  read_raw_frame ~timeout:read_timeout conn
  >>= fun raw ->
  ...
  | Error e ->
    fail_with (Printf.sprintf "Expected OK, got %s" e)
...
module Consumer = struct
  module Config = struct
    ...
    let validate c =
      ...
      >>= VInt.validate_between ~low:0 ~high:65535 c.max_attempts
        "max_attempts"
  ...
  let create ?(mode = `Nsqd) ?config addresses topic channel handler =
    let config =
      ...
      | None ->
        Config.create () |> Result.ok_or_failwith
      | Some c ->
        c
    in
  ...
  let handle_message handler msg max_attempts =
    ...
    >>= (function
        | Ok r ->
          return r
        | Error s ->
          Logs_lwt.err (fun l -> l "Handler error: %s" s)
          >>= fun () -> return `Requeue )
    >>= function
    | `Ok ->
      return (FIN msg.id)
  ...
  let handle_server_message server_message handler max_attempts =
    ...
    | ResponseOk ->
      return_none
    | Heartbeat ->
      Logs_lwt.debug (fun l -> l "Received heartbeat")
      >>= fun () -> return_some NOP
    | ErrorInvalid s ->
      warn_return_none "ErrorInvalid" s
    | ErrorBadTopic s ->
      warn_return_none "ErrorBadTopic" s
    | ErrorBadChannel s ->
      warn_return_none "ErrorBadChannel" s
    | ErrorFINFailed s ->
      warn_return_none "ErrorFINFailed" s
    | ErrorREQFailed s ->
      warn_return_none "ErrorREQFailed" s
    | ErrorTOUCHFailed s ->
      warn_return_none "ErrorTOUCHFailed" s
    | Message msg ->
  ...
  let rec read_loop ~timeout conn mbox =
    ...
    catch_result1 (read_raw_frame ~timeout) conn
    >>= function
    | Ok raw ->
  ...
  let maybe_open_breaker c conn mbox bs =
    ...
    if bs.error_count < c.config.error_threshold then return bs
      ...
      Logs_lwt.debug (fun l -> l "Breaker open, sending RDY 0")
      >>= fun () ->
      send ~timeout:c.config.write_timeout ~conn (RDY 0)
      >>= fun () ->
  ...
  let update_breaker_state c conn open_breaker cmd bs =
    match cmd with
    | NOP ->
      return bs (* Receiving a NOP should not alter our state  *)
    | _ -> (
        ...
        | false, HalfOpen ->
          ...
          >>= fun () ->
          send ~timeout:c.config.write_timeout ~conn (RDY rdy)
          >>= fun () -> return {position= Closed; error_count= 0} )

  let consume c conn mbox =
    let maybe_open_breaker = maybe_open_breaker c conn mbox in
    let update_breaker_state =
      update_breaker_state c conn maybe_open_breaker
    in
    ...
    let rec mbox_loop bs =
      ...
      | RawFrame raw -> (
          ...
          | Ok server_message ->
            async (fun () ->
                ...
                >>= function
                | Some c -> Lwt_mvar.put mbox (Command c) | None -> return_unit
              ) ;
            mbox_loop bs
          | Error s ->
            Logs_lwt.err (fun l ->
      ...
      | RecalcRDY -> (
          ...
          | Closed ->
            ...
            >>= fun () -> send (RDY rdy) >>= fun () -> mbox_loop bs )
    in
    send MAGIC
    >>= fun () ->
  ...
  let rec main_loop ?(error_count = 0) c address ready_calculator mbox =
    ...
    | Ok conn ->
      ...
      >>= fun () ->
      catch (fun () -> consume c conn mbox) handle_ex
      >>= fun () ->
    ...
    | Error e ->
      ...
      let stop_connecting =
        ...
        | `Nsqd ->
          false
        | `Lookupd ->
          error_count > lookupd_error_threshold
      in
      if stop_connecting then (
        ...
        >>= fun () ->
        Lwt_unix.sleep (Seconds.value duration)
        >>= fun () -> main_loop ~error_count c address ready_calculator mbox
  ...
  *)
  ...
  let run c =
    match c.mode with
    | `Lookupd ->
      Logs_lwt.debug (fun l -> l "Starting lookupd poll")
      >>= fun () -> start_polling_lookupd c c.addresses
    | `Nsqd ->
      let consumers =
  ...
  *)
  ...
  let create_pool address size =
    ...
    let dispose c =
      ...
      >>= fun () -> join [Lwt_io.close (fst c.conn); Lwt_io.close (snd c.conn)]
    in
    Lwt_pool.create size ~validate ~check ~dispose (fun () ->
        connect address default_dial_timeout
        >>= fun conn ->
        send ~timeout:default_write_timeout ~conn MAGIC
        >>= fun () ->
  ...
  let publish_cmd t cmd =
    let with_conn c =
      let rec read_until_ok () =
        read_raw_frame ~timeout:default_read_timeout c.conn
        >>= fun frame ->
        match ServerMessage.of_raw_frame frame with
        | Ok ResponseOk ->
          return_ok ()
        | Ok Heartbeat ->
          send ~timeout:default_write_timeout ~conn:c.conn NOP
          >>= fun () ->
          c.last_write := Unix.time () ;
          read_until_ok ()
        | Ok _ ->
          return_error "Expected OK or Heartbeat, got another message"
        | Error e ->
          return_error (Printf.sprintf "Received error: %s" e)
      in
      send ~timeout:default_write_timeout ~conn:c.conn cmd
      >>= fun () ->
      c.last_write := Unix.time () ;
      read_until_ok ()
    in
