...
let map input output =
  let decoder, close_input = match input with
    | `Std ->
      Pecu.decoder (`Channel stdin),
      (fun () -> ())
    | `File file ->
      let ic = open_in file in
      Pecu.decoder (`Channel ic),
      (fun () -> close_in ic) in
  let output_line, output_string, close_output, flush_output = match output with
    | `Std ->
    ...
    | `File file ->
      ...
      (fun () -> close_out oc),
      (fun () -> flush oc) in
  let rec go () = match Pecu.decode decoder, Pecu.decoder_dangerous decoder with
    | `Line line, dangerous ->
