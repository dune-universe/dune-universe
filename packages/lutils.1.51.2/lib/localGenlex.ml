...
(***********************************************************************)
(* modified be Erwan Jahier to use a custom int_of_string *)

let big_max_int = Big_int.big_int_of_nativeint Nativeint.max_int

(* The behavior of ocaml wrt non representable int is arguably
   arguable.  It also makes sense to reason modulo 2^(max_int-1)
   instead of raising a fatal error! Here I try to mimick the
   expected behavior as much as possible.

   Ideally, I'd like it to behave as the scanf("%d") in C (to behave
   as to ecexe). nb: the ocaml Scanf also raises an exception. Sigh.
*)
let local_int_of_string str = 
  try int_of_string str
  with _ ->
       if Str.string_match (Str.regexp "[a-z]") str 0
       then failwith ("Cannot convert "^str^" into an int")
       else
         let big_i0 = Big_int.big_int_of_string str in
         let big_i = Big_int.mod_big_int big_i0 (Big_int.succ_big_int big_max_int) in
         let i = Nativeint.to_int (Big_int.nativeint_of_big_int big_i) in
         if (Big_int.big_int_of_int i <> big_i0) then (
           Printf.eprintf  "Warning: integer overflow (%s truncated to %s)\n" 
                           (Big_int.string_of_big_int big_i0) (string_of_int i);
           flush stderr
         );
         i
...
let make_lexer keywords =
  ...
  and number (strm__ : _ Stream.t) =
    ...
    | _ -> Some (Int (local_int_of_string (get_string ())))
