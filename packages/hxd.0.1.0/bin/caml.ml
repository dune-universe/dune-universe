...
let ic =
  ...
  let parser x = match Sys.file_exists x with
...
let cols =
  ...
  let parser x = match int_of_string x with
    ...
    | exception _ ->
      Rresult.R.error_msgf "Invalid <cols> value: %S" x in
...
let number =
  ...
  let parser x = match int_of_string x with
    | n ->
      if n < 0
      then Rresult.R.error_msgf "Invalid <number> value (must be positive): %d" n
      else Rresult.R.ok n
    | exception _ ->
...
let groupsize =
  let doc = "Separate the output of every <bytes> bytes (two hex characters) by a whitespace. \
             Specify -g 0 to suppress grouping. <bytes> defaults to 2." in
  Arg.(value & opt (some number) None & info [ "g"; "groupsize" ] ~doc ~docv:"<bytes>")
...
let oc =
  Arg.(value & pos 1 oc `Stdout & info [] ~docv:"<outfile>")

let seek =
  let doc = "Start at <seek> bytes abs. (or rel.) infile offset. + indicates thatseek is relative \
             to the current stdin file position (meaningless when not reading from stdin). : indicates \
             that the seek should be that many characters from the end of the input (or if combined with +, \
             before the current stdin file position). Without -s option, xxd starts at the current file \
             position." in
...
let cmd =
  ...
  Term.info "xxd" ~version:"v0.1.0" ~doc ~man
