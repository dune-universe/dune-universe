...
let rec core ~loc ?opt ?assoc ?enum ?obj ?enc ?obj1 ?option c =
  let {co_assoc; co_enum; co_obj; co_enc; _} = core_attrs ?assoc ?enum ?obj ?enc c.ptyp_attributes in
  match co_enc with
  | Some e -> e
  | None ->
  ...
  if enum then
    match List.fold_left (fun acc r ->
        ...
        | Some acc, Rtag ({txt; _}, _, []) ->
          let key = match cs_key with Some k -> k | None -> String.uncapitalize_ascii txt in
...
and field ~loc ?attrs ~name ?option c =
  ...
  let {fa_field=(field, opt, dft); fa_key; fa_title; fa_description; fa_assoc;
       fa_enum; fa_exclude; fa_obj; fa_enc; fa_obj1; fa_merge} =
  ...
  | None ->
    ...
    if fa_merge then `Include (enc, true)
      ...
      let f = pexp_apply ~loc (evar ~loc (enc_mod field)) (
          title @ description @ [ Nolabel, estring ~loc fa_key; Nolabel, enc ] @ dft) in
      `Include (f, false)
  | Some e -> `Exclude e
  ...
  if enum then
    match List.fold_left (fun acc pcd ->
        ...
        | Some acc, Pcstr_tuple [] ->
          let key = match cs_key with Some k -> k | None -> String.uncapitalize_ascii pcd.pcd_name.txt in
          Some ((pexp_tuple ~loc [
              estring ~loc key;
