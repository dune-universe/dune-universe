...
module X : sig
  ...
  let src encoder source off len =
    ...
    else ( encoder.i <- source
         ; encoder.i_off <- off
         ; encoder.i_pos <- 0
         ; encoder.i_len <- len - 1)

  let refill k encoder = match encoder.src with
    | `Manual -> encoder.k <- k; `Await
    | `String _ -> end_of_input encoder; k encoder
    | `Channel ic ->
      let len = input ic encoder.i 0 (Bytes.length encoder.i) in
      (src encoder encoder.i 0 len; k encoder)

  let ret k encoder = encoder.k <- k; encoder.k encoder

  let r
    : type a. a kind -> a -> int -> int -> int -> Bytes.t -> a
    = fun kind t off pos rem src -> match kind with
  ...
  let rec encode encoder =
    ...
    if rem <= 0 then (if rem < 0 then `End encoder.value else refill encode encoder)
    else
      ( let value = r encoder.kind encoder.value encoder.i_off encoder.i_pos rem encoder.i in
        encoder.i_pos <- encoder.i_pos + rem
      ; encoder.value <- value
      ; ret encode encoder )

  let encoder : type a. a kind -> src -> a t = fun kind src ->
