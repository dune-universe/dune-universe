...
let to_list ?whitespace : t -> _ =
  List.map ~f:(fun (name, value) -> name, Value.of_string ?whitespace value)
;;
...
let add_if_missing ?whitespace t ~name ~value =
  if List.Assoc.mem t ~equal:Name.equal name then t else add ?whitespace t ~name ~value
;;
...
let add_all ?whitespace t ts : t =
  List.fold
    ~init:t
    ~f:(fun t (name, value) -> add ?whitespace t ~name ~value)
    (List.rev ts)
;;
...
let add_all_at_bottom ?whitespace t ts =
  List.fold
    ~init:t
    ~f:(fun t (name, value) -> add_at_bottom ?whitespace t ~name ~value)
    ts
;;

let filter ?whitespace t ~f =
  List.filter t ~f:(fun (name, value) ->
    f ~name ~value:(Value.of_string ?whitespace value))
;;
...
let%test_module _ =
  (module struct
    let t = of_list ~whitespace:`Raw [ "A", "a1"; "B", "b1"; "B", "b2" ]

    let%test_unit _ = [%test_result: string] (to_string t) ~expect:"A:a1\nB:b1\nB:b2\n"

    let%test_unit _ =
      [%test_result: string]
        (add ~whitespace:`Raw t ~name:"B" ~value:"b3" |> to_string)
        ~expect:"A:a1\nB:b3\nB:b1\nB:b2\n"
    ;;
    ...
    let%test_unit _ =
      [%test_result: string]
        (add ~whitespace:`Raw t ~name:"C" ~value:"c1" |> to_string)
        ~expect:"C:c1\nA:a1\nB:b1\nB:b2\n"
    ;;

    let%test_unit _ =
      [%test_result: string]
        (set ~whitespace:`Raw t ~name:"B" ~value:"b3" |> to_string)
        ~expect:"A:a1\nB:b3\nB:b2\n"
    ;;
    ...
    let%test_unit _ =
      [%test_result: string]
        (set ~whitespace:`Raw t ~name:"C" ~value:"c1" |> to_string)
        ~expect:"C:c1\nA:a1\nB:b1\nB:b2\n"
    ;;
    ...
    let%test_unit _ =
      [%test_result: string]
        (add_at_bottom ~whitespace:`Raw t ~name:"A" ~value:"a2" |> to_string)
        ~expect:"A:a1\nA:a2\nB:b1\nB:b2\n"
    ;;

    let%test_unit _ =
      [%test_result: string]
        (add_at_bottom ~whitespace:`Raw t ~name:"B" ~value:"b3" |> to_string)
        ~expect:"A:a1\nB:b1\nB:b2\nB:b3\n"
    ;;

    let%test_unit _ =
      [%test_result: string]
        (add_at_bottom ~whitespace:`Raw t ~name:"C" ~value:"c1" |> to_string)
        ~expect:"A:a1\nB:b1\nB:b2\nC:c1\n"
    ;;

    let%test_unit _ =
      [%test_result: string]
        (set_at_bottom ~whitespace:`Raw t ~name:"B" ~value:"b3" |> to_string)
        ~expect:"A:a1\nB:b1\nB:b3\n"
    ;;

    let%test_unit _ =
      [%test_result: string]
        (set_at_bottom ~whitespace:`Raw t ~name:"C" ~value:"c1" |> to_string)
        ~expect:"A:a1\nB:b1\nB:b2\nC:c1\n"
    ;;
  end)
;;
