...
module Checksum : sig
  ...
  let refill k encoder =
    ...
    | `Channel ic ->
        let len = input ic (Bytes.unsafe_of_string encoder.i) 0 (String.length encoder.i) in
  ...
  let encoder : type a. a kind -> default:a -> src -> a t =
   fun kind ~default src ->
    let i, i_off, i_pos, i_len =
      ...
      | `String s -> (s, 0, 0, String.length s - 1)
    in
    { src
    ; kind
    ; i
...
let kind_of_string x = match String.lowercase_ascii x with
...
let open_in filename =
  ...
  then let ic = open_in filename in Ok ic
  ...
  | [| _; kind; filename; |] ->
    let fiber () =
      ...
      ( match Checksum.encode encoder with
        | `Await -> assert false
        | `End v -> close_in ic ; Format.printf "%a\n%!" (pp k) v ; Ok () ) in
    ( match fiber () with
      | Ok () -> ()
