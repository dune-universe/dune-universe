...
let () =
  ...
  for i = 0 to 7 do arr.(i) <- (Char.code raw.[i] lsl 8) lor (Char.code raw.[i + 1]) done ;
...
let test_adler32 =
  Test.make_indexed
    ~name:"ADLER-32" ~fmt:"%s %d"
    ~args (digest_adler32 (Array.of_list inputs))

let test_crc32 =
  Test.make_indexed
    ~name:"CRC-32" ~fmt:"%s %d"
    ~args (digest_crc32 (Array.of_list inputs))

let test_crc32c =
  Test.make_indexed
    ~name:"CRC-32C" ~fmt:"%s %d"
    ~args (digest_crc32c (Array.of_list inputs))

let test_tcpip =
  Test.make_indexed
    ~name:"TCP/IP" ~fmt:"%s %d"
    ~args (digest_tcpip (Array.of_list inputs))

let test =
  Test.make_grouped ~name:"checksum" [ test_adler32
                                     ; test_tcpip
                                     ; test_crc32
                                     ; test_crc32c ]
...
let () =
  ...
  match emit ~dst:(Channel stdout) nothing ~compare ~x_label:Measure.run ~y_label:(Measure.label Instance.monotonic_clock) results with
  | Ok () -> ()
