...
let crc24 : type a.
  ...
  fun ~get buf off len crc ->
    let crc = ref crc in
    for i = 0 to len - 1 do
      crc := Optint.logxor !crc (Optint.of_int (Char.code (get buf (off + i)) lsl 16)) ;
      for _ = 0 to 7 do
...
let unsafe_digest_bigstring a o l v = crc24 ~get:Bigarray_compat.Array1.unsafe_get a o l v
