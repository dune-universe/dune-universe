...
module HO :
  ...
  struct
          ...
          (let rec maux e depth x =
             ...
             | AppUVar ({ contents = t }, vardepth, args) when t != C.dummy
                 ...
                 then deref_appuv ?avoid ~from:vardepth ~to_ args t
                   ...
                   maux empty_env depth
                     (deref_appuv ~from:vardepth ~to_:(from + depth) args t)
             ...
             | AppArg (i, args) when (e.(i)) != C.dummy ->
                 ...
                 deref_appuv ?avoid ~from:argsdepth ~to_:(to_ + depth) args
                   (e.(i))
             | Arg (i, args) ->
                 let to_ = min argsdepth to_ in
                 let r = oref C.dummy in
                 let v = UVar (r, to_, 0) in
                 (e.(i) <- v; if args == 0 then v else UVar (r, to_, args))
             | AppArg (i, args) when
                 List.for_all
                   (deterministic_restriction e ~args_safe:(argsdepth = to_))
                   args
                 ->
                 let to_ = min argsdepth to_ in
                 let args =
                    ...
                    then x
                      ...
                      (let (t, assignment) =
                         expand_uv ~depth:(from + depth) r ~lvl:vardepth
                           ~ano:argsno in
                       option_iter
                         (fun (r, _, assignment) -> (); r @:= assignment)
                         assignment;
                       maux e depth t))
                      ...
                      then
                        (let pruned = ref (vardepth > to_) in
                         ...
                         then
                           ...
                           let r' = oref C.dummy in
                           let newvar =
                             mkAppUVar r' vardepth' (!filteredargs_vardepth) in
           ...
           (match args with
            ...
            | ahd::atl ->
                (match t' with
                 ...
                 | Arg _|AppArg _ -> anomaly "beta takes only heap terms"
                 ...
                 | CData x ->
                     type_error (Printf.sprintf "beta: '%s'" (CData.show x))
    ...
    let is_llam lvl args adepth bdepth depth left e =
      let res = is_llam lvl args adepth bdepth depth left e in (); res
    ...
    let bind r gamma l a d delta b left t e =
      ...
      let cst ?hmove  c b delta = let n = cst ?hmove c b delta in (); n in
      let rec bind b delta w t =
        match t with
        | UVar (r1, _, _)|AppUVar (r1, _, _) when r == r1 ->
            raise RestrictionFailure
        | Const c ->
            let n = cst c b delta in if n < 0 then mkConst n else Const n
        ...
        | UVar ({ contents = t }, from, args) when t != C.dummy ->
            bind b delta w
              (deref_uv ~from ~to_:(((if left then b else a) + d) + w) args t)
        | AppUVar ({ contents = t }, from, args) when t != C.dummy ->
            bind b delta w
              (deref_appuv ~from ~to_:(((if left then b else a) + d) + w)
                 args t)
        | UVar _|AppUVar _|Arg _|AppArg _|Discard as orig ->
            let (r, lvl, (is_llam, args), orig_args) =
              ...
              | UVar (r, lvl, args) ->
                  ...
                  (r @:= (mknLam args v); (r', (lvl + args), (true, []), []))
              | AppUVar (r, lvl, orig_args) ->
                  (r, lvl, (is_llam lvl orig_args a b (d + w) left e),
                    orig_args)
              | Discard ->
                  let r = oref C.dummy in (r, ((a + d) + w), (true, []), [])
              | Arg (i, 0) ->
                  let r = oref C.dummy in
                  let v = UVar (r, a, 0) in
                  (e.(i) <- v; (r, a, (true, []), []))
               ...
               (let n_args = List.length args in
                ...
                then
                  let (args_gamma_lvl_abs, args_gamma_lvl_here) =
                    let mk_arg i =
                      ((mkConst i), (mkConst (cst ~hmove:false i b delta))) in
                       ...
                       (fun (c, c_p) ->
                          fun ((a_lvl, a_here) as acc) ->
                            try
                              ...
                              (((mkConst (c_p + lvl)) :: a_lvl), ((mkConst i)
                                :: a_here))
                            with | RestrictionFailure -> acc) args ([], []) in
                   if n_args = (List.length args_here)
                   then mkAppUVar r lvl (List.map (bind b delta w) orig_args)
                   else
                     (let r' = oref C.dummy in
                      let v = mkAppUVar r' lvl args_lvl in
                      r @:= (mknLam n_args v); mkAppUVar r' lvl args_here)))
             else mkAppUVar r lvl (List.map (bind b delta w) orig_args)) in
      try let v = mknLam new_lams (bind b delta 0 t) in (); r @:= v; true
      with | RestrictionFailure -> ((); false)
      ...
      ((delta = 0) && (a == b)) ||
        (match (a, b) with
                    ...
                    fun args ->
                      fun x ->
                        fun y ->
                          ...
                          if not b then (pruned := true; args) else x :: args)
                 [] xs ys in
             (if !pruned
              then
                (let len = List.length xs in
                 let r = oref C.dummy in
                 r1 @:=
                   (mknLam len (mkAppUVar r vd (List.rev filtered_args_rev))));
              true)
         ...
         | (AppUVar ({ contents = t }, from, args), _) when t != C.dummy ->
             unif matching depth adepth
               (deref_appuv ~from ~to_:(adepth + depth) args t) bdepth b e
         ...
         | (_, AppUVar ({ contents = t }, from, args)) when t != C.dummy ->
             unif matching depth adepth a bdepth
               (deref_appuv ~from ~to_:(bdepth + depth) args t) empty_env
         ...
         | (_, AppArg (i, args)) when (e.(i)) != C.dummy ->
             ...
             unif matching depth adepth a adepth
               (deref_appuv ~from:adepth ~to_:(adepth + depth) args (e.(i)))
               empty_env
         ...
         | (UVar (r, origdepth, 0), _) when not matching ->
             (try
                let t =
                  if depth = 0
                  then move ~avoid:r ~adepth ~from:bdepth ~to_:origdepth e b
                  else
                    (let b =
         ...
         | (_, Arg (i, args)) ->
             ...
             ((); e.(i) <- v; (); unif matching depth adepth a bdepth b e)
         | (UVar ({ rest = [] }, _, a1), UVar ({ rest = _::_ }, _, a2)) when
             (a1 + a2) > 0 -> unif matching depth bdepth b adepth a e
         | (AppUVar ({ rest = [] }, _, _), UVar ({ rest = _::_ }, _, a2))
             when a2 > 0 -> unif matching depth bdepth b adepth a e
         | (_, UVar (r, origdepth, args)) when
             ...
             ((); r @:= v; unif matching depth adepth a bdepth b e)
         ...
         | (other, AppUVar (r, lvl, args)) ->
             ...
             if is_llam
             then bind r lvl args adepth depth delta bdepth false other e
             else
               if !delay_hard_unif_problems
               then
                 (Fmt.fprintf Fmt.std_formatter
         ...
         | (App (c1, x2, xs), App (c2, y2, ys)) ->
             ((((delta = 0) || (c1 < bdepth)) && (c1 = c2)) ||
               ...
               ((((delta = 0) && (x2 == y2)) ||
                  ...
                  (for_all2
                     (fun x ->
                        fun y -> unif matching depth adepth x bdepth y e) xs
                     ys))
         | (Builtin (c1, xs), Builtin (c2, ys)) ->
             (c1 = c2) &&
               (for_all2
                  (fun x -> fun y -> unif matching depth adepth x bdepth y e)
                  xs ys)
         | (Lam t1, Lam t2) ->
    ...
    let unif ~matching  adepth e bdepth a b =
      let res = unif matching 0 adepth a bdepth b e in (); (); res
  ...
  struct
    ...
    let hash_arg_list goal hd ~depth  args mode spec =
      ...
      let hash size k =
        ...
        let kabs = Hashtbl.hash k in let h = kabs mod modulus in (); h in
      ...
      and aux_arg size matching deep arg =
        ...
        (); h in
      let h = aux 0 0 args mode spec in (); h
  ...
  struct
    ...
    let rec claux1 ?loc  get_mode vars depth hyps ts lts lcs t =
      ...
      | App (c, arg, []) when c == Global_symbols.pic ->
          let b = get_lambda_body ~depth:(depth + lts) arg in
          claux1 ?loc get_mode (vars + 1) depth hyps ((Arg (vars, 0)) :: ts)
            (lts + 1) lcs b
            ...
            let open List in
              flatten
                (rev_map
                   (fun (ts, g) ->
                      let g =
                        hmove ~from:(depth + lts) ~to_:((depth + lts) + lcs)
                          g in
      ...
      | Lam _|CData _ as x ->
          error ?loc
            ("Assuming a string or int or float or function:" ^ (show_term x))
...
module Constraints :
  ...
  struct
    ...
    let exect_builtin_predicate c ~depth  idx args =
      ...
      then (declare_constraint ~depth idx args; [])
      else
               ...
               exec
                 (fun m ->
                    ...
                    (let m =
                     ...
                     (); T.to_resume := []; assert ((!T.new_delayed) = []); m))
                 Ice.empty_freezer in
             ();
             check_guard ();
               ...
               | App (c, g2, g1::[]) when c == Global_symbols.rimplc ->
                   (();
                    (let (clauses, pdiff, lcs) = clausify p ~depth g1 in
                     ...
                     run (depth + lcs) (add_clauses ~depth clauses pdiff p)
                       g2 gs next alts lvl))
               | App (c, g1, g2::[]) when c == Global_symbols.implc ->
                   (();
                    (let (clauses, pdiff, lcs) = clausify p ~depth g1 in
               ...
               | App (c, arg, []) when c == Global_symbols.sigmac ->
                   (();
                    (let f = get_lambda_body ~depth arg in
                     ...
                     run depth p fv gs next alts lvl))
               | UVar ({ contents = g }, from, args) when g != C.dummy ->
                   (();
               ...
               | Const k ->
                   (();
                    (let clauses = get_clauses depth k g p in
                     ();
                     backchain depth p (k, C.dummy, [], gs) next alts lvl
                       clauses))
               | App (k, x, xs) ->
                   (();
                    (let clauses = get_clauses depth k g p in
                     ();
                     backchain depth p (k, x, xs, gs) next alts lvl clauses))
               | Builtin (c, args) ->
                   (();
            ...
            (();
             (let old_trail = !T.trail in
              ...
              (let env = Array.make c_vars C.dummy in
               ...
               | false ->
                   (T.undo old_trail ();
                    backchain depth p (k, arg, args_of_g, gs) next alts lvl
                      cs)
               | true ->
                   let oldalts = alts in
                   let alts =
                            ...
                            (fun x ->
                               ((make_subgoal)[@inlined ]) ~depth p
                                 (move ~adepth:depth ~from:c_depth ~to_:depth
                                    env x)) hs in
                        run depth p h hs next alts oldalts))))
