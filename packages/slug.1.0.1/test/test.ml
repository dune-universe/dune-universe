...
let no_duplicated_seps () = expect_string (slugify "foo , bar") "foo-bar"

let remove_non_alpha_num () =
  expect_string (slugify "foo] - -- [ bar") "foo-bar"

let retain_upper_cases () = expect_string (slugify "foo , bar") "foo-bar"
...
let cyrillic_issue_2 () =
  expect_string
    (slugify
       ~charmap:(Charmap.mk_charmap [ Slug_data.base; Slug_data.uk ])
       "йодистый калий")
    "yodystyy-kalyy"
    ...
    [
      ( "slugify",
        [
          ...
          test_case "remote non alphabets and numbers characters" `Quick
            remove_non_alpha_num;
          test_case "retain word cases" `Quick retain_upper_cases;
          test_case "custom map (locale)" `Quick locale;
          test_case "cyrillic issue #2" `Quick cyrillic_issue_2;
        ] );
    ]
