...
(* [print_mem o bag] produces code for an OCaml expression of type [bool]
   which expresses the fact that the variable [o] is equal to one of the
   elements of the bag [bag]. E.g., if the bag contains the elements 1 and 2,
   then [print_mem "observed" bag] produces the string ["observed = 1 ||
   observed = 2"]. Note that it would also be fine to produce the string
   ["List.mem observed [1;2]"]. *)

let print_mem (o : document) (xs : int list) : document =
  flow_map
    (break 1 ^^ string "||" ^^ space)
    (fun x -> o ^^ utf8format " = %d" x)
    xs

let print_mem o bag =
  print_mem o !bag

let extract (bag : 'a t) (ox : 'a option) : diagnostic =
