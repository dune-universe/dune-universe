  ...
  | None ->
    ...
    | None, Some b ->
      ...
      Delim
        ...
        ; delim_patterns =
            begin match List.assoc_opt "patterns" obj with
              | None -> []
              | Some v -> get_pattern_list v
            end
        ...
        ; delim_apply_end_pattern_last =
            begin match List.assoc_opt "applyEndPatternLast" obj with
              | Some (`Int 1) -> true
              | _ -> false
            end
        ; delim_kind }
...
let of_doc_exn (plist : union) =
  let rec get_repo_item obj =
    { repo_item_kind =
        begin match List.assoc_opt "match" obj, List.assoc_opt "begin" obj with
          | None, None -> Repo_patterns (get_patterns obj)
          | _, _ -> Repo_rule (patterns_of_plist obj)
        end
    ; repo_inner =
        begin match List.assoc_opt "repository" obj with
