...
let () =
  ...
  let spec = rot2 (map ^>> fun m -> key m ^> bool) in
  ...
  let spec = rot2 (map ^>> fun m -> key m ^> map) in
  ...
  (* TODO difficulty with [compare]: the candidate implementation is
     free to implement an *arbitrary* total order. A reference
     implementation is useless here. We should observe the candidate
     and test whether it contradicts itself. *)
  (*
  let spec = map ^> map ^> int in
  declare "compare Stdlib.compare" spec (R.compare compare) (C.compare compare);
   *)
  ...
  (* TODO
     iter (* possible difficulty with ordering *)
     fold
     exists
     filter
     filter_map *)
  ...
  (* TODO bindings *) (* again a difficulty with ordering *)
  ...
  (* [choose] is even more interesting: its specification is nondeterministic
     and it can raise [Not_found]. *)
  ...
  let spec = rot2 (map ^>> fun m -> key m ^> option value) in
