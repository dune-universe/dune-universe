...
= fun env rv cv spec ->
  ...
  | SpecNondet spec, _, _ ->
      ...
      | exception _e ->
          ...
          (* TODO we want to classify this as a [ReferenceFailure e],
             but the architecture of our code does not allow it. *)
          assert false
      end
...
let rec main_random source prologue fuel =
  ...
  with Abort (scenario, fuel) ->
    ...
    let command = sprintf "mktemp ./output/crashes/scenario.%03d.XXXX" fuel in
