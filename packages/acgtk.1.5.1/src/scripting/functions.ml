...
module Functions =
  struct
    ...
    let make_context ~wait ~colored_output ~pretty_printed_output ~echo ~svg ~dirs ~rendering_config ~parse_fun =
      {
        wait;
        colored_output;
	...
	(fun (first,last_abs_sg) entry -> 
	  ...
	  | Env.Signature sg -> 
		...
		if first then
		  ...
		  Utils.sformat "@[@["
	      in
	      (match Data_parser.parse_term ~output:true ~color:ctx.colored_output data sg with
	  ...
	  | Env.Lexicon lex -> 
		...
		if first then
		  ...
		  Utils.sformat "@[@["
	      in
	      match Data_parser.parse_term ~output:first ~color:ctx.colored_output data abs with
	      | None -> false,Some abs
	      | Some (t,ty) -> 
		let () = Utils.sformat "@]@]@.@." in
		let s = Format.flush_str_formatter () in
  ...
  let check ctx ?names e data l =
	...
	let _ = Data_parser.parse_term ~output:true ~color:ctx.colored_output data sg in
	let () = Utils.sformat "@]@]@." in
	let s = Format.flush_str_formatter () in
	Utils.format "%s" s)
      signatures in
  ...
  let realize_show ?names ?svg_output ctx e data l =
    ...
    match Data_parser.parse_term ~color:ctx.colored_output data abs with
    | None -> ()
