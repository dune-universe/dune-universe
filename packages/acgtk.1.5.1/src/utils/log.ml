...
let my_reporter ~app ppf = 
  let new_report src level ~over k msgf =
    let k _ = over (); k () in
    match level with
    | Logs.App -> msgf @@ fun ?header ?tags fmt -> Fmt.kpf k ppf ("%a@[" ^^ fmt ^^ "@]@.") Logs_fmt.pp_header (level, header)
    | Logs.Error -> msgf @@ fun ?header ?tags fmt -> Fmt.kpf k ppf ("%a @[" ^^ fmt ^^ "@]@.") Logs_fmt.pp_header (level, header)
    | _ -> 
       ...
       let with_src h tags k ppf fmt =
         ...
         | Some s ->  
            Fmt.kpf k ppf ("%a%a: %a[%0+04.0fus] @[" ^^ fmt ^^ "@]@.")  Fmt.(styled `Magenta string) p1 Fmt.(styled `Magenta string) p2 Logs_fmt.pp_header (level, h) (Mtime.Span.to_us s)
       in
...
let set_level ~app ?(colored=true) l =
  let tt =
    match colored with
    | true -> Fmt. (Some `Ansi_tty)
    | false -> Fmt. (Some `None)in
