...
module Run_on_update_handlers = struct
  ...
  type t = T : 'a Node.t * 'a Node_update.t -> t
  [@@deriving sexp_of]
...
type t =
  ...
  (* We use [timing_wheel] for time-based incrementals.  [now] is a variable holding the
     ...
     because one is not allowed to call [Timing_wheel_ns] functions from the [handle_fired]
     that one passes to [Timing_wheel_ns.advance_clock]. *)
...
let directly_observed t =
  ...
  iter_observers t ~f:(fun (Internal_observer.Packed.T internal_observer) ->
    r := Node.pack internal_observer.observing :: !r);
  !r
;;

let save_dot t file =
  Node.Packed.save_dot file (directly_observed t);
;;
...
let stats t =
  ...
  iter_observer_descendants t ~f:(fun node ->
...
let am_stabilizing t =
  ...
  | Stabilize_previously_raised raised_exn ->
    failwiths "cannot call am_stabilizing -- stabilize previously raised" raised_exn
...
let invariant t =
  ...
  | Running_on_update_handlers | Stabilizing | Not_stabilizing ->
    Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
      ...
      iter_observers t ~f:(fun (Internal_observer.Packed.T internal_observer) ->
        begin match internal_observer.state with
        | In_use | Disallowed -> ()
        | Created | Unlinked ->
          failwiths "member of all_observers with unexpected state" internal_observer
      ...
      iter_observer_descendants t ~f:(fun node ->
        ...
        assert (node.height
                <= Adjust_heights_heap.max_height_seen t.adjust_heights_heap));
      assert (Adjust_heights_heap.max_height_allowed t.adjust_heights_heap
      ...
      Fields.iter
        ...
        ~current_scope:(check (fun current_scope ->
          assert (phys_equal current_scope Scope.top)))
        ~recompute_heap:(check Recompute_heap.invariant)
        ~adjust_heights_heap:(check (fun adjust_heights_heap ->
          assert (Adjust_heights_heap.length adjust_heights_heap = 0);
          Adjust_heights_heap.invariant adjust_heights_heap))
        ~propagate_invalidity:(check (fun propagate_invalidity ->
          assert (Stack.is_empty propagate_invalidity)))
        ~num_active_observers:(check (fun num_active_observers ->
          assert (num_active_observers >= 0)))
        ~all_observers:ignore
        ~finalized_observers:ignore
        ~new_observers:
          (check (Stack.invariant (fun packed ->
        ...
        ~set_during_stabilization:(check (fun set_during_stabilization ->
          match t.status with
...
let ensure_not_stabilizing t ~name ~allow_in_update_handler =
  ...
  | Not_stabilizing -> ()
  | Running_on_update_handlers ->
    if not allow_in_update_handler then begin
      let backtrace = Backtrace.get () in
      failwiths (sprintf "cannot %s during on-update handlers" name) backtrace
        [%sexp_of: Backtrace.t];
    end
  | Stabilize_previously_raised raised_exn ->
    let backtrace = Backtrace.get () in
    failwiths (sprintf "cannot %s -- stabilize previously raised" name)
      (raised_exn, backtrace) [%sexp_of: Raised_exn.t * Backtrace.t]
  | Stabilizing ->
    let backtrace = Backtrace.get () in
    failwiths (sprintf "cannot %s during stabilization" name) backtrace
      [%sexp_of: Backtrace.t]
;;
...
let handle_after_stabilization : type a . t -> a Node.t -> unit = fun t node ->
  if verbose then Debug.ams [%here] "handle_after_stabilization" node [%sexp_of: _ Node.t];
  if not node.is_in_handle_after_stabilization then begin
    node.is_in_handle_after_stabilization <- true;
    Stack.push t.handle_after_stabilization (Node.pack node);
  end;
;;
...
and became_unnecessary : type a. t -> a Node.t -> unit = fun t node ->
...
let rec invalidate_node : type a . t -> a Node.t -> unit = fun t node ->
  if verbose then Debug.ams [%here] "invalidate_node" node [%sexp_of: _ Node.t];
  if Node.is_valid node then begin
    if node.num_on_update_handlers > 0 then handle_after_stabilization t node;
...
let rescope_nodes_created_on_rhs _t (first_node_on_rhs : _ Node.t Uopt.t) ~new_scope =
...
let propagate_invalidity t =
  ...
  while not (Stack.is_empty t.propagate_invalidity) do
    let node = Stack.pop_exn t.propagate_invalidity in
    if verbose
...
let rec add_parent_without_adjusting_heights
  ...
  fun t ~child ~parent ~child_index ->
    if verbose then
      Debug.ams [%here] "add_parent_without_adjusting_heights"
...
and became_necessary : type a. t -> a Node.t -> unit = fun t node ->
  ...
  *)
  ...
  Node.iteri_children node ~f:(fun child_index child ->
...
let add_parent t ~child ~parent ~child_index =
  ...
  then Adjust_heights_heap.adjust_heights t.adjust_heights_heap t.recompute_heap
         ~child ~parent;
  propagate_invalidity t;
  if debug then assert (Node.is_necessary parent); (* we only add necessary parents *)
...
let run_with_scope t scope ~f =
  ...
  try
    let v = f () in t.current_scope <- saved; v
  with exn ->
...
let change_child
  ...
  fun t ~parent ~old_child ~new_child ~child_index ->
    ...
    then add_parent t ~child:new_child ~parent ~child_index
      ...
      if not (phys_equal old_child new_child) then begin
...
let rec recompute : type a. t -> a Node.t -> unit = fun t (node : a Node.t) ->
  ...
  | Bind_lhs_change ({ main
    ...
    if Uopt.is_some old_rhs then begin
      ...
      else rescope_nodes_created_on_rhs t old_all_nodes_created_on_rhs
  ...
  | Freeze { child; only_freeze_when; _ } ->
    ...
    if only_freeze_when value then begin
      remove_children t node;
      Node.set_kind node (Const value);
      if Node.is_necessary node
      then set_height t node 0
      else became_unnecessary t node;
    end;
    maybe_change_value t node value;
  | If_test_change ({ main; current_branch; test; then_; else_; _ }
                    as if_then_else) ->
  ...
  | Map5 (f, n1, n2, n3, n4, n5) ->
    maybe_change_value t node
      (f (Node.value_exn n1) (Node.value_exn n2) (Node.value_exn n3)
         (Node.value_exn n4) (Node.value_exn n5))
  ...
  | Map8 (f, n1, n2, n3, n4, n5, n6, n7, n8) ->
    maybe_change_value t node
      (f (Node.value_exn n1) (Node.value_exn n2) (Node.value_exn n3)
         (Node.value_exn n4) (Node.value_exn n5) (Node.value_exn n6)
         (Node.value_exn n7) (Node.value_exn n8))
...
and maybe_change_value : type a. t -> a Node.t -> a -> unit =
  fun t (node : _ Node.t) new_value ->
    ...
    || not (Cutoff.should_cutoff node.cutoff
              ~old_value:(Uopt.unsafe_value old_value_opt) ~new_value)
    then begin
      ...
      if node.num_on_update_handlers > 0 then begin
        node.old_value_opt <- old_value_opt;
        handle_after_stabilization t node;
      end;
      if node.num_parents >= 1 then begin
        for parent_index = 1 to node.num_parents - 1 do
          let parent =
            Uopt.value_exn (Array.get node.parent1_and_beyond
                              (parent_index - 1))
          in
          begin match parent.kind with
          | Expert expert ->
          ...
          | Unordered_array_fold u ->
            Unordered_array_fold.child_changed
              (* This [Obj.magic] is OK because we maintain the invariant that if
                 [child : a Node.t] is a child of [parent : b Node.t] and [parent.kind]
                 is [Unordered_array_fold u], then [u : (a, b) Unordered_array_fold.t]. *)
              (Obj.magic (u : (_, _) Unordered_array_fold.t)
               : (a, _) Unordered_array_fold.t)
              ~old_value_opt ~new_value
          | _ -> ()
          end;
        ...
        if debug then assert (Node.needs_to_be_computed parent);
        if not (Node.is_in_recompute_heap parent) then begin
          ...
          then begin
            ...
            (* If [parent.height] is [<=] the height of all nodes in the recompute heap
               ...
               recompute heap. *)
            if parent.height <= Recompute_heap.min_height t.recompute_heap
            then begin
...
let recompute_everything_that_is_necessary t =
  ...
  while R.length r > 0 do
    let node = R.remove_min r in
    if debug && not (Node.needs_to_be_computed node)
    then failwiths "node unexpectedly does not need to be computed" node
           [%sexp_of: _ Node.t];
    recompute t node;
  done;
  if debug then begin
...
let unlink_disallowed_observers t =
  while Stack.length t.disallowed_observers > 0 do
    ...
    if debug
    then assert (match internal_observer.state with Disallowed -> true | _ -> false);
...
let disallow_future_use t (internal_observer : _ Internal_observer.t) =
  if verbose
  then Debug.ams [%here] "disallow_future_use" internal_observer
         [%sexp_of: _ Internal_observer.t];
  match internal_observer.state with
  | Disallowed | Unlinked -> ()
  | Created ->
...
let observer_finalizer t =
  stage (fun observer ->
    ...
    if verbose then Debug.ams [%here] "enqueueing finalizer" internal_observer
                      [%sexp_of: _ Internal_observer.t];
...
let create_observer ?(should_finalize = true) t observing =
  if verbose
  then Debug.ams [%here] "create_observer" observing [%sexp_of: _ Node.t];
...
let add_new_observers t =
  while Stack.length t.new_observers > 0 do
    ...
    | Created ->
      ...
      if Uopt.is_some old_all_observers then begin
      ...
      if Uopt.is_some old_observers then begin
...
let observer_value_exn t observer =
  ...
  | Stabilize_previously_raised raised_exn ->
    failwiths "Observer.value_exn called after stabilize previously raised"
      raised_exn [%sexp_of: Raised_exn.t]
  | Stabilizing ->
    failwiths "Observer.value_exn called during stabilization" observer
      [%sexp_of: _ Observer.t];
;;

let observer_value t observer =
  try Ok (observer_value_exn t observer) with exn -> Error (Error.of_exn exn)
;;
...
let set_var_while_not_stabilizing t (var : _ Var.t) value =
  ...
  var.value <- value;
  if Stabilization_num.compare var.set_at t.stabilization_num < 0 then begin
...
let set_var t var value =
  ...
  | Stabilize_previously_raised raised_exn ->
    failwiths "cannot set var -- stabilization previously raised" raised_exn
      [%sexp_of: Raised_exn.t]
  | Stabilizing ->
    if Uopt.is_none var.value_set_during_stabilization
    then Stack.push t.set_during_stabilization (Var.pack var);
...
let stabilize t =
  ...
  try
    ...
    (* We increment [t.stabilization_num] before handling variables set during
       stabilization, so that they are treated as set during the new stabilization
       cycle.  Also, we increment before running on-update handlers, to avoid running
       on update handlers created during on update handlers. *)
    t.stabilization_num <- Stabilization_num.add1 t.stabilization_num;
    if verbose
    then Debug.ams [%here] "handling set_during_stabilization" t [%sexp_of: t];
    while not (Stack.is_empty t.set_during_stabilization) do
      let var = Stack.pop_exn t.set_during_stabilization in
    ...
    while not (Stack.is_empty t.handle_after_stabilization) do
      let node = Stack.pop_exn t.handle_after_stabilization in
...
(* A [const] value could come from the right-hand side of an outer bind.  So, we create
   a [const] node in the current scope, not in [Scope.top]. *)
...
let preserve_cutoff ~(input : _ Node.t) ~output =
  Node.set_cutoff output (Cutoff.create (fun ~old_value:_ ~new_value:_ ->
...
let necessary_if_alive t input =
  ...
  let output = map t input ~f:(fun a -> Gc.keep_alive observer; a) in
  preserve_cutoff ~input ~output;
  output
;;
...
let unordered_array_fold ?(full_compute_every_n_changes = Int.max_value)
      t children ~init ~f ~f_inverse =
  if verbose
  then Debug.ams [%here] "unordered_array_fold" children [%sexp_of: _ Node.t array];
  if Array.length children = 0
  then const t init
  else if full_compute_every_n_changes <= 0
  then failwiths "unordered_array_fold got non-positive full_compute_every_n_changes"
         full_compute_every_n_changes [%sexp_of: int]
  else
    let main = create_node t Uninitialized in
    Node.set_kind main
...
let opt_unordered_array_fold ?full_compute_every_n_changes t ts ~init ~f ~f_inverse =
  ...
  in
  map t
    (unordered_array_fold t ts ~init:(init, 0) ~f ~f_inverse ?full_compute_every_n_changes)
...
let sum ?full_compute_every_n_changes t nodes ~zero ~add ~sub =
  unordered_array_fold t nodes ~init:zero ~f:add ~f_inverse:sub
    ?full_compute_every_n_changes
;;

let opt_sum ?full_compute_every_n_changes t nodes ~zero ~add ~sub =
  opt_unordered_array_fold t nodes ~init:zero ~f:add ~f_inverse:sub
    ?full_compute_every_n_changes
;;
...
let next_interval_alarm t ~base ~interval =
  let after =
    Timing_wheel_ns.interval_num_start t.timing_wheel
      (Timing_wheel_ns.now_interval_num t.timing_wheel)
  in
...
let at_intervals t interval =
  ...
  at_intervals.alarm <-
    add_alarm t ~at:(next_interval_alarm t ~base ~interval)
...
let step_function t ~init steps =
  if not (List.is_sorted steps ~compare:(fun (time, _) (time', _) ->
    Time_ns.compare time time'))
  then failwiths "step_function got unsorted times" steps [%sexp_of: (Time_ns.t * _) list];
  let main = create_node t Uninitialized in
  let step_function =
    { Step_function.
      main
  ...
  advance_step_function t main step_function
...
let change_leaf t (node : _ Node.t) =
  ...
  node.recomputed_at <- Stabilization_num.none; (* force the node to be stale *)
...
let advance_clock t ~to_ =
  ...
  while Uopt.is_some t.fired_alarm_values do
    ...
    let module A = Alarm_value.Action in
