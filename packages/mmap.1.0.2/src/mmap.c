...
/*   special exception on linking described in the file LICENSE.ocaml.    */
/*                                                                        */
/**************************************************************************/
...
CAMLprim value caml_mmap_map_file(value vfd, value vkind, value vlayout,
                                 value vshared, value vdim, value vstart)
{
  ...
  if (num_dims < 1 || num_dims > CAML_BA_MAX_NUM_DIMS)
    caml_invalid_argument("Mmap.V1.map_file" ": bad number of dimensions");
  for (i = 0; i < num_dims; i++) {
    dim[i] = Long_val(Field(vdim, i));
    if (dim[i] == -1 && i == major_dim) continue;
    if (dim[i] < 0)
      caml_invalid_argument("Mmap.V1.map_file" ": negative dimension");
  }
  ...
  if (dim[major_dim] == -1) {
    ...
    if (file_size < startpos) {
      caml_leave_blocking_section();
      caml_failwith("Mmap.V1.map_file" ": file position exceeds file size");
    }
    ...
    if (array_size != data_size) {
      caml_leave_blocking_section();
      caml_failwith("Mmap.V1.map_file" ": file size doesn't match array dimensions");
    }
  } else {
  ...
  return caml_mmap_mapped_alloc(flags, num_dims, addr, dim);
}
...
CAMLprim value caml_mmap_map_file_bytecode(value * argv, int argn)
{
  return caml_mmap_map_file(argv[0], argv[1], argv[2],
                            argv[3], argv[4], argv[5]);
}

void caml_mmap_unmap_file(void * addr, uintnat len)
{
#if defined(HAS_MMAP)
