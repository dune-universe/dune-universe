...
let commit ?(force = false) commit_file open_file =
  ...
  if (not force) && Sys.file_exists commit_file
  then cant_overwrite commit_file
  else if (not force) && Sys.file_exists open_file
  then cant_overwrite open_file
  else
    let c, o = Nocoiner.commit secret in
    write_to c commit_file ;
    write_to o open_file
