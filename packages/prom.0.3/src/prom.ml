...
module Descr = struct
  module T = struct
    type t = { name : string; help : string option }
...
let pp_labels ppf = function
  ...
  | labels ->
      Fmt.pf ppf "{";
      Fmt.list ~sep:(fun ppf () -> Fmt.string ppf ",") pp_label ppf labels;
      Fmt.pf ppf "}"
...
let pp_summary_line name labels ts ppf (le, v) =
  let labels =
    List.rev (("quantile", Fmt.str "%a" pp_float le) :: List.rev labels)
  in
...
let pp_complex_summary name labels ppf { ts; v = { data; _ } as cplx } =
  Fmt.list ~sep:Format.pp_print_newline
    (pp_summary_line name labels ts)
    ppf (FMap.bindings data);
  Format.pp_print_newline ppf ();
  pp_sum_count name labels ppf cplx

let pp_typ : type a. Format.formatter -> a typ -> unit =
 fun ppf -> function
...
let pp_metric name ppf (labels, { ts; v }) =
  Fmt.pf ppf "%s%a %f %a" name pp_labels (SMap.bindings labels) v
    (Fmt.option pp_ts) ts
...
let pp_summary name ppf (labels, t) =
  let aux { sum; count; data = kll, pct } =
    ...
    let quantiles =
      List.filter_map
        (fun (p, v) -> Option.map (fun (e, _) -> (p, e)) v)
        quantiles
    in
    complex_cum count sum quantiles
  in
...
let pp ppf { descr; series = Metric (typ, data) } =
  ...
  match typ with
  | Counter ->
      Fmt.list ~sep:Format.pp_print_newline (pp_metric descr.name) ppf
        (LabelsMap.bindings data)
  | Gauge ->
      Fmt.list ~sep:Format.pp_print_newline (pp_metric descr.name) ppf
        (LabelsMap.bindings data)
  | Histogram ->
      Fmt.list ~sep:Format.pp_print_newline (pp_histogram descr.name) ppf
        (LabelsMap.bindings data)
  | Summary ->
      Fmt.list ~sep:Format.pp_print_newline (pp_summary descr.name) ppf
        (LabelsMap.bindings data)

let pp_list ts = Fmt.list ~sep:Format.pp_print_newline pp ts
