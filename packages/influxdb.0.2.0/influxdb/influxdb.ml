...
module TimestampNS = struct
  ...
  let%expect_test "to_string_precision_nanoseconds" = 
    Stdio.print_endline (to_string_precision Precision.Nanosecond 1529349109966270847L);
    [%expect{| 1529349109966270847 |}]

  let%expect_test "to_string_precision_microsecond" = 
    Stdio.print_endline (to_string_precision Precision.Microsecond 1529349109966270847L);
    [%expect{| 1529349109966270 |}]

  let%expect_test "to_string_precision_millisecond" = 
    Stdio.print_endline (to_string_precision Precision.Millisecond 1529349109966270847L);
    [%expect{| 1529349109966 |}]

  let%expect_test "to_string_precision_second" = 
    Stdio.print_endline (to_string_precision Precision.Second 1529349109966270847L);
    [%expect{| 1529349109 |}]

  let%expect_test "to_string_precision_minute" = 
    Stdio.print_endline (to_string_precision Precision.Minute 1529349109966270847L);
    [%expect{| 25489151 |}]

  let%expect_test "to_string_precision_hour" = 
    Stdio.print_endline (to_string_precision Precision.Hour 1529349109966270847L);
    [%expect{| 424819 |}]
...
module Point = struct
  ...
  let%expect_test "to_line_seconds" = 
    Stdio.print_endline (to_line ~precision:Precision.Second {
        name = "count";
        field = ("value", Int 100);
        tags = [("tag1","val1");("tag2","val2")];
