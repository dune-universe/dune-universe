...
(* handle to access result from query *)
type result
...
let quick_connect ?options ?host ?database ?port ?password ?user ?socket () =
  connect ?options { host; name = database; port; pwd = password; user; socket }


external change_user : dbd -> db -> unit = "db_change_user"

let quick_change ?user ?password ?database conn =
  change_user conn { defaults with user; pwd = password; name = database }


external select_db : dbd -> string -> unit = "db_select_db"

external list_dbs : dbd -> ?pat:string -> unit -> string array option
  = "db_list_dbs"
...
let escape s =
  init () ;
  (* libmariadb crashes without init *)
  db_escape s
...
let set2ml str =
  ...
  and loop acc p_start i =
    if i = String.length str
    then String.sub str ~pos:p_start ~len:(String.length str - p_start) :: acc
    else
      match str.[i] with
      | ' ' | '\t' | '\n' | '\r' | ',' ->
          wsp (String.sub str ~pos:p_start ~len:(i - p_start) :: acc) (i + 1)
      | _ ->
          loop acc p_start (i + 1)
  in
  List.rev (wsp [] 0)
...
*)
let not_null f arg =
  match arg with None -> fail "not_null was applied to None" | Some x -> f x
...
let column result =
  ...
  let col ~key ~row = row.(StrMap.find key map) in
  col
...
let iter_cols res ~key ~f =
  ...
  iter res ~f:(function row ->
      f (Array.map key ~f:(function key -> col ~key ~row)))
...
let map res ~f =
  ...
  then (
    let rec loop lst =
      match fetch res with Some row -> loop (f row :: lst) | None -> lst
    in
    to_row res Int64.zero ;
    List.rev (loop []) )
  else []
...
module Prepared = struct
  ...
  external execute : stmt -> string array -> stmt_result
    = "caml_mysql_stmt_execute"

  external execute_null : stmt -> string option array -> stmt_result
    = "caml_mysql_stmt_execute_null"
  ...
  external fetch : stmt_result -> string option array option
    = "caml_mysql_stmt_fetch"
