...
#define SET_OPTION(option, p)                      \
  if (0 != mysql_options(init, MYSQL_##option, p)) \
    mysqlfailwith("MYSQL_" #option);               \
  break
#define SET_OPTION_BOOL(option) \
  option_bool = Bool_val(v);    \
  SET_OPTION(option, &option_bool)
#define SET_OPTION_INT(option) \
  option_int = Int_val(v);     \
  SET_OPTION(option, &option_int)
#define SET_OPTION_STR(option) SET_OPTION(option, String_val(v))
#define SET_CLIENT_FLAG(flag) \
  client_flag |= flag;        \
  break
  ...
  else
  {
    while (options != Val_emptylist)
    {
      if (Is_block(Field(options, 0)))
      {
        ...
        case 2:
          SET_OPTION_INT(OPT_SSL_MODE);
        case 3:
          SET_OPTION_BOOL(REPORT_DATA_TRUNCATION);
        case 4:
          SET_OPTION(OPT_PROTOCOL, &ml_mysql_protocol_type[Int_val(v)]);
        case 5:
          SET_OPTION_INT(OPT_CONNECT_TIMEOUT);
        case 6:
          SET_OPTION_INT(OPT_READ_TIMEOUT);
        case 7:
          SET_OPTION_INT(OPT_WRITE_TIMEOUT);
        case 8:
          SET_OPTION_STR(INIT_COMMAND);
        case 9:
          SET_OPTION_STR(READ_DEFAULT_FILE);
        case 10:
          SET_OPTION_STR(READ_DEFAULT_GROUP);
        case 11:
          SET_OPTION_STR(SET_CHARSET_DIR);
        case 12:
          SET_OPTION_STR(SET_CHARSET_NAME);
        case 13:
          SET_OPTION_STR(SHARED_MEMORY_BASE_NAME);
        default:
      ...
      else
      {
        ...
        {
        case 0:
          SET_OPTION(OPT_COMPRESS, NULL);
        case 1:
          SET_OPTION(OPT_NAMED_PIPE, NULL);
        case 2:
          SET_CLIENT_FLAG(CLIENT_FOUND_ROWS);
        default:
          caml_invalid_argument("Mysql.connect: unknown option");
        }
      }
    ...
    caml_enter_blocking_section();
    mysql = mysql_real_connect(init, host, user, pwd, db, port, socket, client_flag);
    caml_leave_blocking_section();
...
value caml_mysql_stmt_execute_gen(value v_stmt, value v_params, int with_null)
{
...
caml_mysql_stmt_fetch(value result)
{
  ...
  caml_leave_blocking_section();
  if (0 != res && MYSQL_DATA_TRUNCATED != res)
    CAMLreturn(Val_none);
