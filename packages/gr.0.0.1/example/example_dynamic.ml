...
let damped_pendulum_deriv (gamma, length) _t state =
  let theta, omega = Arr.( .%{} ) state [| 0 |], Arr.( .%{} ) state [| 1 |] in
  of_array [| omega; (~-.gamma *. omega) -. (9.81 /. length *. Maths.sin theta) |]
...
let pendulum t theta omega acceleration =
  ...
  tex_text (0.05, 0.16) @@ Printf.sprintf "\\theta:%7.2f" (theta /. Float.pi *. 180.)
  |> ignore;
...
let () =
  ...
  while !t < 30. do
    ...
    state := state';
    let theta, omega = Arr.( .%{} ) !state [| 0 |], Arr.( .%{} ) !state [| 1 |] in
    let acceleration = Maths.sqrt (2.0 *. 9.81 *. length *. (1. -. Maths.cos theta)) in
