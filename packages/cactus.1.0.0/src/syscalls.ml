...
external pread_int : Unix.file_descr -> int -> bytes -> int -> int -> int = "caml_index_pread_int"
...
let pread : fd:_ -> fd_offset:int63 -> _ =
  ...
  | False ->
      fun ~fd ~fd_offset ~buffer ~buffer_offset ~length ->
        pread_int64 fd (Int63.Boxed.to_int64 fd_offset) buffer buffer_offset length

external pwrite_int : Unix.file_descr -> int -> bytes -> int -> int -> int = "caml_index_pwrite_int"
...
let pwrite : fd:_ -> fd_offset:int63 -> _ =
  ...
  | False ->
      fun ~fd ~fd_offset ~buffer ~buffer_offset ~length ->
        pwrite_int64 fd (Int63.Boxed.to_int64 fd_offset) buffer buffer_offset length
