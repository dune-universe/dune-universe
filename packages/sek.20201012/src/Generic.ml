...
(* We use unchecked iterators when we are certain that we cannot possibly
   ...
   validity check when we are finished. *)

(* In [to_seq], for instance, our iterator can be invalidated if the user
   ...
   used. To detect this, we use a checked iterator. *)

(* In [iter2_segments], for instance, our iterators can be invalidated if the
   ...
   use unchecked iterators and verify at the end that they are still valid. *)

(* Checked iterators are more costly when [check_iterator_validity] is [true].
   When it is [false], they are the same as unchecked iterators. *)
...
module[@inline] IteratorsInit (S : sig
  ...
  let to_seq pov s =
    let it = S.Iter.create pov s in (* checked iterator *)
    let rec produce () =
      try
        let x = S.Iter.get_and_move pov it in
        Seq.Cons (x, produce)
      with End ->
        Seq.Nil
    in
  ...
  (* We use a checked iterator because a call to [f] might modify the sequence
     [s]. Re-implementing this code using [iter_segments] would eliminate this
     inefficiency. *)

  let[@inline] mapi d f s =
    let it = S.Iter.create forward s in (* checked iterator *)
    unchecked_init d (length s) (fun i ->
      let x = S.Iter.get_and_move forward it in
      f i x
    )
  ...
  let rev s =
    let it = S.UncheckedIter.create backward s in (* unchecked iterator *)
    unchecked_init (default s) (length s) (fun _i ->
      S.UncheckedIter.get_and_move backward it
    )
  ...
  (* [with_final_validity_check it1 it2 f] executes f()] and checks that the
     iterators [it1] and [it2] are still valid at the end. *)

  let[@inline] with_final_validity_check it1 it2 f =
    Adapters.try_finally f (fun () ->
      if not (is_valid it1 && is_valid it2) then
        invalid_arg
          "ephemeral sequence was modified while iteration was ongoing"
    )
  ...
  let[@inline] iter2_segments pov s1 s2 f =
    ...
    with_final_validity_check it1 it2 (fun () ->
      try
  ...
  (* We assume that the function [f] cannot invalidate [it1] and [it2],
     so we do not perform a final validity check. This assumption is
     safe because [bounded_iter2_segments] is not exposed to the user. *)
  ...
  let map2 d f s1 s2 =
    let it1 = S.Iter.create forward s1    (* checked iterator *)
    and it2 = S.Iter.create forward s2 in (* checked iterator *)
    unchecked_init d (min (length s1) (length s2)) (fun _i ->
      let x1 = S.Iter.get_and_move forward it1
      and x2 = S.Iter.get_and_move forward it2 in
      f x1 x2
    )
  ...
  (* In theory, there is a risk that the user-provided function [cmp] might
     modify the collections [s1] or [s2], so we use checked iterators. *)

  let merge cmp s1 s2 =
    let module I = S.Iter in
    let it1 = I.create forward s1    (* checked iterator *)
    and it2 = I.create forward s2 in (* checked iterator *)
    unchecked_init (default s1) (length s1 + length s2) (fun _i ->
      if I.finished it1 then
        I.get_and_move forward it2
      else if I.finished it2 then
        I.get_and_move forward it1
      else
        let x1 = I.get it1
        and x2 = I.get it2 in
        let c = cmp x1 x2 in
