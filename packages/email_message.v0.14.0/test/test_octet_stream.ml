...
let%expect_test "base64" =
  ...
  let%bind () = [%expect {| YW55IGNhcm5hbCBwbGVhc3VyZS4= |}] in
  test "any carnal pleasure";
  let%bind () = [%expect {| YW55IGNhcm5hbCBwbGVhc3VyZQ== |}] in
  test "any carnal pleasur";
  let%bind () = [%expect {| YW55IGNhcm5hbCBwbGVhc3Vy |}] in
  test "any carnal pleasu";
  let%bind () = [%expect {| YW55IGNhcm5hbCBwbGVhc3U= |}] in
  test "any carnal pleas";
  let%bind () = [%expect {| YW55IGNhcm5hbCBwbGVhcw== |}] in
  test
...
let%expect_test "quoted printable" =
  ...
  let%bind () = [%expect {| =00=01=02a=03   =20 |}] in
  test "\000\001\002a\003    \n";
  let%bind () = [%expect {| =00=01=02a=03   =20 |}] in
  test "\000\001\002a\003    \n";
  let%bind () = [%expect {| =00=01=02a=03   =20 |}] in
